...
文件头注释可声明测试性质，`yaoxiang test` 据此判定：

```yaoxiang
// [test:error]: <应报错的原因>
```

- `[test:error]` — 本文件**应编译失败**（验证编译器正确报错，06-compile-errors 目录用）。
  runner 反向判定：`yaoxiang run` 退出码非 0 = PASS；退出码 0 = FAIL（该报错没报）。
  标记后的 `预期: 编译错误 E2018` 行可附期望错误码（暂为文档性，不做校验）。
- `[test:ignore]: <原因>` — 本文件被跳过（追踪 issue 编号，见提交检查清单）。
