//! ref 表达式 Parser 测试
//!
//! 测试 ref 表达式的解析器功能是否正常（RFC-004 支持）
...
mod ref_tests {
    ...
    fn test_ref_keyword_recognized() {
        // ref 应该是独立的关键字
        let tokens = tokenize("ref x").unwrap();
    ...
    fn test_ref_with_identifier() {
        let tokens = tokenize("ref myvar").unwrap();
    ...
    fn test_ref_expression_parsed() {
        ...
        let tokens = tokenize("x = ref y").unwrap();
    ...
    fn test_ref_assignment() {
        let tokens = tokenize("shared = ref obj").unwrap();
        let result = parse(&tokens);
    ...
    fn test_ref_in_simple_assignment() {
        let tokens = tokenize("r = ref x").unwrap();
        let result = parse(&tokens);
