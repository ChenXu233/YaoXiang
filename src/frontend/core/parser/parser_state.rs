...
impl<'a> ParserState<'a> {
    ...
    pub fn at_end(&self) -> bool {
        self.pos >= self.tokens.len() || matches!(self.current().map(|t| &t.kind), Some(TokenKind::Eof))
    }
    ...
    pub fn parse_statement(&mut self) -> Option<crate::frontend::core::parser::ast::Stmt> {
        ...
        match self.current().map(|t| &t.kind) {
            ...
            Some(kw @ TokenKind::KwRef) | Some(kw @ TokenKind::KwUnsafe) | Some(kw @ TokenKind::KwElif) | Some(kw @ TokenKind::KwElse) | Some(kw @ TokenKind::KwIn) | Some(kw @ TokenKind::KwAs) => {
                let kw = match kw { TokenKind::KwRef => "ref", TokenKind::KwUnsafe => "unsafe", TokenKind::KwElif => "elif", TokenKind::KwElse => "else", TokenKind::KwIn => "in", TokenKind::KwAs => "as", _ => "keyword" };
                self.error(ErrorCodeDefinition::keyword_as_name(kw).at(ss).build()); self.bump(); None
            }
