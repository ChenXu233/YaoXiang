...
/// Temporary parser error (refined later to precise error codes)
#[inline]
pub fn parse_msg(msg: impl Into<String>, span: Span) -> Diagnostic {
    ErrorCodeDefinition::invalid_syntax(&msg.into()).at(span).build()
}

pub struct ParserState<'a> { tokens: &'a [Token], pos: usize, errors: Vec<Diagnostic> }

impl<'a> ParserState<'a> {
    pub fn new(tokens: &'a [Token]) -> Self { Self { tokens, pos: 0, errors: Vec::new() } }
    pub fn at_end(&self) -> bool { self.pos >= self.tokens.len() || matches!(self.current().map(|t| &t.kind), Some(TokenKind::Eof)) }
    ...
    pub fn bump(&mut self) -> Option<Token> { let t = self.tokens.get(self.pos).cloned()?; self.pos += 1; Some(t) }
    pub fn at(&self, kind: &TokenKind) -> bool { self.current().map_or(false, |t| &t.kind == kind) }
    pub fn skip(&mut self, kind: &TokenKind) -> bool { if self.at(kind) { self.bump(); true } else { false } }
    pub fn expect(&mut self, kind: &TokenKind) -> bool {
        if self.at(kind) { self.bump(); true }
        else {
            let found = self.current().map(|t| t.kind.clone()).unwrap_or(TokenKind::Eof);
            self.errors.push(ErrorCodeDefinition::expected_token(&format!("{:?}", kind), &format!("{:?}", found)).at(self.span()).build());
            false
        }
    }
    pub fn error(&mut self, error: Diagnostic) { self.errors.push(error); }
    ...
    pub fn can_start_stmt(&self) -> bool { !self.at_end() && !self.at(&TokenKind::Semicolon) }
    pub fn synchronize(&mut self) {
        while !self.at_end() {
            if self.at(&TokenKind::Semicolon) { self.bump(); break; }
            if let Some(TokenKind::Identifier(_)) = self.current().map(|t| &t.kind) {
                if matches!(self.peek().map(|t| &t.kind), Some(TokenKind::Colon | TokenKind::Eq | TokenKind::LParen | TokenKind::LBrace)) { break; }
            }
            self.bump();
        }
    }
    pub fn parse_statement(&mut self) -> Option<crate::frontend::core::parser::ast::Stmt> {
        ...
        match self.current().map(|t| &t.kind) {
            ...
            Some(TokenKind::At) => { self.error(ErrorCodeDefinition::unexpected_token("@").at(ss).build()); None }
            Some(kw @ TokenKind::KwRef) | Some(kw @ TokenKind::KwUnsafe) | Some(kw @ TokenKind::KwElif) | Some(kw @ TokenKind::KwElse) | Some(kw @ TokenKind::KwIn) | Some(kw @ TokenKind::KwAs) => {
                let keyword = match kw { TokenKind::KwRef => "ref", TokenKind::KwUnsafe => "unsafe", TokenKind::KwElif => "elif", TokenKind::KwElse => "else", TokenKind::KwIn => "in", TokenKind::KwAs => "as", _ => "keyword" };
                self.error(ErrorCodeDefinition::keyword_as_name(keyword).at(ss).build()); self.bump(); None
            }
            Some(_) => parse_expr_stmt(self, ss),
        }
    }
    pub fn parse_expression(&mut self, min_bp: u8) -> Option<crate::frontend::core::parser::ast::Expr> {
