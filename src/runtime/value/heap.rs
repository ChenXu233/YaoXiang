...
//! to enable efficient in-place modification of collection types without
//! needing to clone the entire value.
...
/// RuntimeValue holds a Handle that references these values.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum HeapValue {
    /// Tuple storage
...
impl Heap {
    ...
    ///
    /// # Arguments
    /// * `value` - The HeapValue to allocate
    ///
    /// # Returns
    /// A handle to the allocated value
    pub fn allocate(
        &mut self,
        value: HeapValue,
    ) -> Handle {
    ...
    ///
    /// # Returns
    /// Reference to the HeapValue, or None if handle is invalid
    pub fn get(
        &self,
        handle: Handle,
    ) -> Option<&HeapValue> {
    ...
    ///
    /// # Returns
    /// Mutable reference to the HeapValue, or None if handle is invalid
    pub fn get_mut(
        &mut self,
        handle: Handle,
    ...
    /// * `value` - The new HeapValue
    ///
    /// # Returns
    /// Ok(()) if successful, Err if handle is invalid
    pub fn write(
        &mut self,
        handle: Handle,
        value: HeapValue,
    ...
    ///
    /// # Returns
    /// The deallocated HeapValue, or None if handle was invalid
    pub fn deallocate(
        &mut self,
        handle: Handle,
    ) -> Option<HeapValue> {
...
mod tests {
    ...
    fn test_heap_get_mut() {
        ...
        let handle = heap.allocate(HeapValue::List(vec![RuntimeValue::Int(42)]));
        {
            let value = heap.get_mut(handle).unwrap();
            if let HeapValue::List(items) = value {
                if let Some(RuntimeValue::Int(_n)) = items.get(0) {
                    // Can't modify through &mut reference easily in test
                }
            }
        }
        assert!(heap.is_valid(handle));
    ...
    fn test_heap_write() {
        let mut heap = Heap::new();
        let handle = heap.allocate(HeapValue::List(vec![RuntimeValue::Int(42)]));
        heap.write(handle, HeapValue::List(vec![RuntimeValue::Int(100)]))
            .unwrap();
    ...
    #[test]
    fn test_heap_invalid_handle() {
        let heap = Heap::new();
        let handle = Handle(999);
        assert!(!heap.is_valid(handle));
    ...
    fn test_heap_dict() {
        ...
        map.insert(RuntimeValue::Int(1), RuntimeValue::Int(100));
        map.insert(RuntimeValue::Int(2), RuntimeValue::Int(200));
        let handle = heap.allocate(HeapValue::Dict(map));
        match heap.get(handle) {
            Some(HeapValue::Dict(m)) => {
                assert_eq!(m.len(), 2);
            }
