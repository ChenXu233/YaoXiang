...
impl VM {
    ...
    pub fn new_with_config(config: VMConfig) -> Self {
        Self {
            ...
            allocator: RefCell::new(BumpAllocator::with_capacity(64 * 1024)),
    ...
    fn memory_read_i64(
        ...
        let memory = &mut self.allocator.borrow_mut().buffer;
    ...
    fn memory_read_i32(
        ...
        if addr + 4 <= memory.len() {
            let mut bytes = [0u8; 4];
            bytes.copy_from_slice(&memory[addr..addr + 4]);
