...
use crate::middle::ir::{BasicBlock, FunctionIR, Instruction, ModuleIR};
use crate::middle::module::{ModuleGraph, ModuleId};
use crate::middle::monomorphize::module_state::{
...
/// 管理所有模块的泛型定义和跨模块实例化
#[derive(Debug)]
pub struct GlobalMonomorphizer {
    /// 模块依赖图
...
impl GlobalMonomorphizer {
    ...
    pub fn add_dependency(
        ...
        to: ModuleId,
        is_public: bool,
    ) -> Result<(), crate::middle::module::ModuleGraphError> {
            ...
            if let Some(items) = &import.items {
                for item in items {
                    ...
                    state.record_import(
                        ...
                        item.clone(),
                        vec![], // TODO: 提取类型参数
                        GlobalMonomorphizer::is_function_name(item),
                    );
                }
            }
        }
    ...
    pub fn instantiate_function_global(
        ...
        // 执行类型替换
        let specialized = self.substitute_types(&generic_func, type_args.clone());
    ...
    pub fn instantiate_type_global(
        ...
        // 执行类型替换
        let specialized =
            self.substitute_type(&generic_type, &type_args, &generic_info.type_params);
    ...
    fn substitute_types(
        ...
        FunctionIR {
            name: self.generate_specialized_name(&generic_func.name, &type_args),
            params: new_params,
    ...
    fn substitute_single_type(
        ...
        match ty {
            ...
            MonoType::Arc(inner) => {
                MonoType::Arc(Box::new(self.substitute_single_type(inner, type_map)))
            }
            MonoType::Range { elem_type } => MonoType::Range {
                elem_type: Box::new(self.substitute_single_type(elem_type, type_map)),
            },
    ...
    fn substitute_type(
        ...
        self.substitute_type_with_map(generic_type, &type_map)
    }
            ...
            MonoType::Struct(struct_type) => MonoType::Struct(StructType {
                ...
                fields: struct_type
                    ...
                    .map(|(name, field_ty)| {
                        (
                            name.clone(),
                            self.substitute_type_with_map(field_ty, type_map),
                        )
                    })
                    .collect(),
            }),
            MonoType::List(elem) => {
                MonoType::List(Box::new(self.substitute_type_with_map(elem, type_map)))
            }
            MonoType::Dict(key, value) => MonoType::Dict(
                Box::new(self.substitute_type_with_map(key, type_map)),
                Box::new(self.substitute_type_with_map(value, type_map)),
            ),
            MonoType::Set(elem) => {
                MonoType::Set(Box::new(self.substitute_type_with_map(elem, type_map)))
            }
            MonoType::Tuple(types) => MonoType::Tuple(
                types
                    .iter()
                    .map(|t| self.substitute_type_with_map(t, type_map))
                    .collect(),
            ),
            MonoType::Fn {
                params,
                ...
                params: params
                    ...
                    .map(|t| self.substitute_type_with_map(t, type_map))
                    .collect(),
                return_type: Box::new(self.substitute_type_with_map(return_type, type_map)),
                is_async: *is_async,
            },
            MonoType::Arc(inner) => {
                MonoType::Arc(Box::new(self.substitute_type_with_map(inner, type_map)))
            }
            MonoType::Range { elem_type } => MonoType::Range {
                elem_type: Box::new(self.substitute_type_with_map(elem_type, type_map)),
            },
            MonoType::Union(types) | MonoType::Intersection(types) => {
                let substituted: Vec<MonoType> = types
                    .iter()
                    .map(|ty| self.substitute_type_with_map(ty, type_map))
                    .collect();
                if matches!(ty, MonoType::Union(_)) {
                    MonoType::Union(substituted)
                } else {
...
impl Default for GlobalMonomorphizer {
    fn default() -> Self {
        Self::new()
    }
}
