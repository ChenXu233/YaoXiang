...
fn test_same_name_different_params() {
    ...
    mono.test_insert_generic_function(id1.clone(), ir1);
...
fn test_closure_instance_accessors() {
    let capture_vars = vec![crate::middle::passes::mono::instance::CaptureVariable::new(
        "x".to_string(),
        int_type(),
        Operand::Local(0),
    )];
...
fn test_type_instance_accessors() {
    ...
    let mut instance = crate::middle::passes::mono::instance::TypeInstance::new(
        type_id.clone(),
        generic_id.clone(),
        vec![int_type()],
    );
