// scripts/i18n/migrate-flat-keys.mjs
//
// 一次性数据迁移：把 locale JSON 里的扁平错误码键折叠回嵌套对象。
//
// 背景：scripts/i18n/adapters/locales.mjs 的写回逻辑曾要求「目标语言已有该码的
...
//
// bot 是 hash 增量缓存（scripts/i18n/cache.mjs），键没变不会重翻，
// 所以存量扁平键不会自愈，必须迁移一次。
//
// 冲突处理：若某码同时有嵌套与扁平两份，以**嵌套为准**（来自更晚的权威注册表
// 机制 #320），扁平作废。
//
// 用法：node scripts/i18n/migrate-flat-keys.mjs [--dry-run]
...
export function foldFlatCodeKeys(json) {
  const result = {};
  const folded = [];
  const dropped = [];
  ...
  for (const [key, value] of Object.entries(json)) {
    ...
    if (dot > 0 && CODE_KEY.test(key.slice(0, dot))) {
      ...
      if (existing !== undefined && Object.hasOwn(existing, field)) {
        dropped.push(key);
        continue;
      }
      result[head] = { ...(existing ?? {}), [field]: value };
      folded.push(key);
      continue;
    }
    result[key] = value;
...
function main() {
  ...
  for (const file of fs.readdirSync(LOCALES).sort()) {
    ...
    const { json, folded, dropped } = foldFlatCodeKeys(JSON.parse(raw));
