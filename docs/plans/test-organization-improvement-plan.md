# YaoXiang 测试组织改进计划

> 日期：2026-01-03  
> 审核人：Roo (Architect Mode)  
> 文档版本：v1.1.0 (根据用户反馈更新)

---

## 一、现状分析

### 1.1 测试目录结构现状

```
yaoxiang/
├── src/
│   ├── frontend/
│   │   ├── parser/
│   │   │   ├── mod.rs
│   │   │   └── tests/              ✅ 有 tests 子目录 (5个文件)
│   │   │       ├── mod.rs
│   │   │       ├── basic.rs
│   │   │       ├── boundary.rs
│   │   │       ├── state.rs
│   │   │       └── type_parser.rs
│   │   │
│   │   └── typecheck/
│   │       ├── mod.rs
│   │       └── tests/              ✅ 有 tests 子目录 (7个文件)
│   │           ├── mod.rs
│   │           ├── basic.rs
│   │           ├── check.rs
│   │           ├── error.rs
│   │           ├── generic.rs
│   │           ├── infer.rs
│   │           └── types.rs
│   │
│   ├── middle/
│   │   ├── codegen/                ❌ 没有 tests 子目录
│   │   ├── monomorphize/           ❌ 没有 tests 子目录
│   │   ├── escape_analysis/        ❌ 没有 tests 子目录
│   │   └── ir.rs                   ❌ 没有配套测试
│   │
│   ├── vm/
│   │   ├── errors.rs               ⚠️ 内联测试
│   │   ├── executor.rs             ⚠️ 内联测试
│   │   ├── frames.rs               ⚠️ 内联测试
│   │   ├── instructions.rs         ⚠️ 内联测试
│   │   └── opcode.rs               ⚠️ 内联测试
│   │
│   └── runtime/
│       ├── gc/mod.rs               ⚠️ 内联测试
│       ├── memory/mod.rs           ⚠️ 内联测试
│       └── scheduler/mod.rs        ⚠️ 内联测试
│
│   └── util/
│       └── span.rs                 ⚠️ 内联测试
│
│   └── middle/
│       └── monomorphize/
│           └── instance.rs         ⚠️ 内联测试
│
└── tests/
    └── unit/
        └── codegen.rs              ⚠️ 混合多个模块的测试 (240行)
```

### 1.2 当前测试统计

| 模块 | 测试文件数 | 测试用例数 | 覆盖率 | 状态 |
|------|-----------|-----------|--------|------|
| frontend/parser | 5 | ~50+ | 高 | ✅ 完整 |
| frontend/typecheck | 7 | ~100+ | 高 | ✅ 完整 |
| middle/codegen | 0 | 0 | 0% | ❌ 缺失 |
| middle/monomorphize | 0 | 0 | 0% | ❌ 缺失 |
| middle/escape_analysis | 0 | 0 | 0% | ❌ 缺失 |
| middle/ir | 0 | 0 | 0% | ❌ 缺失 |
| vm (内联测试) | 5 | ~15 | 中 | ⚠️ 需迁移 |
| runtime (内联测试) | 3 | ~10 | 低 | ⚠️ 需迁移 |
| util/span | 1 | ~5 | 低 | ⚠️ 需迁移 |
| middle/monomorphize/instance | 1 | ~5 | 低 | ⚠️ 需迁移 |
| **总计** | **22** | **~190+** | **~25%** | **不完整** |

---

## 二、问题清单

### 2.1 严重问题 (Critical)

| # | 问题 | 影响 | 位置 |
|---|------|------|------|
| 1 | **测试目录不一致** | 前端模块有 `tests/` 子目录，中端模块没有，导致测试组织混乱 | `src/middle/` |
| 2 | **测试文件臃肿** | `tests/unit/codegen.rs` 混合了 opcode、单态化、逃逸分析、IR、字节码测试，难以维护 | `tests/unit/codegen.rs:1-240` |
| 3 | **中端模块缺少测试** | `codegen/`, `monomorphize/`, `escape_analysis/` 完全没有单元测试 | `src/middle/` |
| 4 | **IR 缺少测试** | 核心数据结构 IR 没有配套测试 | `src/middle/ir.rs` |

### 2.2 中等问题 (Major)

| # | 问题 | 影响 | 位置 |
|---|------|------|------|
| 5 | **VM 缺少测试** | 虚拟机核心组件没有测试，执行逻辑无法验证 | `src/vm/` |
| 6 | **Runtime 缺少测试** | GC、内存管理、调度器没有测试，运行时可靠性无法保证 | `src/runtime/` |
| 7 | **缺少集成测试** | 没有端到端测试流程，无法验证完整编译管线 | `tests/` |
| 8 | **测试命名不统一** | 有些用 `basic.rs`，有些用 `check.rs`，缺乏统一规范 | 多个位置 |

### 2.3 轻微问题 (Minor)

| # | 问题 | 影响 |
|---|------|------|
| 9 | **缺少性能测试** | 没有基准测试，性能回归无法检测 |
| 10 | **缺少模糊测试** | 没有随机测试，边界情况可能遗漏 |
| 11 | **缺少文档测试** | 文档中的示例代码没有验证机制 |

---

## 三、改进方案

### 3.1 目录结构重构

```
yaoxiang/
├── src/
│   ├── frontend/
│   │   ├── parser/
│   │   │   ├── mod.rs
│   │   │   └── tests/              ✅ 保持现状
│   │   │       ├── mod.rs
│   │   │       ├── basic.rs
│   │   │       ├── boundary.rs
│   │   │       ├── state.rs
│   │   │       └── type_parser.rs
│   │   │
│   │   └── typecheck/
│   │       ├── mod.rs
│   │       └── tests/              ✅ 保持现状
│   │           ├── mod.rs
│   │           ├── basic.rs
│   │           ├── check.rs
│   │           ├── error.rs
│   │           ├── generic.rs
│   │           ├── infer.rs
│   │           └── types.rs
│   │
│   ├── middle/
│   │   ├── codegen/
│   │   │   ├── mod.rs
│   │   │   └── tests/              🔄 新增 - 表达式和语句生成测试
│   │   │       ├── mod.rs
│   │   │       ├── expr.rs         🔄 新增 - 表达式代码生成
│   │   │       └── stmt.rs         🔄 新增 - 语句代码生成
│   │   │
│   │   ├── monomorphize/
│   │   │   ├── mod.rs
│   │   │   ├── instance.rs         🔄 迁移 - 从内联测试
│   │   │   └── tests/              🔄 新增 - 单态化测试
│   │   │       ├── mod.rs
│   │   │       ├── instance.rs     🔄 从 instance.rs 迁移
│   │   │       └── specialization.rs 🔄 新增 - 特化策略
│   │   │
│   │   ├── escape_analysis/
│   │   │   ├── mod.rs
│   │   │   └── tests/              🔄 新增 - 逃逸分析测试
│   │   │       ├── mod.rs
│   │   │       ├── propagate.rs    🔄 新增 - 传播逻辑
│   │   │       └── allocation.rs   🔄 新增 - 分配决策
│   │   │
│   │   └── ir/
│   │       ├── mod.rs
│   │       └── tests/              🔄 新增 - IR 数据结构测试
│   │           ├── mod.rs
│   │           └── instruction.rs  🔄 新增 - 指令测试
│   │
│   ├── vm/
│   │   ├── opcode.rs
│   │   ├── errors.rs
│   │   ├── executor.rs
│   │   ├── frames.rs
│   │   ├── instructions.rs
│   │   └── tests/                  🔄 新增 - 从内联测试迁移
│   │       ├── mod.rs
│   │       ├── opcode.rs           🔄 从 opcode.rs 迁移
│   │       ├── errors.rs           🔄 从 errors.rs 迁移
│   │       ├── executor.rs         🔄 从 executor.rs 迁移
│   │       ├── frames.rs           🔄 从 frames.rs 迁移
│   │       └── instructions.rs     🔄 从 instructions.rs 迁移
│   │
│   ├── runtime/
│   │   ├── gc/
│   │   │   ├── mod.rs
│   │   │   └── tests/              🔄 新增 - 从内联测试迁移
│   │   │       ├── mod.rs
│   │   │       └── collector.rs    🔄 从 gc/mod.rs 迁移
│   │   │
│   │   ├── memory/
│   │   │   ├── mod.rs
│   │   │   └── tests/              🔄 新增 - 从内联测试迁移
│   │   │       ├── mod.rs
│   │   │       └── allocator.rs    🔄 从 memory/mod.rs 迁移
│   │   │
│   │   └── scheduler/
│   │       ├── mod.rs
│   │       └── tests/              🔄 新增 - 从内联测试迁移
│   │           ├── mod.rs
│   │           └── task.rs         🔄 从 scheduler/mod.rs 迁移
│   │
│   └── util/
│       └── span.rs
│           └── tests/              🔄 新增 - 从内联测试迁移
│               ├── mod.rs
│               └── span.rs         🔄 从 span.rs 迁移
│
│   └── frontend/
│       └── lexer/
│           └── tests/              🔄 新增 - 从内联测试迁移
│               ├── mod.rs
│               └── lexer.rs        🔄 从 lexer/mod.rs 迁移
│
└── tests/
    ├── unit/                       ❌ 删除 - 单元测试移到模块内
    │
    └── integration/               ✅ 保留 - 集成测试和端到端测试
        ├── mod.rs
        ├── parser_typecheck.rs     🔄 新增 - 前端集成测试
        ├── codegen_vm.rs           🔄 新增 - 后端集成测试
        └── end_to_end.rs           🔄 新增 - 端到端测试
```

### 3.2 测试策略

| 测试类型 | 位置 | 说明 |
|---------|------|------|
| **单元测试** | `src/module/tests/` | 所有模块的单元测试，测试内部实现 |
| **内联测试** | 全部移除 | 迁移到 `tests/` 子目录 |
| **集成测试** | `tests/integration/` | 跨模块测试，验证模块间协作 |
| **端到端测试** | `tests/integration/` | 完整编译管线测试 |

### 3.3 测试命名规范

| 测试类型 | 文件命名 | 示例 |
|---------|---------|------|
| 单元测试 | `{模块名}.rs` | `expr.rs`, `stmt.rs` |
| 边界测试 | `boundary.rs` | 边界条件和异常情况 |
| 错误测试 | `error.rs` | 错误处理和消息 |
| 泛型测试 | `generic.rs` | 泛型代码测试 |
| 集成测试 | `integration/` | 跨模块测试 |
| 端到端测试 | `end_to_end.rs` | 完整流程测试 |

### 3.3 测试用例规范

```rust
// 文件: src/middle/codegen/tests/expr.rs

#[cfg(test)]
mod binary_operation_tests {
    use super::*;

    /// 测试用例命名规范：test_<功能>_<场景>
    #[test]
    fn test_i64_add_positive_numbers() {
        // Given: 准备测试数据
        let left = Operand::Const(ConstValue::Int(10));
        let right = Operand::Const(ConstValue::Int(20));
        
        // When: 执行被测函数
        let result = generate_binop(&BinOp::Add, &left, &right);
        
        // Then: 验证结果
        assert!(result.is_ok());
    }

    /// 测试浮点数加法
    #[test]
    fn test_f64_add_decimal_numbers() {
        let left = Operand::Const(ConstValue::Float(3.14));
        let right = Operand::Const(ConstValue::Float(2.86));
        
        let result = generate_binop(&BinOp::Add, &left, &right);
        assert!(result.is_ok());
    }

    /// 测试溢出情况
    #[test]
    fn test_i64_add_overflow() {
        let max = Operand::Const(ConstValue::Int(i64::MAX));
        let one = Operand::Const(ConstValue::Int(1));
        
        let result = generate_binop(&BinOp::Add, &max, &one);
        // 应该返回错误或正确处理溢出
        assert!(result.is_err());
    }
}
```

---

## 四、行动计划

### 阶段一：迁移内联测试到模块内的 tests/ 子目录 (P0)

| 任务 | 源文件 | 目标目录 | 依赖 |
|------|--------|---------|------|
| 迁移 vm/opcode.rs 内联测试 | `src/vm/opcode.rs` | `src/vm/tests/opcode.rs` | 无 |
| 迁移 vm/errors.rs 内联测试 | `src/vm/errors.rs` | `src/vm/tests/errors.rs` | 任务1 |
| 迁移 vm/executor.rs 内联测试 | `src/vm/executor.rs` | `src/vm/tests/executor.rs` | 任务1 |
| 迁移 vm/frames.rs 内联测试 | `src/vm/frames.rs` | `src/vm/tests/frames.rs` | 任务1 |
| 迁移 vm/instructions.rs 内联测试 | `src/vm/instructions.rs` | `src/vm/tests/instructions.rs` | 任务1 |
| 迁移 runtime/gc 内联测试 | `src/runtime/gc/mod.rs` | `src/runtime/gc/tests/mod.rs` | 无 |
| 迁移 runtime/memory 内联测试 | `src/runtime/memory/mod.rs` | `src/runtime/memory/tests/mod.rs` | 任务6 |
| 迁移 runtime/scheduler 内联测试 | `src/runtime/scheduler/mod.rs` | `src/runtime/scheduler/tests/mod.rs` | 任务6 |
| 迁移 util/span 内联测试 | `src/util/span.rs` | `src/util/span/tests/mod.rs` | 无 |
| 迁移 frontend/lexer 内联测试 | `src/frontend/lexer/mod.rs` | `src/frontend/lexer/tests/mod.rs` | 无 |
| 迁移 monomorphize/instance 内联测试 | `src/middle/monomorphize/instance.rs` | `src/middle/monomorphize/tests/instance.rs` | 无 |

### 阶段二：拆分 tests/unit/codegen.rs (P1)

| 任务 | 说明 | 依赖 |
|------|------|------|
| 拆分 opcode_tests | 移到 `src/vm/tests/opcode.rs` | 阶段一任务1 |
| 拆分 monomorphize_tests | 移到 `src/middle/monomorphize/tests/` | 阶段一任务11 |
| 拆分 escape_analysis_tests | 移到 `src/middle/escape_analysis/tests/` | 阶段一后新建 |
| 拆分 codegen_tests | 移到 `src/middle/codegen/tests/` | 阶段一后新建 |
| 拆分 bytecode_instruction_tests | 移到 `src/middle/codegen/tests/` | 阶段一后新建 |

### 阶段三：新增缺失测试目录 (P1)

| 任务 | 说明 | 依赖 |
|------|------|------|
| 创建 middle/codegen/tests/ | 代码生成测试目录 | 阶段一完成后 |
| 创建 middle/escape_analysis/tests/ | 逃逸分析测试目录 | 阶段一完成后 |
| 创建 middle/ir/tests/ | IR 测试目录 | 阶段一完成后 |
| 删除 tests/unit/ 目录 | 单元测试已迁移到模块内 | 阶段二完成后 |

### 阶段四：创建集成测试 (P2)

| 任务 | 说明 | 依赖 |
|------|------|------|
| 创建 tests/integration/ 目录 | 集成测试目录 | 阶段三完成后 |
| 创建 parser_typecheck.rs | 前端集成测试 | 阶段三完成后 |
| 创建 codegen_vm.rs | 后端集成测试 | 阶段三完成后 |
| 创建 end_to_end.rs | 端到端测试 | 阶段三完成后 |

---

## 五、测试覆盖目标

### 5.1 覆盖指标

| 模块 | 当前覆盖率 | 目标覆盖率 | 差距 |
|------|-----------|-----------|------|
| frontend/parser | ~90% | 95% | +5% |
| frontend/typecheck | ~85% | 95% | +10% |
| middle/codegen | 0% | 80% | +80% |
| middle/monomorphize | ~10% | 80% | +70% |
| middle/escape_analysis | ~10% | 80% | +70% |
| middle/ir | 0% | 90% | +90% |
| vm | ~20% → 80% | 80% | +60% |
| runtime | ~10% → 60% | 70% | +60% |
| util | ~10% → 60% | 80% | +70% |
| frontend/lexer | ~10% → 60% | 80% | +70% |
| **总体** | **~25%** | **80%** | **+55%** |

### 5.2 测试类型分布

```
测试类型分布目标：
├── 模块内单元测试 (src/module/tests/)  70%
│   ├── 数据结构测试                    20%
│   ├── 算法测试                        30%
│   └── 边界测试                        20%
├── 集成测试 (tests/integration/)       20%
│   ├── 模块间测试                      10%
│   └── 管线测试                        10%
└── 端到端测试 (tests/integration/)     10%
```

### 5.3 迁移统计

| 指标 | 当前 | 目标 |
|------|------|------|
| 内联测试模块数 | 12 | 0 |
| 模块内 tests/ 目录数 | 2 | 17 |
| tests/unit/ 文件数 | 1 | 0 |
| tests/integration/ 文件数 | 0 | 4 |

---

## 六、总结

本计划提出了以下改进措施：

1. **迁移所有内联测试** - 将 12 个文件中的内联测试模块迁移到模块内的 `tests/` 子目录
2. **统一测试目录结构** - 为所有模块添加 `tests/` 子目录，保持一致性
3. **拆分臃肿测试文件** - 将 `tests/unit/codegen.rs` 拆分为多个专用测试文件
4. **新增缺失测试** - 为 codegen、monomorphize、escape_analysis、ir 添加单元测试
5. **建立集成测试** - 创建前端、后端、端到端集成测试
6. **清理 tests/ 目录** - 删除 `tests/unit/` 目录，只保留 `tests/integration/`

### 迁移前后对比

```
迁移前：                              迁移后：
├── tests/                           ├── tests/
│   └── unit/                       │   └── integration/  ← 只保留集成测试
│       └── codegen.rs              │       ├── parser_typecheck.rs
│                                   │       ├── codegen_vm.rs
├── src/vm/opcode.rs                │       └── end_to_end.rs
│   └── #[cfg(test)]                │
│       mod tests { ... }           ├── src/vm/
│                                   │   └── tests/  ← 新增
├── src/runtime/gc/mod.rs           │       ├── opcode.rs  ← 从 opcode.rs 迁移
│   └── #[cfg(test)]                │       ├── errors.rs  ← 从 errors.rs 迁移
│       mod tests { ... }           │       └── executor.rs ← 从 executor.rs 迁移
│   ... (12个文件)                   │
│                                   └── src/runtime/gc/
│                                       └── tests/  ← 新增
│                                           └── collector.rs ← 从 mod.rs 迁移
```

实施本计划后：
- 测试覆盖率预计从 ~25% 提升到 80%
- 内联测试模块从 12 个减少到 0 个
- 模块内 tests/ 目录从 2 个增加到 17 个
- 测试组织更加一致和可维护

---

> 「千里之行，始于足下。」建议从迁移 vm 模块的内联测试开始，逐步完善测试体系。
