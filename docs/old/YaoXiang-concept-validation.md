...
| 4 | `mod` | 模块定义 | `mod Math { ... }` |
| 5 | `use` | 导入模块 | `use std.io` |
| 6 | `spawn` | 异步标记 | `fn fetch() -> T spawn { ... }` |
| 7 | `ref` | 不可变引用 | `fn foo(x: ref T) { ... }` |
| 8 | `mut` | 可变引用 | `fn bar(x: mut T) { ... }` |
| 9 | `if` | 条件分支 | `if x > 0 { ... }` |
| 10 | `elif` | 多重条件 | `elif x == 0 { ... }` |
| 11 | `else` | 默认分支 | `else { ... }` |
| 12 | `match` | 模式匹配 | `match x { 0 -> "zero" }` |
| 13 | `while` | 条件循环 | `while i < 10 { ... }` |
| 14 | `for` | 迭代循环 | `for item in items { ... }` |
| 15 | `return` | 返回值 | `return result` |
| 16 | `break` | 跳出循环 | `break` |
| 17 | `continue` | 继续循环 | `continue` |
| 18 | `as` | 类型转换 | `x as Float` |
