...
#### 2.7 MVP 最小化路线（最小可行产品）

> **目标**：快速验证 YaoXiang 语言的可行性，运行第一个程序
...
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│   P1:Lexer ──▶ P2:Parser ──▶ P3:TypeCheck ──▶ P4:Codegen ──▶ Bytecode    │
│                                                                             │
│   • 输入：.yx 源文件                                                       │
│   • 输出：.yxb 字节码文件                                                   │
│                                                                             │
│   验证方式：yaoxiangc compile hello.yx -o hello.yxb                         │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│   P8:Value + Allocator（最小值表示 + 内存分配）                             │
│   P11:VM（最小解释器）                                                      │
│                                                                             │
│   • 输入：.yxb 字节码文件                                                   │
│   • 输出：程序执行结果                                                      │
│                                                                             │
│   验证方式：yaoxiang run hello.yxb                                          │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

**必需实现**：
- Value 类型：Int、Float、Bool、String
- 简单内存分配器：Arena 或 BumpAllocator
- VM 指令解释器：基础指令集
- 标准输入输出：print 函数
...
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│   1. P9:DAG 任务依赖图 ──▶ 支持 spawn 并发                                  │
│   2. P10:Scheduler ──▶ 多任务调度                                          │
│   3. P5:逃逸分析 ──▶ 内存优化                                              │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│   7. P12:Embedded Runtime ──▶ WASM/脚本嵌入                                │
│   8. P13:WorkStealer ──▶ 负载均衡                                          │
│   9. P14:@blocking ──▶ 同步执行                                            │
│  10. P15:JIT ──▶ 热点代码编译                                              │
│  11. P16:Debugger ──▶ 调试支持                                            │
│  12. P18:Bootstrap ──▶ 自举编译                                            │
│  13. P19:AOT ──▶ 原生编译                                                  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
> **关键决策**：DAG 是否作为 MVP 的一部分？

| 选项 | 说明 | 优点 | 缺点 |
|------|------|------|------|
| **A: MVP 不含 DAG** | P9 可选，用简单调度替代 | 快速验证语言可行性 | 并发功能延迟 |
| **B: MVP 包含 DAG** | 标准 MVP 路线 | 并发功能完整 | 开发周期较长 |

**建议**：选择 **A 选项**，理由：
1. 语言核心可行性验证优先于并发特性
2. DAG 可在 MVP 完成后快速添加
3. 参考 Rust 早期路线：先稳定语言，再发展生态

##### 2.7.6 相关文档

- [Phase 实现方案（RFC-008 对齐版）](../../plan/PHASE-IMPLEMENTATION-PLAN.md)
