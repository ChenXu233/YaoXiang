...
> - [RFC-003: 版本规划与实现建议](003-version-planning.md)
...
> **详见**：[附录A讨论1 - @blocking 与异步实现位置](#讨论1-blocking-与异步实现位置)
...
│  L2    │ 显式 spawn    │ `spawn`        │ Core Runtime       │ 开发者可控       │
│  L3    │ 透明并发      │ 无（默认）     │ Core Runtime       │ 自动最优并行     │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  核心要点：                                                              │   │
│  │  ├── DAG 天然支持异步（num_workers=1 时）                               │   │
│  │  ├── 多线程时自动并发（num_workers>1 时）                               │   │
│  │  └── 同一个核心上也能异步，保持与同步语义一致                            │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│                           Full Runtime（可选）                                  │
...
│  层级  │   模式        │  实现位置      │  说明                               │
│  ──────┼──────────────┼────────────────┼─────────────────────────────────── │
│  L1    │ @blocking    │ Full Runtime   │ 强制急切求值，由标准库提供         │
│  L4    │ WorkStealer  │ Full Runtime   │ num_workers>1 时的负载均衡         │
│  L5    │ 高级 async   │ Full Runtime   │ async/await 语法糖                 │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  核心要点：                                                              │   │
│  │  ├── @blocking 是标准库功能，不是语言内置                               │   │
│  │  ├── WorkStealer 是 num_workers>1 的高级特性                           │   │
│  │  └── Core Runtime 不依赖这些特性也能正常运行                            │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
| Full Runtime | ❌ WorkStealer | num_workers>1 时的高级特性 |
...
> - @blocking API 设计 → RFC-001 或标准库 RFC
...
异步支持是否需要在 Core Runtime 实现？@blocking 注解应该放在哪一层？

#### 备选方案

| 方案 | 描述 | 优点 | 缺点 |
|------|------|------|------|
| A | @blocking 是语言内置 | 用户友好 | 增加语言复杂度 |
| B | @blocking 在 Core Runtime | 性能好 | Core Runtime 膨胀 |
| C | @blocking 由标准库提供 | 解耦、灵活 | 需要导入 |
...
- 2025-01-05 @晨煦：@blocking 由标准库提供，强制函数内的所有语句急切求值，表现为同步

#### 决议

采用方案 C：
- ✅ 异步支持由 DAG 天然支持，在 Core Runtime
- ✅ @blocking 由标准库提供，不在语言内置
- ✅ 保持 Core Runtime 简洁
