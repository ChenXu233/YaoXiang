...
| 层级 | 模式 | 语法 | 执行方式 | 并行度 | 适用场景 |
|------|------|------|----------|--------|----------|
| **L1** | `@block` | `@block` | 无 DAG，纯顺序执行 | ❌ 无 | 调试、新手、关键代码段 |
| **L2** | 显式 spawn | `spawn` | 开发者可控 DAG | ⚠️ 部分 | 中级用户、需要控制并发 |
| **L3** | 完全透明 | 默认 | 自动分析 DAG | ✅ 完整 | 专家、自动并行优化 |
...
3. **阶段3 (v0.5)**： L3 完全透明、DAG 自动分析
4. **阶段4 (v1.0)**： 错误图、图调试器

### 依赖关系

- RFC-001 无外部依赖（基础核心）
- RFC-002（libuv 集成）可并行开发
- RFC-003（版本规划）可并行开发
...
| 决策 | 决定 | 日期 |
|------|------|------|
| 采用三层并发架构 | L1/L2/L3 渐进式 | 2025-01-05 |
| @block 与 spawn 位置一致 | 方案A | 2025-01-05 |
| @ordered 循环语义 | 已删除，用 @eager 替代 | 2025-01-06 |
| DAG 错误传播 | 沿依赖边向上游传播 | 2025-01-06 |
| DAG 性能优化 | 增量构建 + 缓存 | 2025-01-06 |
| 运行时选择机制 | 泛型 + 编译时注入 | 2025-01-06 |
| 节点接口设计 | 泛型 + 函数注入（无 trait） | 2025-01-06 |
| 错误图内存优化 | DAG 仅在单函数内构建 | 2025-01-06 |
| 静态 DAG 优化 | 编译期预构建 DAG 结构 | 2025-01-06 |
| L1 自动回退 | 小函数自动回退到 @block | 2025-01-06 |
| 资源冲突检测 | DAG 数据流依赖，用户变量传递 | 2025-01-06 |
| 副作用一致性 | 不引入副作用系统，用户负责 | 2025-01-06 |
...
| 问题 | 删除原因 |
|------|----------|
| L3 透明模式回退机制缺失 | 不需要降级，通过 `@eager`、`@lazy` 可强制切换运行模式 |
...
**决策**：不需要降级机制，通过 `@eager`、`@lazy` 等语法注解可强制某个函数作用域切换运行模式。
...
| 副作用一致性 | 不引入副作用系统，用户负责 | 2025-01-06 | @晨煦 |
...
| @lazy | 惰性求值注解，默认行为（调度器完整参与，最大化并行） |
| 错误图 | 可视化的错误传播路径 |
