...
language, including type definitions, function declarations, method bindings, and the handling of
opaque types.

> **Detailed Design**: For the complete design, motivation, and trade-offs of FFI, see
...
```
All `return` statements inside `{}` return their content to the enclosing scope
By default, the absence of `return` returns Void
```
...
The compiler automatically distinguishes opaque types from void types:
...
// Void type (not referenced by any native function)
MyType: Type = {}
// → MyType is not referenced by any native function → void type
```
...
FFI function parameter types use YaoXiang types directly; the compiler automatically handles the
mapping to C types:
...
// The compiler will ultimately check all bindings
```
...
(a, b) = spawn {
    ...
    db2 = SqliteDb.open("db2.sqlite")   // different instances, may run in parallel
}
...
If an FFI type is not a resource type, it can be executed in parallel within a `spawn` block:
