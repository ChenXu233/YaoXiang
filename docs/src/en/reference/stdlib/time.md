...
> **This module has several implementation gaps (#338 / #340)**, confirmed by actual testing when
> writing this documentation. Available and unavailable surfaces are marked separately below to
> avoid writing examples that don't compile based on the signatures alone.
>
> Normally available: [`now`](#now) / [`timestamp`](#timestamp) / [`timestamp_ms`](#timestamp_ms) /
> [`sleep`](#sleep) / [`format_time`](#format_time) (accepts `Int` timestamp literals).
>
...
<!-- stdlib:table:time end -->## Getting the Time
...
Returns: a `DateTime` value, which prints like `DateTime(1789471990)`. **It is not an `Int`**, so it
cannot directly participate in arithmetic or comparison, nor can it be passed to other functions as
an `Int` parameter (see [`format_time`](#format_time)).
...
> When you need to use it in calculations, use [`timestamp`](#timestamp) or
...
Sleeps for the specified number of **seconds** (decimals allowed). The homonymous
[`std.concurrent.sleep`](./concurrent#sleep) uses **milliseconds**, so be careful to distinguish
them.

- `seconds` —— number of seconds to sleep; accepts `Int` (interpreted as seconds) or `Float`

Error: throws `E6007` when the parameter is neither `Int` nor `Float`.
...
Decomposed in **local time**. Unrecognized placeholders are kept as-is.

Error: throws `E6007` when `dt` is not `Int`, `fmt` is not `String`, or arguments are insufficient.
...
>    space); any other form will fail, regardless of what `fmt` is set to.
...
Error: throws `E6007` when the format does not match.
...
| `DateTime::to_string` | `(dt: Int) -> String` | ISO 8601 formatted string |

**But these names currently cannot be called from YaoXiang source code (#338).** The export name
contains `::`, and `::` is a reserved token in the syntax, which cannot appear in a field access
position. The following attempts have been confirmed by actual testing to all fail:
...
**Workaround**: when you need date components, use [`format_time`](#format_time) to format as needed
— it already performs the timestamp → year/month/day decomposition internally:
