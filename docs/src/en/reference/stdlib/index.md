...
and then called as `module.function(...)`. This directory contains the API reference documentation,
split by module.
...
| [`std.result`](./result)         | 7       | Construction and unwrapping of Result and Error                         |
...
## Argument Borrow Convention

The `&` in a signature denotes a **read-only automatic borrow** (RFC-009 §2.8): the variable passed
in by the caller is not moved, and can still be used after the call. This is the default form for
the many read-only functions in the standard library.
...
main: () -> Void = {
    ...
    // All three calls only borrow nums read-only; it is still usable afterward
...
Parameters without `&` are **passed by value**. As a result, most "modifying" functions take a
**functional form that consumes the source value and returns a new one**, rather than mutating in
place:
...
The "Semantic Classification" section of each module page lists which functions in that module
borrow, which consume, and which mutate in place. There is one case that needs special attention:

- [`list.pop`](./list#pop) / [`list.remove_at`](./list#remove_at) are declared as `&List(A)`, but
  **mutate the list in place**

## Error Model

The standard library has two forms of failure, each labeled in the function entries as "Error" and
...
| Returns sentinel value | Does not interrupt; returns `Void` / `-1` / `""`      | Out-of-bounds list read, taking the first element of an empty list, missing environment variable |
...
wrapping the failure into a `Result`'s `Err` value to return, which can be unwrapped with
...
> **Move semantics**: both `next` and `has_next` **move** the iterator (the signatures have no `&`),
> so every access needs to recreate it, or simply use `for ... in`.
...
The following issues were confirmed one by one by actually running the examples when writing the
documentation, and are all tracked in issues.

**Fixed (2026-09-19)**: #337 / #338 / #339 / #340 have all been fixed, and the corresponding page
bodies have been rewritten to normal usage descriptions:

| Location                                          | Original issue                                                          | Fix                                           |
| ------------------------------------------------- | ----------------------------------------------------------------------- | --------------------------------------------- |
| [`os.open`](./os#open)                            | Handle was one-shot; `open`→`write`→`close` couldn't compile            | Handle changed to pass by reference ✅        |
| [`time.datetime_*`](./time#datetime-field-access) | 8 accessors couldn't be called from source (export name contained `::`) | Renamed to flat names like `datetime_year` ✅ |
...
- **Generated regions** (between the `<!-- stdlib:KEY start/end -->` markers): the function overview
  table and signature blocks, derived from `StdModule::exports()`. The signatures come byte-for-byte
  from `NativeExport::signature` and cannot drift from the implementation.
- **Handwritten regions** (outside the markers): module overview, borrow/move semantics, error
...
| Coverage          | `test_stdlib_docs_covers_interface_modules`   | The documented module set must cover the interface view |
...
- [FFI Specification](../language-spec/ffi.md) — User-side `native` extension and C ABI binding
