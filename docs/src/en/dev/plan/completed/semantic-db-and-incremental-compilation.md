# Semantic Information Hub and Incremental Compilation Implementation Plan

> **Task**: Implement the Semantic Information Hub, providing LSP semantic highlighting, incremental compilation, and dead code warning capabilities
> **Based on RFC**: This plan is for new feature design
> **Related RFC**: [RFC-008: Runtime Concurrency Model](../design/rfc/accepted/008-runtime-concurrency-model.md) - DAG concurrency belongs to the runtime and is not within the scope of this plan
> **Date**: 2026-02-23
...
This plan decomposes the Semantic Information Hub implementation into 3 main phases. The core idea is **single traversal, multiple uses**:

1. **Semantic collection is done during the typecheck phase** (instead of LSP layer traversing AST separately)
...
> - **DAG concurrency** is a runtime feature (RFC-008) and is not within the scope of this plan
> - **Parallel compilation of module dependency graphs** is a build system feature and is a different concept from runtime DAG
> - Semantic collection should be completed in the typecheck phase, and LSP should reuse it directly instead of writing a separate traverser
...
pub struct ScopeInfo {
    ...
    pub symbols: Vec<String>,   // Symbols within scope
    pub kind: ScopeKind,        // Global, Function, Block, Lambda
}
```

**Acceptance Criteria**:
- [x] SemanticDB structure defined
...
**Design Decision**: Semantic collection **should not** be implemented separately in the LSP layer, but should be done during the typecheck phase.

**Reasons**:
- Typecheck is already traversing the AST and already knows all symbol definitions and reference locations
- LSP implementing SemanticCollector separately = redundant traversal + maintaining two sets of logic
- **Good Taste**: Single traversal, multiple uses

**Implementation Goals**:
- Extend semantic collection functionality in the `src/frontend/typecheck/` module
- Generate `SemanticDB` data during type checking
- LSP layer queries and reuses directly without redundant AST traversal

**Collection Rules by TypeCheck** (Phase Deliverables):
```
...
- Record the list of symbols within each scope
- Support correct parent-child relationships for nested scopes
- Support 4 scope types: Global, Function, Block, Lambda

**Note**: This information is already managed in the typecheck's `TypeEnvironment` and now needs to be exported for SemanticDB use.
...
**Design Adjustment**:
- No longer need to call SemanticCollector separately in the LSP layer
...
- [x] Version number tracked correctly
- [x] Delete operations handled correctly

**Test Items**:
- [x] Add token incremental test
- [x] Delete token incremental test
- [x] Modify token incremental test
...
- Document the mapping between token types and theme colors
...
- Parse import/use statements to construct module dependencies
- Support circular dependency detection

**Data Structure**:
```rust
pub struct ModuleDependencyGraph {
    // Module ID -> List of module IDs it depends on
...
**Implementation Goals**:
- Implement compilation product cache (AST, type information, IR)
...
- [x] Changed files recompile correctly
...
- [x] Cache hit test
- [x] Cache miss test
- [x] Cache serialization test (in-memory cache, Clone approach)
...
- Compile only files affected by changes
...
**Acceptance Criteria**:
- [x] Single file change recompiles only necessary files
...
> **Target Version**: v0.11
> **Dependency**: Phase 1 Complete (semantic information from typecheck phase)

> **Explanation**: Dead code warnings depend on symbol reference information from the typecheck phase and are a compile-time analysis feature, not a runtime feature.

> **Architecture Adjustment**: Dead code analysis is integrated into the typecheck phase because both require traversing AST/SemanticDB
...
**Design Decision**: Dead code analysis should be completed in the **typecheck phase** because:
...
- [x] Terminal output format aesthetic
...
| **Module Dependency Graph** | Phase 3 of this plan | Compiler-level module dependencies, used for incremental compilation |
| **Module-level Parallel Compilation** | Build System | Implement based on Phase 3's dependency graph, not part of LSP |
...
```
┌─────────────────────────────────────────────────────────────────┐
│                    Semantic Information Hub Architecture         │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   Source Code                                                    │
│     │                                                              │
...
1. **Single Traversal**: The typecheck phase produces semantic information simultaneously without redundant AST traversal
2. **Multiple Uses**: LSP semantic highlighting, incremental compilation, and dead code analysis share the same data
3. **Good Taste**: No unnecessary abstraction layers added for the sake of "decoupling"
...
| 2 | `src/lsp/handlers/semantic_tokens.rs` | `src/lsp/handlers/mod.rs` | ✅ Completed (with delta support) |
| 2 | - | `src/lsp/server.rs` | ✅ Completed (added semanticTokens/full + delta request routing) |
