...
To improve the developer experience of YaoXiang, it is necessary to implement a language server that conforms to the Language Server Protocol. This server will be embedded in the compiler frontend, providing real-time code analysis, completion, go-to-definition, diagnostics, and other features, enabling integration with mainstream editors (VS Code, Vim, Emacs, etc.).

## Goals

- Implement the YaoXiang language server binary `yaoxiang-lsp`.
...
- Implement incremental compilation/checking capabilities with the compiler to avoid full re-parsing on every change.

## Steps

1. **Design Server Architecture** (1 week)
   - Build on existing compiler frontend libraries (lexical analysis, parsing, type checking).
   - Use asynchronous I/O (such as Tokio) to handle LSP requests.
   - Design data structures for document caching and incremental updates.

2. **Implement Basic LSP Protocol Communication** (1 week)
   - Implement JSON-RPC message sending and receiving.
...
3. **Integrate Compiler Frontend** (2 weeks)
   ...
   - Maintain a "compilation session" for each document, supporting incremental updates (only re-parsing modified parts).
   - Implement mapping from AST nodes to source code locations for navigation and hover.

4. **Implement Diagnostics Feature** (1 week)
   - Capture errors and warnings during type checking, converting them to LSP diagnostic messages.
...
5. **Implement Completion and Symbol Navigation** (2 weeks)
   ...
   - Implement go-to-definition: Find the file and position of a symbol's definition based on cursor position.
...
6. **Testing and Integration** (1 week)
   - Write a VS Code client plugin (simple wrapper) for testing.
...
- Jumping to definition on an identifier opens the corresponding file and navigates to the definition line.
- After typing a dot, fields of the record type can be completed (requires type information).
- The server responds within an acceptable time (<500ms) on large files (thousands of lines).

## Dependencies

- The compiler frontend library must be stable and provide APIs for obtaining AST, symbol tables, and type information.
