# Compile-Time Computation Engine (CTE Engine) and Hoare Logic Static Verification — Implementation Plan

> **Task**: Implement compile-time evaluation engine + Hoare logic static verification channel to support value-dependent types and compile-time dimension validation  
> **Based on RFCs**: RFC-010 (Unified Type Syntax), RFC-011 (Generics System/Value-Dependent Types), RFC-022 (Hoare Logic Static Verification)  
> **Date**: 2026-05-10  
> **Status**: In Design  
> **Target Milestones**:
...
> - M4: Hoare logic verification (`//!` spec parsing → VC generation → SMT integration)

---

## Abstract

YaoXiang's value-dependent types (RFC-011) require types to depend on compile-time known values (e.g., `Vec(factorial(5))` → `Vec(120)`), and Hoare logic static verification (RFC-022) requires compile-time specification checking for pure functions. Both share the same core requirement: **safely execute/analyze pure functions at compile time**.

This plan proposes a **unified Compile-Time Computation Engine (CTE Engine)**, abstracting purity analysis, termination checking, and expression evaluation as shared infrastructure, serving two consumers: type-level evaluation and Hoare logic verification.
...
2. **Pure functions are compile-time evaluable**: No `const fn` keyword needed; the compiler automatically infers purity
3. **Termination proof = cornerstone of type safety**: Evaluation in type positions must prove termination (`decreases` spec), otherwise the type system becomes undecidable
4. **Partial evaluation preferred over full evaluation**: Evaluate as many known parameters as possible; leave the rest for runtime
5. **Compile-time evaluation and Hoare logic share the interpreter**: The same expression evaluation core supports both consumers
6. **Dual-mode evaluation**: Concrete evaluation (known parameters → produces `CTValue`) and symbolic evaluation (unknown parameters → produces SMT expressions) share the same interpreter framework, differing only in the evaluation environment
...
```
  ...
  │  │ ┌────────────┐ │  │                       │
  │  │ │ Symbolic Eval│─┼──┼───────────────────────┘
  │  │ │ env: partially known│ │  │  (Hoare logic consumer)
  │  │ │ → SMTExpr   │ │  │
  │  │ └────────────┘ │  │
  │  └───────┬────────┘  │
  │          ↓           │
  │  Embed results in types/monomorphization│
  └──────────────────────┘
```
...
enum CTValue {
    ...
    /// Unevaluated function reference (retained for partial evaluation)
...
**Key Design**: `CTValue::Type(TypeId)` makes types first-class compile-time values. The C in `If(C, T, E)` evaluates to `CTValue::Bool`, and T/E evaluate to `CTValue::Type`.
...
| `x: &mut T` (exclusive reference) | Can modify | ⚠️ depends on T's origin |
...
If the compiler misjudges a pure function as impure, that's a compiler bug and should be fixed in the compiler, not patched by users. Providing a "trust me, this function is pure" annotation would only mask the real problem.
...
Compile-time evaluation in type positions must guarantee termination; otherwise the type system becomes undecidable. YaoXiang proves termination via `//! decreases` specifications.

```
//! decreases: <expr>
```

Where `<expr>` is a well-founded value with a lower bound (typically a natural number of integer type).
...
Some obvious termination cases don't need annotations:
...
- **Hoare logic**: decreases variants for loop invariants (`/*! decreases: n - i !*/`) are also verified by the termination checker
...
The interpreter is based on AST traversal, maintaining an evaluation environment (variable name → CTValue mapping). The core capability is **partial evaluation**: evaluate known parameters, preserve unknown ones.
    ...
    match expr:
        ...
        Call(func, args) =>
            ...
            if known_args.len() == args.len():
                ...
                // Partial known → partial evaluation (generate monomorphic code)
...
Compile-time evaluation must have a hard limit to prevent accidental timeouts even with `decreases`:
...
- Function call parameters are all known → inline and evaluate
...
- Known sub-expressions evaluate to concrete values; unknown parts retained as SMT symbols
...
```rust
/// Interpreter trait: concrete and symbolic evaluation each implement
trait Interpreter {
...
**Key Insight**: Concrete and symbolic evaluation share identical AST traversal logic; differences lie only in:
...
## 5. CTE Integration Points

### 5.1 Type-Level Computation Use Cases

| Position | CTE Operation | Result |
|----------|---------------|--------|
| Type annotation position | `Vec(factorial(5))` → CTE::eval(factorial(5)) → CTValue::Int(120) | Type substitution to Vec(120) |
| Generic value parameters | `Array(Int, factorial(3))` → CTE::eval(factorial(3)) → CTValue::Int(6) | Instantiate Array(Int, 6) |
| Assert type | `Assert(N > 0)` → CTE::eval(N > 0) → CTValue::Bool(true/false) | True → Void, False → compile_error("N must be > 0") |
| If conditional type | `If(C, T, E)` → CTE::eval(C) → CTValue::Bool(b) | True → T, False → E |
| Match type family | `AsString(Int)` → match Int { Int => String, ... } → String | Type-level pattern matching |
...
1. Partial evaluation of specification expressions
   ...
   CTE::eval(factorial(n)) → if n known at compile time → constant
                           → if n unknown → retain as symbol, hand to SMT

2. Specification condition simplification
   //! ensures: result >= 0 && result < n
   CTE tries to simplify known sub-expressions, reducing SMT solving burden

3. Specification type instantiation
   NonEmpty(n) = n > 0
   CTE expands specification types to boolean expressions
```
...
└─────────────┘                       └─────┬─────┘
                      ...
                      unsat                            sat
                          ↓                                ↓
                    ┌──────────┐                   ┌──────────────┐
                    │ Verification passed│                   │ Extract counterexample model│
...
| Mode | Behavior | CLI |
|------|------|-----|
| **Debug Build** | Parse specs, generate VCs, call Z3 to prove; verification must pass for Release Build | `yaoxiangc --debug` |
...
- [ ] Implement `eval()` basic paths: literals, variables, binary ops, conditionals, blocks
...
- [ ] Constant folding: `1 + 2 * 3` evaluated to `7` at compile time
...
**Deliverables**: `src/middle/cte/` module with `value.rs`, `eval.rs`, `purity.rs`
...
**Deliverables**: `src/middle/cte/interpreter.rs`, `src/middle/cte/termination.rs`
...
- [ ] Compile-time dimension validation: matrix multiplication dimension mismatch → compile error
- [ ] Integration with monomorphization (mono pass)

**Deliverables**: `src/middle/cte/type_level.rs`, updated `src/middle/passes/mono/`

### Phase 4: Hoare Logic Static Verification

**Goal**: Complete spec parsing, VC generation, SMT verification pipeline

**Contents**:
- [ ] Parser extension: recognize `//!` and `/*! ... !*/` as specification nodes
- [ ] Specification type definitions (`NonEmpty`, `Sorted`, `GreaterOrEqual`, etc. standard library specs)
...
**Deliverables**: `src/middle/verification/` module
...
```
src/middle/
├── cte/                          # Compile-Time Computation Engine
│   ├── mod.rs                    # CTE entry, orchestrates three subsystems
...
└── passes/
    └── mono/                     # Existing monomorphization (enhanced CTE integration)
        └── ...                   # Use CTE results for instantiation
```
...
|------|------|------|------|
| Purity determination method | Explicit annotation vs auto-inference vs hybrid | **Auto-inference** | Ownership system provides sufficient info; no explicit annotation escape hatch |
...
| Termination proof | Mandatory annotation vs auto-inference | **Mandatory in type positions, auto elsewhere** | Type position non-termination = undecidable = compile error; elsewhere can be lenient |
...
| Compilation mode | Unified mode vs Debug/Release split | **Debug verifies → Release zero overhead** | Verification is expensive; Release should not bear the cost |
...
| Purity misjudgment | Compile-time evaluation result differs from runtime | Ownership system provides strong guarantees; if misjudged, it's a compiler bug to be fixed |
...
| Significant compile time increase | Slower CI | Incremental verification + module-level cache + verification result files (like `.o` files) |
...
| RFC-009 §Ownership Model | Basis for purity analysis | Phase 1/2 reuse ownership information |
...
- [RFC-011: Generics System Design](../design/rfc/accepted/011-generic-type-system.md)
