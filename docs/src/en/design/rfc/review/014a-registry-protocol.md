...
Define the Registry protocol for the YaoXiang package management system: open interface design,
official Registry specification, GitHub adapter layer, package publish/withdraw flows, and
authentication model.

## Motivation

RFC-014's main document defines the overall architecture of the package management system, but the
Registry section is only marked as "reserved." Without a Registry protocol, packages cannot be
distributed—this is like designing a shopping cart without a store.

### Current Problems

- `RegistrySource` is stub code (`source/mod.rs:150-203`), where `resolve` directly returns the
  declared version and `download` returns an empty path
...
All implementations (`LocalSource`, `GitSource`, `RegistrySource`) are uniformly converted to async.
The CLI entry point is driven by `#[tokio::main]` or `Runtime::block_on`.
...
The official Registry is similar to crates.io and serves as the main channel for package
distribution.
...
1. **Prefer Release assets**: Check the GitHub Release page for matching pre-built artifacts for the
   platform
...
- **Security purpose**: Prevent npm-style supply chain attacks. Attackers have previously seized
  deleted package version numbers to inject malicious code; yank's version number lock completely
  closes off this attack vector.
...
URL. If no match is found, create a new entry (auto-generate name, e.g., `reg-1`).
