...
| --------------------- | ------------------------------------ | ----------------------- |
| Single-threaded loop  | Sequential execution in spawn blocks | Inherently race-free    |
...
| --------------------------- | ------------------------------------------------------------------------------------ |
| Rust std::io                | Synchronous blocking, cannot coordinate with spawn blocks for async                  |
| tokio                       | Designed for Rust async/await, misaligned with YaoXiang's explicit concurrency model |
| mio                         | Only provides raw async primitives, lacks high-level IO features                     |
...
| Positioning              | Resource type IO implementation layer | Integrates with RFC-024 resource type system         | 2026-06-16 |
