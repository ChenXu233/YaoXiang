...
**AST, parsing errors, types, symbols, references, and formatting results**. Reuse the
`World` backend already implemented in RFC-017, add the `yaoxiang mcp` subcommand,
single binary with dual mode, multi-process independent World.
...
- AI agent workflow is **code snippet-based**: "paste a piece of code" in conversation to ask questions, **without** saving to disk first

AI agents actually usable LSP clients (vscode-langservers-extracted, `mcp-lsp-bridge`
type projects) **only translate L1**: goto-def, hover. What AI wants to do:
...
2. YaoXiang project lacks "AI-First" interface layer: humans use LSP in IDE, AI agents cannot use LSP
3. Mainstream AI agents like Claude Code / Continue have default MCP support, which is a blank ecosystem for YaoXiang

### What is MCP?

MCP (Model Context Protocol) is an AI agent tool-calling protocol released and open-sourced
by Anthropic in 2024-2025, and has become a de facto standard (adopted by OpenAI, Google,
Microsoft, Zed, Continue, Cody, etc.). Features:
...
**This RFC uses only Tools primitive**——aligned with LSP's "provide service" concept,
without introducing the file model complexity of Resources.
...
- **Same binary**: `yaoxiang` switches via subcommand; LSP process and MCP process
  **do not coexist** in same runtime
- **Multi-process independent World**: Each `yaoxiang mcp` process holds one
  `World`; independent from LSP process and other MCP processes (no lock contention,
  independent crash isolation)
...
Designed with "eliminate edge cases + phased delivery" principle: pure source tools
stateless first, workspace tools share LSP World, AST rewrite tools added independently.
...
- `rename_symbol` —— **Pure source AST rewrite**, no LSP-style position query (semantics differ
...
**World Loading Timing**: At server startup, scan `yaoxiang.toml` and
`src/**/*.yx` by `--project-root`, reuse LSP-017's already implemented `World::load_*`
API to load `World.documents` at once. **No new** lib API added.

### Tool Contract

**Input**: Described with JSON Schema, each field has `description` + `examples`
...
- **Diagnostics**: Parse/type errors, follow RFC-013 (`E0001` etc.) —— **Not considered tool
  errors**
- **Tool-level errors**: Use `MCP-` prefix (`MCP-INVALID-INPUT`,
  `MCP-PROJECT-NOT-FOUND`, `MCP-INTERNAL`) —— Treated as `isError: true`
- **panic/crash**: JSON-RPC `-32603 Internal error`, server does not exit

**Path Resolution Rules** (applicable to `workspace_root` for `lookup_symbol` /
`find_references`, `file_paths` for `typecheck`):
...
4. `file_paths` must be within project root (prevent traversal); out of bounds → `MCP-PATH-OUTSIDE-PROJECT`
...
- **Listen on loopback only** (127.0.0.1 / ::1); public network binding explicitly
  refused with error and exit
...
requires users to manage port allocation.
**World Isolation**: Each process has independent LSP sync state——one MCP process crash
**does not affect** LSP/other MCP processes. **Future Sessions**: v2 considers multi-workspace
dispatch (multiple `Session` in same process), **this RFC does not**.
...
Tool schema auto-generated from input structs using `schemars` crate, avoiding manual
JSON Schema drift:
...
**`parse_source` / `format_source` tool schema has no `file_path` field**
——these two tools only accept string source, do not participate in project semantics.
`lookup_symbol` / `find_references` / `typecheck` accept `workspace_root` or `file_paths`
(required or not per tool table).
...
- ✅ **LSP server unchanged**: All capabilities, APIs, internal state implemented by RFC-017 unchanged
...
Each tool must have at least 1 happy path + 1 diagnostic scenario +
1 tool-error scenario integration test.

## Tradeoffs

### Pros

- **Extremely low reuse cost**: `World` / `Session` / `handlers`
  / diagnostic collection all already implemented (RFC-017), this RFC is "add an MCP shell"
- **AI-First interface**: Tool contract is 3-5x more intuitive than LSP; LLM directly reads schema
- **Multi-process isolation**: Decoupled from LSP editor session and other MCP processes,
  **zero lock contention**
- **stdio friendly**: All mainstream AI agents default subprocess mode, zero-config integration
- **YAGNI passed**: This RFC cuts Resources, Sessions, cross-process state, remote MCP——v2
  reopens these
...
- **HTTP mode is second-class citizen**: Loopback restriction positions as local tool,
  remote scenarios need v2 redesign
- **Repeated parse overhead**: AI repeatedly fine-tuning source and calling `parse_source`
  re-runs lexer+parser. **Mitigation**: RFC-017's `DocumentCache`
  still accelerates second parse of **disk** same source; pure transient source unavoidable
...
low-risk zero-adaptation "YaoXiang-specific" tools to verify integration correctness; v0.10.x
opens high-risk "AST rewrite" new module (independent PR review more focused).

### Risks

1. **`mcp-rust-sdk` maintenance activity**: Released in 2025, API may change drastically. **Mitigation**:
   If unstable, write lightweight JSON-RPC 2.0 + tool dispatcher ourselves (< 500 lines)
2. **Repeated parse overhead**: AI repeatedly fine-tuning source and calling `parse_source`
   re-runs lexer+parser. **Mitigation**: RFC-017's `DocumentCache`
   still accelerates second parse of **disk** same source; pure transient source unavoidable
3. **AI agent schema compatibility**: Different agents have different MCP schema strictness.
   **Mitigation**: Use `schemars` crate to auto-generate schema from Rust input structures,
   zero manual drift
...
5. **MCP tool schema not 1:1 with LSP params**: LSP `workspace_symbol` takes
   `(query)`; when passing to LSP internals, need to wrap into position+URI for existing handler
   reuse. **Mitigation**: Do adaptation layer in `mcp/tools/lookup.rs`, encapsulation details on MCP side
6. **`rename_symbol` AST rewrite semantics differ from LSP `rename`**: LSP `textDocument/rename`
...
- [ ] Does MCP need to output `tools/list` at startup for AI proactive discovery? (MCP standard requires, **implement by default**)
- [ ] Does `typecheck` support `mode: "fast|full"` (fast = current file subset only, full = full workspace)?
