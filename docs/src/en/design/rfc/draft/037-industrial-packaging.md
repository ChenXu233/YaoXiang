...
> This RFC complements [RFC-014b: Build System and Binary Distribution](../review/014b-build-system.md).
...
Replace the current hand-written CI build/packaging logic with `cargo-dist` (the standard binary distribution tool in the Rust ecosystem) to achieve cross-platform automated releases. This addresses issues such as missing `libz3.dll`, unpackaged standard library interface files, messy directory structure, and duplicated CI script maintenance.
...
The current Release only uploads `yaoxiang.exe`, but `libz3.dll` is not packaged in. Double-clicking to run on Windows will produce an error:
...
Standard library interface files (`.yx` files, needed by LSP) are not included in the release package. Users need to run `yaoxiang package init` to generate them. The industrial approach is for the release package to include the standard library.
...
Most of it is repetitive (install Rust → cache → build → rename → upload), and has to be written once for each platform. `cargo-dist` can generate equivalent pipelines with a single command.

#### Problem 4: Inno Setup version number is hardcoded

`MyAppVersion` is hardcoded to `0.7.0` in `setup.iss`, and the build relies on `sed` for substitution. This will eventually fail.

#### Problem 5: Ambiguous boundary with RFC-014b

RFC-014b defines the "YaoXiang package build and distribution mechanism" (i.e., the `[build]` and `[binaries]` configuration in `yaoxiang.toml`), but **does not cover "how the YaoXiang compiler itself is published"**. This RFC fills that gap.

## Proposal

### Core Design

Adopt **cargo-dist** as the release pipeline skeleton, combined with custom post-build scripts to handle package structure and additional files.
...
The default cargo-dist zip is flat (binary + auto-included README/LICENSE are all in the root directory).
This is not a problem — clear division of labor: cargo-dist handles compilation + CI + installers, and YaoXiang uses a 50-line `package-dist.sh` to handle zip structure.
...
- **This is an external library and should be distributed as a shared library**. Python (`python3.dll`+`DLLs/lib*.dll`), Node (`node`+`lib/`) all do this
- **Users don't need to wait for a new compiler version to upgrade Z3** — just replace the `.so`/`.dylib`/`.dll`
...
**"Static linking Z3 on all platforms" is no longer a goal.** This is not eliminating special cases; it's eliminating a reasonable case in the wrong way. Shared libraries are the normal distribution method for external libraries.
...
Subcommand name: **`yaoxiang package gen-std`** (in the same family as the existing `package init`/`add`/`install`)

The current `src/std/gen_interfaces.rs` already has a complete implementation (`generate_all_interfaces()`, `write_interfaces_to_dir()`); just add the subcommand entry in `main.rs` and call it in `package-dist.sh`:
...
cargo-dist handles "delivering the compiler to users"; wasm handles "embedding the online playground in the documentation website" — two completely different deliverables.
...
cargo-dist has no native nightly support ([#1143](https://github.com/axodotdev/cargo-dist/issues/1143), still an open feature request).
...
After running `cargo dist init`, the initial configuration is generated, with the core expected to be:
...
The current `src/std/gen_interfaces.rs` already implements the functionality for generating `.yx` interface files (`write_interfaces_to_dir`), and the `package init` command also calls it.
...
- **Out of the box** — Users can run it directly after downloading and extracting, no missing DLL issues
...
- **Custom packaging script still has maintenance cost** — The script for package structure and standard library interface files needs maintenance
...
1. Modify `build.rs`: unified dynamic linking on all platforms, extend `copy_dll()` to `copy_shared_lib()`
...
- ~~Conditional execution of cargo-dist extra-artifacts?~~ → **Handle with `package-dist.sh` script, use shell case branches**
- ~~Standard library interface version compatibility?~~ → **Published together with compiler version, in the same archive**
