...
This document proposes introducing a Cranelift JIT compiler into YaoXiang's VM backend, upgrading
the VM from a pure interpreter to a **multi-tier execution engine**: cold code is interpreted, hot
functions are compiled by Cranelift into native code. The JIT path shares the IR normalization pass
with RFC-018's LLVM AOT path — Cranelift handles fast JIT compilation, LLVM handles deep AOT
optimization — each playing to its strengths.
...
development, we frequently run tests, scripts, and local debugging — these scenarios don't need
AOT's extreme optimization, but do need execution noticeably faster than an interpreter.

### Why not just use LLVM AOT?

LLVM AOT compilation takes a long time (seconds), unsuitable for development iteration. Development
requires a "change and run" experience: modify one line of code → re-run → see results almost
instantly. Cranelift JIT compiles a single function in just 1-5ms, with no user-perceptible
compilation delay.
...
Cranelift compiles fast, and the code quality is sufficient. LLVM is reserved for AOT's offline deep
...
codegen swaps from LLVM to Cranelift.
...
- `backends/jit/` only depends on `middle/` (IR definitions, normalization passes), standard
  library, and the Cranelift crate
...
> Thresholds are configurable; the above are defaults. Refer to the actual threshold ranges used by
...
Every 5 seconds, all counters are right-shifted by 1 bit (multiplied by 0.5). This prevents
startup-time high-frequency but one-shot code (such as initialization traversals) from triggering
...
During compilation, the function continues to execute through the interpreter. After compilation
...
AOT need registers/SSA. The IR normalization pass performs the conversion once (RFC-018 §4.0); JIT
and AOT share it without changing the IR representation itself. Each backend consumes the same IR
...
may miss, after that all correct. Overhead is about 1 cycle.
...
No interpreter pause, no safepoint waiting, no call-site traversal. A single atomic operation
...
Each module is allocated contiguous mmap executable pages, and all JIT functions within a module are
allocated from the same page. When a module is invalidated, the entire page is reclaimed, with no
per-function release needed.

### 6. Hot-Reload Reserved Extension Points

The following interfaces compile but are not called until hot reload is implemented. Interface
design principle: **JIT implementation only needs `insert` and single-function `compare_exchange`;
module-level operations are reserved for hot reload.**
...
**Why group by module?** JIT itself only needs functions. Organizing by module is entirely to serve
hot reload: after a module is recompiled, the entire module's function set can be replaced
atomically, rather than CAS-ing one function at a time — the latter would cause inconsistent states
when there are cyclic dependencies between functions.

## Trade-offs

### Advantages

1. **Zero-perceptible compilation latency**: Cranelift 1-5ms/function, compiled in background
   thread, interpreter not paused
2. **Shared infrastructure**: JIT and AOT share the IR normalization pass (RFC-018 §4.0), no
   reinventing the wheel
3. **Non-destructive**: Pure incremental feature. VM unchanged, interpreter unchanged, just one
   faster hot path added
4. **No LLVM dependency**: VM does not introduce LLVM, keeping it lightweight
5. **Multi-platform by design**: Cranelift natively supports x86_64 and ARM64, covering all target
   platforms
...
2. **Debugging complexity**: Stack frames of JIT-generated code must be compatible with interpreter
   stack frames; debug info mapping requires additional handling
3. **Cold-start hotness latency**: First few seconds after program startup have no JIT acceleration;
   hotness accumulation is needed
