...
1. **User Experience**: Users can quickly determine the type and severity of errors by seeing the
   error code
...
> anchored by unit tests, but have no reachable yx source surface (explicit drop statements, Ptr
> dereference syntax, spawn ref loop construction paths) — claims of semantically complete do not
> cover these four codes; they are treated as "reserved" until implementation is completed.
...
> error paths are pending implementation, to be wired against real trigger surfaces when
> implemented.
...
> not match actual implementation needs. YaoXiang has no null pointer/heap allocation failure/type
> cast concepts (value semantics + Rust memory safety), and the runtime overflow path has no
> detection implementation. After calibration:
>
...
> W-code position rules: Isomorphic to E codes grouped by phase (W + phase thousands segment), W1xxx
> = type checking phase warnings.
>
> **Dead code semantics (#321 decision B)**: `pub` definitions are external interfaces, never
> reported — whether external consumers use them is beyond the single-file analysis boundary, prefer
> silence over false positives. W1001/W1002/W1004/W1005 only target **private (non-pub)
> definitions**: reported when never referenced in the reachability analysis from `main` and `pub`
> definitions. Methods (W1005) are matched by call-site short name. bin/lib target semantics (warn
> for unused pub in bin) is a future extension (#289 plan A), requiring project model support.
>
> **Unused import (W1003)**: Detected by typecheck's use elaboration (pass2 registers imported local
> names; expression resolution and type annotation location hits count as used), covering whole
...
> according to the W prefix (explicit specification takes precedence), collected and rendered on the
> same track as errors (rendered with `warning[W####]` prefix), but do not block compilation and do
> not affect successful exit code. `yaoxiang check --deny-warnings` escalates warnings to failures
> (exits with non-zero code when warnings exist), used for CI strict mode. Per-code suppression
> (allow attributes, etc.) is a future extension.
...
2. **Code validity**: Using unregistered codes and fake codes (e.g., `E_INTERNAL`) is prohibited;
   call-site code literals must already be defined in the registry. Internal errors uniformly land
...
4. **Solver internal state isolation**: Solver intermediate state TypeVars (Display form `t<N>`)
   must not enter user-visible messages. Test anchor:
...
> E6xxx/E7xxx semantic space simultaneously hosts two channels, with the same code space but
...
- **Consumption surface**: In-program `e.code == "E6xxx"` comparison is the only programmable
  judgment contract; `yaoxiang explain E6xxx` documentation connectivity; toolchain (LSP / DAP, see
  RFC-034) uses codes as exceptionId.
...
1. Runtime error value codes and compiler diagnostic codes share the E6xxx/E7xxx space, new codes
   are assigned by **real trigger surface**, not reserved for imagined scenarios.
2. Register before use: New codes enter the authoritative registry and pass three-party consistency
   validation (codes/*.rs ↔ locales ↔ this document's code table) before they can be emitted. The
...
`{ kind: ErrorKind, message: String }`, with `code` converted to an attribute derived from kind
(variant definition site is the code registry). The code stability contract in this section remains
unchanged during evolution; this upgrade is an independent decision and does not constitute a
...
> **Note**: Not all error codes use placeholders. Some error codes (like E0001) have static messages
...
│  2. Load i18n JSON for the corresponding language from the compiler binary                                │
...
│  // Directly output the final string, no lookups                                        │
└─────────────────────────────────────────────────────────────────────────┘
```
