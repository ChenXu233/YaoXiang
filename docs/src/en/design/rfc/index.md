...
- [Documentation Revision Rules](#documentation-revision-rules)
...
| ID      | Title                                                                                                        | Author | Created     | Status |
| ------- | ------------------------------------------------------------------------------------------------------------ | ------ | ----------- | ------ |
| RFC-032 | [RFC-032: spawn Unified Expression Modifier — Eliminating spawn for Special Cases](./review/032-spawn-unified-expression.md) | 晨煦 | 2026-06-16 | Under Review |
...
| RFC-008   | [RFC-008: Runtime Concurrency Model and Scheduler Decoupling](./accepted/008-runtime-concurrency-model.md)                                        | 晨煦        | 2025-01-05  | Accepted        |
| RFC-009   | [RFC-009: Ownership Model Design](./accepted/009-ownership-model.md)                                                     | 晨煦        | 2025-01-08  | Accepted        |
| ↳ RFC-009a| [RFC-009a: Token Lifetime Analysis — Based on Hoare Proof Pipeline](./accepted/009a-borrow-proof-pipeline.md)                                     | 晨煦        | 2026-06-13  | Accepted        |
...
| ↳ RFC-014a| [RFC-014a: Registry Protocol Specification](./review/014a-registry-protocol.md)                                                  | 晨煦        | 2026-06-11  | RFC Under Review|
| ↳ RFC-014b| [RFC-014b: Build System and Binary Distribution](./review/014b-build-system.md)                                                    | 晨煦        | 2026-06-11  | RFC Under Review|
| ↳ RFC-014c| [RFC-014c: Workspace Support](./review/014c-workspace.md)                                                             | 晨煦        | 2026-06-11  | RFC Under Review|
...
| ↳ RFC-026a| [RFC-026a: Extensible FFI Mechanism System](./review/026a-extensible-ffi-system.md)                                            | 晨煦        | 2026-06-05  | RFC Under Review|
| ↳ RFC-026b| [RFC-026b: yx-bindgen Toolchain](./draft/026b-yx-bindgen.md)                                                              | 晨煦        | 2026-06-05  | Draft RFC       |
...
| --------------- | ------------------ | ----------------------------------------------------- |
| **Draft**       | `rfc/draft/`       | Author draft, awaiting submission for review          |
...
**RFC documents must contain only correct information.** When designs change, directly revise the original text to express the current correct semantics;
**do not preserve incorrect content and add "errata" blocks to correct it**.

Keeping "original text + errata" is the worst approach: readers discover halfway through that everything before was voided, wasting their prior reading effort, and they may mistakenly cite invalidated sections as current semantics. Errata blocks may appear cautious, but they actually transfer the整理成本 (organization cost) to readers.

### Correct Approaches

| Situation                                   | Approach                                                                                           |
| -------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Implementation differs from text, text refuted | Directly rewrite that paragraph with correct content, delete original statement                    |
| Example code no longer runnable             | Directly change to a runnable form, do not preserve old examples                                  |
| Need to inform readers "what it used to be" | Only keep incorrect content when doing deliberate error comparison, with immediate "this写法 is incorrect" label and explanation |
| Need to trace design evolution              | Write in Git commit messages or issues, not in RFC body                                           |

### Exceptions

The following incorrect information may be preserved:

- **Deliberate contrast teaching**: Clearly labeled as correct/incorrect comparison, with the incorrect side immediately followed by explanation of "why it's wrong"
- **Deprecated RFCs** (`rfc/deprecated/`): As historical records, but should note what they were superseded by

### Auxiliary Methods

- The `status` / `updated` fields at the top of RFCs reflect the latest revision time, no need to write "what was revised this time" in the body
- Implementation status is expressed with ✅ / ❌ in tables, avoid interspersing status narratives in body text
- Complete revision history can be viewed with `git log -- <file>`
