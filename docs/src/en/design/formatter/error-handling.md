...
**§E1.2 Configuration Errors.** When the configuration file is incorrectly formatted, a clear error
...
occur during parsing, the formatter directly returns the error message and does not perform
formatting.

**§E3.2 No Placeholders.** The formatter does not insert any placeholders (such as `/* error */`) at
the error location. When encountering syntax errors, the formatter directly reports the error and
terminates.
