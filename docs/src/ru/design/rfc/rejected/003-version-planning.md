...
| Проблема | Описание |
|----------|----------|
| **Неправильный тип документа** | RFC — это документ решений о том, «почему сделано так», а данная статья — документ управления проектом «где мы сейчас, куда идём» |
| **Отсутствие дизайн-решений** | Нет анализа компромиссов при выборе технологий, нет сравнения альтернатив, нет обоснований дизайна |
| **Чрезмерное планирование** | Запланировано 9 версий (v0.1-v1.0), но выполнено только 1.5, после v0.3 всё — предположения |
| **Высокая стоимость поддержки** | Таблица критериев успеха имеет 50 состояний, трудно поддерживать |
| **Путаница с номерами версий** | Пропущен v0.8, без объяснений |

### Правильный тип документа

Данный документ следует вести как **дорожную карту проекта (Roadmap)**, а не RFC. Дорожная карта фиксирует факты и кракосрочные планы, RFC фиксирует дизайн-решения.

---

> **Ниже приведено исходное содержание, сохранено для справки.**
...
**Основные цели**:
1. **Компиляция в байт-код**: поддержка REPL и постепенной компиляции
