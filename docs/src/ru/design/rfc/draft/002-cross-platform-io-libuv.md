...
Данный документ определяет уровень реализации IO для YaoXiang: кроссплатформенные возможности IO на
основе libuv как базовую реализацию системы ресурсных типов из RFC-024.
...
- ❌ Не «разработчику не нужно заботиться о деталях» — система ресурсных типов обеспечивает
...
2. **Ограничения самобутстрапа**: после самобутстрапа может потребоваться замена на нативную
   реализацию YaoXiang
