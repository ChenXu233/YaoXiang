...
yx-bindgen механически преобразует заголовочные файлы C в файлы FFI-привязок `.yx`,
...
Ручное написание FFI-привязок утомительно и подвержено ошибкам — библиотеки C содержат десятки или сотни функций. Ещё опаснее **раскладка прозрачных типов**: при ручном написании
`Timespec: Type = { tv_sec: Int64, tv_nsec: Int64 }`
размеры полей, выравнивание и дополнение должны точно соответствовать структуре C
`struct timespec` целевой платформы, иначе C будет читать/записывать по неправильной раскладке, что приведёт к выходу за границы (гарантия доверия из RFC-026 §2.2). yx-bindgen механически вычисляет раскладку из `.h`-файлов, устраняя этот риск ручной гарантии.
