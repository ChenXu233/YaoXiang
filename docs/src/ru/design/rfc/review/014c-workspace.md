...
> Настоящий RFC является подчиненным RFC [RFC-014: Дизайн системы управления пакетами](../accepted/014-package-manager.md).

## Краткое описание

Определяет механизм рабочих пространств (workspace) для YaoXiang: совместное использование зависимостей, ссылки на пути, унификация lockfile и интеграция с Cargo workspace при разработке нескольких связанных пакетов.
...
**Корневой toml не определяет dependencies.** Зависимости каждого участника записываются в его собственном `yaoxiang.toml`.
...
`{ workspace = "member-name" }` ссылается на **key** из `[workspace.members]` (не на `[package].name` участника).
...
- При публикации ссылки workspace заменяются на версионные зависимости, key не раскрывается в публичном API
...
**Источник версии:** читается `[package].version` зависимого участника, добавляется префикс `^`. Registry не проверяется——авторитетным источником версии является `yaoxiang.toml` участника, Registry — это лишь канал распространения.
...
**Поведение `yaoxiang build`:** собирает всех участников в порядке топологической сортировки зависимостей. Если core → utils → app, порядок сборки будет core → utils → app.
...
**Логика определения:** при загрузке toml, если есть секция `[workspace]`, парсится как `WorkspaceManifest`, иначе как `PackageManifest`.
...
- Все участники должны использовать одинаковые версии внешних зависимостей (может быть слишком строго)
