...
Разработка унифицированной системы конфигурации для языка YaoXiang с двумя уровнями:
пользовательским и проектным, обеспечивающей общую инфраструктуру конфигурации для менеджера
пакетов, компилятора, REPL, LSP и других компонентов.
...
**Правило**: верхний уровень переопределяет нижний, неуказанные параметры наследуются от нижнего
уровня.
...
**Первый запуск**: При первом запуске любой команды `yaoxiang` автоматически проверяется наличие
пользовательской конфигурации. Если она отсутствует, конфигурация генерируется автоматически с
...
- ✅ Существующий режим без файла конфигурации сохраняется (все компоненты используют встроенные
...
- ✅ При ошибке парсинга конфигурации отображается понятное сообщение с указанием конкретной строки
  и причины ошибки
...
- [x] Синтаксис `features` для условной компиляции? → **Вынесен в отдельный RFC**, зависит от
      RFC-011 системы generics
- [x] Дизайн `workspace` рабочих пространств? → **Вынесен в отдельный RFC**, высокая сложность,
...
> **Внимание**: следующий синтаксис используется в файле конфигурации `yaoxiang.toml`, **не** в
> исходном коде YaoXiang (файлы `.yx`). Пользователям не нужно писать `cfg(...)` в коде.
...
- Сторонние инструменты могут интегрироваться через `yaoxiang tool run <название>` или напрямую
  обращаться к конфигурации
