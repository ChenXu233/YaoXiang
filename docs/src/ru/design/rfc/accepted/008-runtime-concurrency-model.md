...
> **⚠️ Указание о согласовании**: Данный документ согласован с [RFC-024 Новая модель конкурентности](/reference/language-spec/concurrency.md).
> Старый анализ DAG всей программы, аннотации `@block`/`@eager`, модель уровней L1/L2/L3 заменены
> примитивом параллельных блоков `spawn {}`. Анализ DAG теперь применяется только внутри блоков `spawn {}`.
...
с анализом DAG и автоматической конкурентностью внутри блоков spawn. При num_workers=1 — однопоточный асинхронный режим.
...
> **Важное изменение**: Анализ DAG больше не применяется ко всей программе, а только внутри блоков `spawn {}`.
> Обычный код (вне блоков spawn) выполняется последовательно, анализ DAG не требуется.
...
> **Важное изменение**: Анализ DAG снизу вверх выполняется только внутри блоков `spawn {}`,
