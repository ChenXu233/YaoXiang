...
Проектирование системы управления пакетами для языка YaoXiang, поддерживающей семантическое управление версиями, локальные зависимости и зависимости из GitHub, унифицированный синтаксис импорта, конфигурационный файл `yaoxiang.toml` и файл блокировки `yaoxiang.lock`.

## Мотивация

### Зачем нужна эта возможность/изменение?

Управление пакетами является основой экосистемы современных языков программирования. В настоящее время языку YaoXiang не хватает:
...
| `yaoxiang clean` | Очистка результатов сборки | `yaoxiang clean` |
...
- Детерминированная сборка: lock-файл обеспечивает согласованность сборки
...
| Только поддержка реестра | GitHub является основной платформой хостинга кода на данный момент |
...
| **Phase 2** | Поддержка GitHub, управление .yaoxiang/vendor, инструменты загрузки |
...
| Нестабильная загрузка с Git | Механизмы повтора и кэширования |
| Проблемы производительности | Ленивая загрузка, инкрементное разрешение |
