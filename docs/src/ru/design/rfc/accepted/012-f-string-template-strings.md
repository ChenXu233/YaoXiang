...
Добавление в язык YaoXiang特性 строк шаблонов f-string с поддержкой интерполяции переменных,
вычисления выражений и форматированного вывода. F-string использует синтаксис в стиле Python
(`f"..."` префикс), где выражения встраиваются в строку с помощью синтаксиса `{expression}`, а во
время компиляции преобразуются в эффективные строковые операции.
