...
# RFC 012: Строки шаблонов F-String

## Краткое описание

Добавление в язык YaoXiang特性 поддержки строк шаблонов f-string с переменной интерполяцией, вычислением выражений и форматированным выводом. F-string использует синтаксис в стиле Python (`f"..."` префикс), где выражения嵌入在字符串中通过 синтаксис `{expression}`, компилируемый в高效的字符串操作 во время компиляции.
...
Компилятор 在词法分析阶段识别 `f` 前缀字符串字面量，解析花括号内的表达式和可选的格式化说明符。
...
### Приложение A: Справочник по спецификаторам форматирования
...
### Приложение B: Примеры вариантов использования
...
RFC имеет следующие状态 переходы：
