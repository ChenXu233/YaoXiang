```yaml
---
title: RFC-004：多опозиционное объединённое связывание для каррированных методов
---

# RFC-004: 多опозиционное объединённое связывание для каррированных методов

> **Статус**: Принято
> **Автор**: Чэнь Сюй (晨煦)
> **Дата создания**: 2025-01-05
> **Последнее обновление**: 2026-02-18 (добавлены встроенные связывания, синтаксис постфиксного связывания)

## Аннотация

В данном RFC предлагается совершенно новый синтаксис **многопозиционного объединённого связывания**, позволяющий точно привязать функцию к произвольной позиции параметра типа, с поддержкой однопозиционного и многопозиционного объединённого связывания,从根本上解决 каррированном связывании проблему «кто является вызывающей стороной», без введения ключевого слова `self`.

## Мотивация

### Зачем нужен этот функционал?

При текущем дизайне языка при связывании независимой функции как метода типа возникают следующие проблемы:

1. **Положение вызывающей стороны негибкое**: традиционное связывание может зафиксировать только `obj` в `obj.method(args)` как первый параметр
2. **Сложности с многопараметрическим связыванием**: когда методу необходимо принимать несколько параметров одного типа, невозможно элегантно выразить это
3. **Неоднозначность каррированной семантики**: при частичном применении сложно различить, «к какой позиции привязано»
...
```yaoxiang
# Функциональный взгляд: явно передаются все параметры
distance(p1, p2)

# ООП взгляд: неявное this
p1.distance(p2)

# [positions] 让两种写法等价，本质都是函数调用
...
**说明**:
- **默认绑定**: 自动查找第一个 тип совпадающей позиции
- `[0]`: this 绑定到第 0 位 (первый параметр)
- `[1]`: this 绑定到第 1 位 (второй параметр)
- `[-1]`: this 绑定到最后一位 (отсчёт с конца)

## Компромиссы

### Преимущества

- **智能默认绑定**: 默认绑定第一个 тип совпадающей позиции,无需显式指定 `[positions]`
- **精确控制**: 可以绑定到任意参数位置,灵活度高
- **类型安全**: 编译时完全类型检查,只有 тип совпадающий才绑定
- **语法简洁**: `[position]` 语法直观易懂
- **无 `self` 关键字**: 保持语言简洁性
- **Каррирование дружелюбно**: 天然支持 частичное применение и цепочечные вызовы
- **ООП дружелюбно**: 自动 каррирование 让 ООП程序员无脑迁移
...
| **默认绑定逻辑** | **绑定第一个 тип совпадающей позиции** | **更智能、更安全,符合直觉** |
...
| 绑定位置 | 函数参数列表中的索引位置 |
| 联合绑定 | 将类型绑定到多个参数位置 |
| Частичное применение | 只提供部分参数,返回待完成调用的函数 |
| **统一语法** | **`name: (params) -> Return = body`,参数名在签名中声明** |
| **类型匹配绑定** | **默认绑定逻辑:自动查找第一个与调用者 тип совпадающей позиции** |
| **工厂函数绑定** | **当函数参数中没有匹配类型时,作为 конструктор使用** |
