...
нескольких языков, а также команду `yaoxiang explain` для предоставления пояснений к ошибкам.
...
Уровень серьёзности ошибки управляется через перечисление `DiagnosticLevel` и не привязан к
нумерации кода ошибки:
