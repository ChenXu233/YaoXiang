...
- **Простота**: Одноуровневая нумерация, пользователю не нужно запоминать сложные правила
  классификации
- **Удобство**: Формат сообщений об ошибках, аналогичный Rust, с вспомогательной информацией и
  примерами
...
| **Разделение шаблонов**                           | Шаблон сообщения отделён от кода, упрощая i18n                |
...
| E1094 | Неиспользованный параметр значения времени компиляции |
...
| E1102 | Управляющий оператор цикла находится вне цикла        |
...
| E2029 | Циклическая ссылка внутри spawn                  |
...
> Описание зарезервированных кодов (инвентаризация от 2026-09-14, релиз по #251): E2019 (двойное
> освобождение), E2020 (использование после освобождения), E2027 (разыменование unsafe), E2029
> (циклическая ссылка ref в spawn) зарегистрированы и имеют привязку к модульным тестам, но не имеют
> доступной поверхности в исходном коде yx (явный оператор drop, грамматика разыменования Ptr, путь
> построения цикла ref в spawn) — утверждение о полной семантической корректности не покрывает эти
...
> путь оптимизации ошибок ожидают реализации, при реализации будут подключены в соответствии с
> реальными триггерами.
...
| E6003 | Выход за границы массива               |
...
> указателя/сбоя выделения в куче/преобразования типов (семантика значений + безопасность памяти
> Rust), и путь обнаружения переполнения во время выполнения не реализован. После корректировки:
>
...
| W1063 | Не удалось вычислить ограничение const-дженерика |
| W1080 | Деградация доказательства времени компиляции     |

<!-- code-table:W1xxx end -->

> Правило позиционирования W-кодов: Изоморфно кодам E с группировкой по этапам (W + этап в тысячной
> позиции), W1xxx = предупреждения этапа проверки типов.
>
> **Семантика кода мёртвого кода (решение B по #321)**: Определения `pub` являются внешним
> интерфейсом и никогда не сообщаются — выходит ли за рамки анализа одного файла вопрос, используют
> ли их внешние потребители; лучше молчать, чем ложно срабатывать. W1001/W1002/W1004/W1005
> применяются только к **приватным (не `pub`) определениям**: если из `main` и `pub` определений
> анализ достижимости не находит ссылок, выдаётся предупреждение. Метод (W1005) сопоставляется по
> короткому имени в точке вызова. Семантика bin/lib target (предупреждение о неиспользуемом pub в
> bin) — это будущее расширение (схема A по #289), требует поддержки модели проекта.
>
> **Неиспользуемый импорт (W1003)**: Обнаруживается путём use-элаборации при проверке типов (pass2
...
>
> **Канал выброса**: Диагностика W-кодов по умолчанию помечается builder'ом префиксом W как
> `Severity::Warning` (явное указание имеет приоритет), сбор и представление идут по тому же пути,
> что и ошибки (префикс рендеринга `warning[W####]`), но не блокируют компиляцию и не влияют на код
> успешного завершения. `yaoxiang check --deny-warnings` повышает предупреждения до сбоя (ненулевой
> код выхода при наличии предупреждений), используется для строгого режима CI. Подавление per-code
> (атрибут allow и т.д.) — будущее расширение.

### Спецификация качества сообщений

> Этот раздел введён ревизией единства и качества сообщений (2026-09-03). Принудительно выполняется
> в CI скриптом `scripts/audit_diagnostics.py`.

1. **Единство сообщений**: Все видимые пользователю диагностические сообщения должны проходить через
   методы быстрого доступа авторитетного реестра + рендеринг шаблонов locales, код передаёт только
   структурированные параметры. Запрещено обходить реестр и напрямую создавать собственные значения,
   такие как `Diagnostic::error(...)` — этот путь обходит валидацию кода и i18n.
2. **Корректность кодов**: Запрещено использовать незарегистрированные и псевдокоды (например,
...
3. **Отображение типов**: Отображение типа должно различать форму до и после инстанцирования (голое
   имя вида `Expected 'Container', found 'Container'` неразличимо).
...
5. **Граница E8xxx**: E8xxx используется только для проблем внутренней согласованности компилятора
   ...
   вариант; сообщения ICE должны содержать указания на минимальное воспроизведение.

---

### Связь значений ошибок времени выполнения с кодами

> Этот раздел введён ревизией значений Error времени выполнения с кодами (2026-09-03). Семантическое
> пространство E6xxx/E7xxx одновременно несёт два канала, кодовое пространство едино, каналы
...
| Канал значений ошибок внутри программы | Носитель Err `Result(T, Error)` из std                 | Значение языка, потребляется программой через match/сравнение |
...
- `code` повторно использует нумерацию E6xxx/E7xxx из данной спецификации, строковая форма
  (например, `"E6008"`).
...
- **Пользовательские ошибки**: E в `Result(T, E)` — параметр дженерика, для серьёзного моделирования
  используются пользовательские типы; std `Error` — лишь удобный резервный носитель, его система
...
1. Коды значений ошибок времени выполнения и коды диагностики компилятора совместно используют
   пространство E6xxx/E7xxx, новые коды распределяются по **реальным триггерам**, без резервирования
...
2. Сначала регистрация, затем использование: новый код попадает в авторитетный реестр и проходит
   ...
   `RUNTIME_ERROR_CODES` в `src/std/result.rs` (так же подвержена порогу времени сборки из
   `build.rs` + валидации `tools/code-tables`).
...
/// Реестр отображаемого текста i18n (загружается во время компиляции из JSON, нулевой поиск во время выполнения)
...
1. Сохранение маппинга старых кодов ошибок на новые
...
1. Обновление всех точек выброса ошибок для использования новой системы
