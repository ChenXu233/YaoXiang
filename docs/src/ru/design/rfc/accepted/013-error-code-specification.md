...
- **Простота**: одноуровневая нумерация, пользователям не нужно запоминать сложные правила
  классификации
- **Дружелюбность**: формат сообщений об ошибках, аналогичный Rust, с справочной информацией и
  примерами
...
> [RFC-011b: Перегрузка операторов](./011b-operator-overloading.md) При реализации это затронет три
> места в данном разделе — ① тексты `E1081` / `E1082` лишатся слова "Result" (`?` будет определяться
> через интерфейс `Try` вместо привязки к конкретному имени типа) и будут синхронизированы на этапе
> 2 по процессу «трёхсторонней согласованности» этого документа (codes/*.rs ↔ locales ↔ таблица
> кодов); ② диагностика отказа из-за несоблюдения предварительного ограничения `Equal` (линейный
> токен) будет повторно использовать семейство `E1101` (тип не реализует интерфейс); ③ после
> подключения на этапе 1 `Struct == Struct` перейдёт из ошибки времени выполнения `E6007` в
> compile-time проверку, поверхность срабатывания `E6007` сократится. Зарегистрированные тексты в
> таблице остаются без изменений до реализации.
...
> Описание зарезервированных кодов (инвентаризация 2026-09-14, формат релиза #251): E2019 (двойное
> освобождение), E2020 (использование после освобождения), E2027 (разыменование unsafe), E2029 (цикл
> ref в spawn) — все зарегистрированы и имеют привязку к модульным тестам, но пока не имеют
> достижимого исходного слоя yx (явные операторы drop, синтаксис разыменования Ptr, пути построения
> ref-цикла в spawn) — утверждение о полной семантической корректности не распространяется на эти
> четыре кода, до завершения реализации они рассматриваются как «зарезервированные».
...
> E4006/E8004 в настоящее время не имеют точек генерации (зарезервированные коды): ограничение Sized
> и пути ошибок оптимизации ожидают реализации; при реализации следует подключать их к реальным
> точкам срабатывания.
...
| E6005 | Сбой утверждения (assertion)           |
...
> семантики Rust (Assertion failed/Arithmetic overflow/Heap allocation failed/Type cast failed) и не
> соответствовала реальным потребностям реализации. YaoXiang не имеет концепций нулевого
> указателя/сбоя кучи/приведения типов (семантика значений + безопасность памяти Rust), пути
> переполнения времени выполнения не реализуют обнаружение. После калибровки:
>
...
> - E6006 изменён с Runtime index out of bounds на Function not found (так в реализации уже давно)
...
> Правило позиций W-кодов: изоморфно E-кодам, группируются по этапам (W + сегмент тысяч этапа),
> W1xxx = предупреждения этапа проверки типов.
>
> **Семантика кода мёртвого кода (решение B по #321)**: `pub` определения — это внешний интерфейс,
> никогда не сообщается — выходит за рамки анализа одного файла, лучше молчать, чем ложно
> срабатывать. W1001/W1002/W1004/W1005 применяются только к **приватным (не pub) определениям**:
> если при анализе достижимости, начиная с `main` и `pub` определений, ссылка никогда не была
> использована — сообщается. Метод (W1005) сопоставляется по короткому имени в точке вызова.
> Семантика bin/lib target (предупреждение о неиспользуемом pub в bin) — будущее расширение (вариант
> A по #289), требует поддержки модели проекта.
>
> **Неиспользуемый импорт (W1003)**: обнаруживается при use elaboration в typecheck (pass2
> регистрирует локальные имена импорта; если имя попадает в позицию разрешения выражений или
> аннотации типа, оно считается использованным), покрывает как общий импорт (`use std.io` →
> псевдоним модуля), так и именованный импорт (`use std.io.{print}`).
>
> **Канал генерации**: W-коды диагностики по умолчанию помечаются builder-ом префиксом W как
...
> наличии предупреждений выходит с ненулевым кодом), используется для строгого режима CI. Подавление
> per-code (атрибут allow и т.д.) — для будущего расширения.
...
1. **Единый путь сообщений**: все видимые пользователю диагностические сообщения должны проходить
   ...
   структурированные параметры. Запрещено обходить реестр и напрямую создавать сырые значения, такие
   как `Diagnostic::error(...)` — этот путь обходит проверку кода и i18n.
2. **Легитимность кода**: запрещено использовать незарегистрированные коды и псевдокоды (такие как
   `E_INTERNAL`); использование литералов кодов в точке вызова должно быть зарегистрировано в
   реестре. Внутренние ошибки всегда попадают в E8001 (`internal_error`).
...
5. **Граница E8xxx**: E8xxx используется только для проблем внутренней согласованности компилятора
   (ICE). Исправляемые пользователем ошибки не должны использовать E8001 как запасной вариант;
   сообщение ICE должно содержать указания по минимальному воспроизведению.

---

### Связь между значениями ошибок времени выполнения и кодами

> Этот раздел введён ревизией «значения ошибок времени выполнения с кодами» (2026-09-03).
> Семантическое пространство E6xxx/E7xxx одновременно обслуживает два канала, пространство кодов
> одно, каналы представления разные.
...
| Канал значений ошибок в программе | Носитель Err `Error` из `Result(T, Error)` библиотеки std | Значение языка, потребляется программой через match/сравнение |
...
- `code` повторно использует нумерацию E6xxx/E7xxx данной спецификации в строковой форме (например,
  `"E6008"`).
- **Стабильный контракт**: назначенные коды сохраняют семантику между версиями; одна и та же
...
- **Поверхность потребления**: сравнение в программе `e.code == "E6xxx"` — единственный
  программируемый контракт; `yaoxiang explain E6xxx` обеспечивает документацию; toolchain (LSP /
  DAP, см. RFC-034) использует код как exceptionId.
- **Аксессоры**: `std.result.code(e)` / `std.result.message(e)`.
- **Пользовательские ошибки**: E в `Result(T, E)` — параметр типа, серьёзное моделирование
  выполняется через пользовательские типы; `Error` из std — лишь удобный запасной носитель, его
  система кодов не ограничивает пользовательские типы E.
...
1. Коды значений ошибок времени выполнения и диагностические коды компилятора совместно используют
   пространство E6xxx/E7xxx, новые коды назначаются по **реальной поверхности срабатывания**, без
   резервирования для воображаемых сценариев.
2. Сначала регистрация, затем использование: новый код входит в авторитетный реестр и проходит
   проверку трёхсторонней согласованности (codes/*.rs ↔ locales ↔ таблица кодов данного документа),
   после чего может генерироваться. Источник регистрации кодов значений ошибок времени выполнения —
   таблица `RUNTIME_ERROR_CODES` в `src/std/result.rs` (подчиняется тому же порогу сборки
   `build.rs` + проверке `tools/code-tables`, что и диагностические коды).
3. E7xxx зарезервирован для значений ошибок std.io / std.net (сейчас пуст, будет активирован при
   переходе io/net на Result).
4. Точки генерации: модули std создают значения Error через `error_new(code, message)`; на стороне
   потребления `std.result.unwrap_err` извлекает носитель Err, `std.result.code/message` читает
   поля.
...
`{ kind: ErrorKind, message: String }`, `code` становится свойством, выводимым из kind (определение
варианта = точка регистрации кода). В период эволюции стабильный контракт code данного раздела
остаётся неизменным; это обновление — независимое решение, не является обязательством данного
раздела.
...
│                                                                           │
│  AOT-бинарник напрямую хранит финальные строки, без шаблонов, без таблиц  │
└─────────────────────────────────────────────────────────────────────────┘
...
Серьёзность ошибки управляется перечислением `DiagnosticLevel`, декаплированным от номера кода
ошибки:
...
1. Поддерживать сопоставление старых кодов ошибок с новыми
...
1. LSP-сервер интегрирует вывод explain JSON
