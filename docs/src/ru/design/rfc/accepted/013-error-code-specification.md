...
нескольких языков, а также предоставляет функцию объяснения ошибок через команду `yaoxiang explain`.
...
- `locales/*.json`: Отображаемый текст для каждого языка (title, message, help; коды ошибок —
  вложенные объекты)
- `DiagnosticBuilder`: Универсальный построитель, заменяющий дизайн с отдельным трейтом на каждую
  ошибку
...
| **Безопасность типов**                            | Быстрые методы гарантируют корректность параметров                           |
...
| **Нулевые накладные расходы во время выполнения** | Рендеринг во время компиляции, отсутствие поиска по таблицам в AOT-бинарнике |
...
| E1062 | Не выполнено ограничение const-дженерика            |
...
| E1092 | Недопустимая форма уточняющего аргумента типа       |
...
| E2030 | Нарушение ограничения уточняющего типа           |
...
> (ref-цикл в spawn) — зарегистрированы и привязаны к модульным тестам, но пока не имеют достижимых
> поверхностных точек в исходном коде yx (явные операторы drop, грамматика разыменования Ptr, пути
> создания ref-цикла в spawn) — утверждения о полной семантической корректности не покрывают эти
...
| E4012 | Слишком глубокая рекурсия в константе |
| E4014 | Сбой вычисления константы             |
| E4018 | Нарушение уточняющего предиката       |
| E4019 | Равенство типов не выполняется        |
| E4020 | Требуется доказывающая функция        |

<!-- code-table:E4xxx end -->

> E4006/E8004 в настоящее время не имеют точек выброса (зарезервированные коды): пути ограничения
> Sized и ошибок оптимизации ожидают реализации; при реализации следует подключать их к реальным
> триггерам.
...
> **Редакция кодовой таблицы (2026-08-09)**: Изначально таблица кодов была определена в соответствии
...
> нулевого указателя/сбоя выделения кучи/приведения типов (семантика значений + безопасность памяти
> Rust), а пути обнаружения переполнения во время выполнения не реализованы. После корректировки:
>
...
| W1080 | Деградация доказательства во время компиляции    |

<!-- code-table:W1xxx end -->

> Правило позиционирования W-кодов: изоморфно E-кодам, группируются по этапам (W + сегмент тысяч
> этапа); W1xxx = предупреждения этапа проверки типов.
>
> **Семантика мёртвого кода (решение B по #321)**: Определения `pub` — это внешний интерфейс, и для
> них отчёт никогда не формируется — вопрос о том, использует ли их внешний потребитель, выходит за
> рамки однofайлового анализа, и лучше молчать, чем ошибочно сообщать. W1001/W1002/W1004/W1005
> применяются только к **приватным (не-`pub`) определениям**: если в анализе достижимости ссылок,
> исходящем из `main` и определений `pub`, определение никогда не упоминается — формируется отчёт.
> Методы (W1005) сопоставляются по короткому имени в точке вызова. Семантика bin/lib target
> (предупреждение о неиспользуемом `pub` в bin) — будущее расширение (вариант A по #289), требующее
> поддержки модели проекта.
>
> **Неиспользуемый импорт (W1003)**: Обнаруживается при use-elaboration в typecheck (pass2
> регистрирует локальные имена импорта; попадание в разбор выражений или в позицию аннотации типа
> считается использованием); охватывает как общий импорт (`use std.io` → псевдоним модуля), так и
> именованный импорт (`use std.io.{print}`).
>
> **Канал выброса**: Диагностика W-кода формируется построителем с привязкой префикса W к
> `Severity::Warning` по умолчанию (явное указание имеет приоритет); сбор и представление происходят
> по тому же каналу, что и ошибки (префикс рендеринга `warning[W####]`), но не прерывают компиляцию
> и не влияют на код успешного завершения. Команда `yaoxiang check --deny-warnings` повышает
> предупреждения до ошибок (при наличии предупреждений завершается с ненулевым кодом) — для строгого
> режима CI. Подавление per-code (атрибут `allow` и т. п.) — последующее расширение.

### Спецификации качества сообщений

> Этот раздел введён ревизией качества и унификации сообщений (2026-09-03). Принудительно
...
1. **Единый канал сообщений**: Все видимые пользователю диагностические сообщения должны проходить
   через быстрые методы авторитетного реестра и рендеринг шаблонов `locales`; код передаёт только
...
2. **Легитимность кода**: Запрещено использовать незарегистрированные коды и псевдокоды (например,
   `E_INTERNAL`); используемый литерал кода должен быть уже определён в реестре. Внутренние ошибки
...
4. **Изоляция внутреннего состояния солвера**: Промежуточные `TypeVar` солвера (форма Display
   `t<N>`) не должны проникать в видимые пользователю сообщения. Тестовая привязка:
...
5. **Граница E8xxx**: E8xxx используется только для проблем внутренней согласованности компилятора
   ...
   должны сопровождаться минимальными инструкциями по воспроизведению.
...
> пространство E6xxx/E7xxx одновременно обслуживает два канала: кодовое пространство общее, но
> каналы представления различаются.
...
| Канал значений ошибок внутри программы | Носитель `Err` значения `Error` для `Result(T, Error)` из std | Значение языка, потребляется программой через `match`/сравнение |
...
- **Стабильный контракт**: Семантика уже распределённых кодов не меняется между версиями; одна и та
  же семантика не использует повторно удалённые коды (прецедент E6002).
- **Поверхность потребления**: Сравнение `e.code == "E6xxx"` внутри программы — единственный
  программируемый контракт определения; документация согласуется через `yaoxiang explain E6xxx`;
...
- **Пользовательские ошибки**: E в `Result(T, E)` является параметром дженерика; серьёзное
  ...
  носитель, его система кодов не ограничивает пользовательские типы E.

#### Правила распределения кодов

1. Коды значений ошибок времени выполнения разделяют пространство E6xxx/E7xxx с диагностическими
   кодами компилятора; новые коды распределяются по **реальным триггерам**, а не резервируются под
   воображаемые сценарии.
2. Сначала регистрация, затем использование: новые коды входят в авторитетный реестр и проходят
   трёхстороннюю проверку согласованности (codes/*.rs ↔ locales ↔ таблица кодов в данном документе)
   до того, как могут быть выброшены. Источником регистрации кодов значений ошибок времени
   выполнения служит таблица `RUNTIME_ERROR_CODES` в `src/std/result.rs` (наряду с диагностическими
   кодами подчиняется порогу сборки `build.rs` и проверке `tools/code-tables`).
3. E7xxx зарезервирован под значения ошибок std.io / std.net (в настоящее время пуст; активируется
   при превращении io/net в `Result`).
4. Точки выброса: различные модули std создают значения `Error` через `error_new(code, message)`; на
   стороне потребления `std.result.unwrap_err` извлекает носитель `Err`, а `std.result.code/message`
   считывает поля.
...
(в месте определения варианта — реестр кодов). В период развития стабильный контракт `code` данного
...
> являются статическими сообщениями и не требуют параметров.
...
```
1. Считать language.default из yaoxiang.toml уровня проекта
2. Если не настроено, считать из пользовательского ~/.yaoxiang/yaoxiang.toml
3. Если ни одно не настроено, по умолчанию используется "en"
4. Компилятор создаёт I18nRegistry (один раз) на основе выбранного языка
5. Все ошибки отображают сообщения через этот I18nRegistry
```
...
│  Цель: команда explain может напрямую считывать данные i18n               │
└─────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────┐
│  Этап 2: YaoXiang компилирует пользовательский проект (здесь             │
│  происходит рендеринг)                                                   │
│                                                                           │
│  При вызове макроса error!:                                              │
│  1. Считать yaoxiang.toml для получения языковых предпочтений             │
│  2. Загрузить JSON i18n для соответствующего языка из бинарника           │
│     компилятора                                                          │
...
│  // Напрямую выводит финальную строку, без какого-либо поиска             │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| `Diagnostic.message`         | Отрисованная строка                        | Хранит финальный результат                |
...
Серьёзность ошибки управляется перечислением `DiagnosticLevel`, отделённым от нумерации кодов
ошибок:
...
Поскольку данный RFC проектирует систему кодов ошибок с нуля, проблем обратной совместимости нет.
...
1. LSP-сервер интегрирует вывод JSON команды `explain`
