...
> **⚠️ Отклонён**: Данный документ отклонён, содержимое объединено в [RFC-026: Основной механизм FFI](./026-ffi-core-mechanism.md).
...
> **Хороший вкус (Good
> Taste)**: Язык должен предоставлять атомарные возможности, а не универсальный функциональный набор. Сложность следует решать через библиотеки, а не накапливать в компиляторе.
...
`load_library` возвращает дескриптор `DynamicLibrary`, `register_library_symbols`
регистрирует имена символов в таблице известных символов `FfiRegistry`. После этого пользователь по-прежнему использует объявление через `native`:
...
Перед вызовом FFI функции вызывающая сторона должна убедиться, что целевая функция является реентерабельной. Среда выполнения не выполняет автоматическую проверку — это неразрешимая статическая задача.
