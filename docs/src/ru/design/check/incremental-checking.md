...
В режиме watch любое изменение файла приводит к повторной проверке всех файлов (полная повторная
проверка), а дебаунсинг использует busy-wait (проверка каждые 50 мс), что вызывает бесполезную
нагрузку на CPU.

## Решение

Использовать `CheckSession` для управления состоянием инкрементной проверки, используя
...
- Режим watch по-прежнему использует busy-wait дебаунсинг (`Instant::now()` + `recv_timeout` в
  `command.rs`)
- `check_incremental` внутренне всё ещё вызывает `check_files_with_diagnostics` (полные пути), не
  используя по-настоящему инкремент
