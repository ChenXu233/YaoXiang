...
1. **Параметр пути**: `yaoxiang check` по умолчанию проверяет текущий каталог, но также можно
   указать путь: `yaoxiang check src/`
2. **Разделение проверки и форматирования**: запускайте `check` и `format --dry-run` отдельно — это
   упрощает локализацию проблем
