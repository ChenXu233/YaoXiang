...
| Код выхода | Значение                       | Действие CI         |
| ---------- | ------------------------------ | ------------------- |
| `0`        | Нет ошибок                     | Успех               |
| `1`        | При проверке обнаружены ошибки | Провал              |
| `2`        | Файл `.yx` не найден           | Зависит от настроек |
...
2. **Разделяйте проверку и форматирование**: запускайте `check` и `format --dry-run` отдельно для
   упрощения обнаружения проблем
3. **Используйте `--no-progress`**: в среде CI индикатор прогресса не нужен
4. **Используйте `--color never`**: во избежание загрязнения логов ANSI-цветами
5. **Кешируйте зависимости**: используйте механизм кеширования CI для ускорения сборки
