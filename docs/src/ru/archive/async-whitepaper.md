...
**Модель Spawn**, заимствованная из «И-Цзин. Гуа Фу» — «Все существа действуют сообща, и я наблюдаю их возвращение». Это парадигма параллелизма в языке программирования, которая позволяет разработчикам описывать логику синхронным, последовательным образом, в то время как среда выполнения языка автоматически и эффективно выполняет вычислительные единицы параллельно, как все существа, действующие сообща, и в конечном итоге объединяет их.
...
| **混合求值模式** | `@eager`（装饰器，强制急切）、`@auto`（装饰器，保持并行） |
