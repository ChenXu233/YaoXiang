...
В разделе [основы match](../control-flow/match.md) вы изучили базовое использование `match` —
литералы, идентификаторы, шаблоны-джокеры. Теперь мы подробно рассмотрим все возможности
сопоставления с образцом в YaoXiang.
...
Первые три базовых образца вы уже изучили в предыдущей главе. Эта глава посвящена четырём
продвинутым видам образцов.

## Образцы перечислений

Образцы перечислений — наиболее часто используемая расширенная возможность `match`. Они позволяют
деструктурировать варианты перечисления и извлекать внутренние данные.
...
В `rgb(r, g, b)` идентификаторы `r`, `g`, `b` — это образцы-идентификаторы: они захватывают три
значения внутри варианта `rgb`.
...
Запись `{ width: w, height: h }` означает «извлечь поле `width` из записи и связать его с переменной
`w`, а поле `height` связать с переменной `h`». Записи `x: _` и `y: _` означают «эти поля
присутствуют, но их значения нас не интересуют».

**Сокращённая запись**: когда имя поля и имя переменной совпадают, можно использовать краткую форму
— компилятор автоматически выполнит деструктуризацию в одноимённые переменные:
...
Оператор `|` объединяет несколько образцов: совпадение засчитывается, если значение соответствует
любому из них:
...
Добавление `if условие` после ветви сопоставления делает её срабатывание условным: ветвь выбирается,
только если образец совпал **и** условие истинно:
...
Переменные в охранном выражении берутся из образца перед ним: в `adult(n) if n >= 18` сначала `n`
захватывает значение, а затем проверяется условие `n >= 18`.

## Проверка полноты

Компилятор YaoXiang гарантирует, что `match` покрывает все возможные случаи. Если какая-то ветвь
пропущена, компилятор сообщит об ошибке:
...
Это важный механизм YaoXiang для защиты от неожиданностей во время выполнения: при добавлении нового
варианта компилятор напомнит обновить все места использования `match`.

## Вложенные образцы

Настоящая сила образцов раскрывается в **вложенности** — внутри одного образца можно использовать
другой:
...
В `add(literal(0), right)` внешний слой — это образец перечисления `add`, а внутренний — литеральный
образец `literal(0)`: два уровня вложенности, одно сопоставление.
...
YaoXiang. Освоив его, вы будете писать более безопасный и выразительный код.
