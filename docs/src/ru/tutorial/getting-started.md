...
>
> **Примечание**: Примеры кода в этом документе написаны в соответствии со спецификацией языка YaoXiang. При обнаружении синтаксических различий при запуске обратитесь к [спецификации языка](../design/language-spec.md).
...
# или с использованием релизной версии
...
# Явные аннотации типов (рекомендуется использовать договорённости о типовых соглашениях)
count: Int = 100

# По умолчанию — неизменяемые (мера безопасности)
x = 10
...
```yaoxiang
# Определение сопрограммы (автоматическое асинхронное выполнение)
fetch_data: (url: String) -> JSON spawn = {
    HTTP.get(url).json()
}

# Вызов сопрограммы (автоматический параллелизм, без await)
main: () -> Void = {
    # Два вызова выполняются автоматически параллельно
    user = fetch_user(1)     # Автоматически параллельно
    posts = fetch_posts()    # Автоматически параллельно

    # При запросе результатов — автоматическое ожидание
    print(user.name)
    print(posts.length)
}
```
...
- 🏗️ Ознакомьтесь с [документацией по архитектуре](../architecture/) для понимания деталей реализации
- 💡 Прочитайте [Манифест дизайна](../YaoXiang-design-manifesto.md) для понимания базовых принципов
...
- [Руководство по вкладу](../guides/dev/)