...
Эта глава углубляется в систему типов YaoXiang и научит **определять собственные типы**.
...
| Родовой тип | `List: (T: Type) -> Type = { ... }`            |

Обратите внимание: **определение типа само по себе также имеет вид `name: Type = value`**.

## Тип записи

Тип записи (в других языках называемый «структурой») — это самый базовый способ организации данных в
YaoXiang:
...
Поля могут иметь значения по умолчанию, и при создании их можно указывать необязательно:
...
В одном и том же файле функции, объявленные с `pub`, автоматически привязываются к типам,
определённым в этом же файле:
...
регистре, варианты с данными — с использованием функционального синтаксиса:
...
Основная идея перечислений: **каждый вариант сам по себе также является типом**.
...
заключается в том, чтобы включить имя интерфейса в запись:
...
Интерфейсы реализуют полиморфизм — любой тип, реализующий `Drawable`, можно передать в функцию,
принимающую `Drawable`.
