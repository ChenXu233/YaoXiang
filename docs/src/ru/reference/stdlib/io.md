...
Функции начиная с `read_line` и далее зависят от I/O операционной системы и **не экспортируются** в
целевой платформе `wasm32`: `read_line`, `read_file`, `write_file`, `append_file`. `print` /
...
Последовательно выводит все аргументы, **без перевода строки**. Между несколькими аргументами
вставляется один пробел.

Аргументы форматируются: `String` выводит своё содержимое напрямую; `List` / `Dict` / `Tuple`
раскрываются рекурсивно; остальные значения выводятся в виде литералов.
...
Аналогично [`print`](#print), но в конце вывода добавляется перевод строки.
...
Возврат: вся прочитанная строка, **без завершающего символа перевода строки** (`\n` или `\r\n`).
Ошибка: при ошибке чтения выбрасывается `E6007`.

> Интерактивные примеры не могут быть автоматически запущены в документации, приведённый ниже код
> даётся только для справки.
...
Читает всё содержимое файла целиком в строку.

- `path` — путь к файлу (только для чтения, заимствование)

Возврат: всё содержимое файла. Ошибка: при отсутствии файла или отсутствии прав доступа
...
Возврат: `true` при успешной записи. Ошибка: если каталог не существует или нет прав доступа,
выбрасывается `E6007` (не возвращает `false`).
...
Возврат: `true` при успешной записи. Ошибка: при отсутствии прав доступа выбрасывается `E6007`.
...
Это внутренняя вспомогательная функция, вызываемая по обратному вызову из общего пути форматирования
среды выполнения; в повседневном коде следует напрямую использовать
[`std.convert.to_string`](./convert#to_string).

- `value` — произвольное значение
- `type_name` — строка с именем типа

Возврат: строковое представление с префиксом типа.
