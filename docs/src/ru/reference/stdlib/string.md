...
Модуль операций со строками. За исключением `format`, все функции принимают аргументы по
неизменяемой ссылке (`&String`); исходная строка остаётся доступной после вызова.

При несоответствии типов аргументов все функции деградируют до **семантики пустой строки** (вместо
выдачи ошибки): `split`/`trim`/`upper` и другие трактуют аргументы, не являющиеся `String`, как
`""`. Это означает, что неверный тип аргумента не вызовет сбой, но и не даст ожидаемого результата —
рекомендуется полагаться на проверку типов, которая перехватит ошибку на этапе компиляции.
...
Разбивает `s` по `sep` и возвращает список подстрок.

- `s` — строка для разбиения
- `sep` — разделитель; **если это пустая строка, выполняется посимвольное разбиение**
...
Возвращает: новую строку без начальных и конечных пробелов (`s` не изменяется).
...
- `old` — если это пустая строка, `s` **возвращается без изменений** (без вставок)
...
Проверяет, содержится ли `sub` в `s`. Для пустой строки всегда возвращает `true`.
...
Проверяет, заканчивается ли `s` суффиксом `suffix`. Для пустого `suffix` всегда возвращает `true`.
...
> Возвращается байтовое смещение. Если строка содержит многобайтовые символы, можно использовать
> `chars` для преобразования и последующего определения индекса символа.
...
- `end` — конечный индекс символа (не включая), по умолчанию — конец строки

Возвращает: результат вырезки. Границы, выходящие за допустимый диапазон, **ограничиваются** до
корректных значений без ошибки; при `start > end` результат ограничивается до пустой строки.
...
Разбивает на список строк из отдельных символов (по скалярным значениям Unicode).
...
Сцепляет две строки. Можно также использовать оператор `+`.
...
Форматирует строку по плейсхолдерам `{index}` с необязательными спецификаторами ширины/выравнивания.
...
Литеральные фигурные скобки записываются удвоением: две открывающие дают одну литеральную
открывающую, две закрывающие — аналогично.
...
Возвращает: при успехе — `Result.ok(Float)`; при неудаче — `Result.err(Error)` с полем `code` равным
`E6011`.
