...
Модуль строковых операций. За исключением `format`, все функции принимают аргументы по неизменяемой
ссылке (`&String`); исходная строка остаётся доступной после вызова.

Все функции при несоответствии типов аргументов вырождаются в **семантику пустой строки** (а не
сообщают об ошибке): `split`/`trim`/`upper` и т.п. рассматривают аргументы, не являющиеся `String`,
как `""`. Это означает, что неверный тип аргумента не прервёт выполнение, но и не даст ожидаемого
результата — рекомендуется полагаться на проверку типов в компиляторе, чтобы отловить такие случаи
на этапе компиляции.
...
Возвращает: новую строку без начальных и конечных пробелов (без изменения `s`).
...
- `old` — если пустая строка, **возвращает `s` без изменений** (без вставок)
...
Встречается ли `sub` в `s`. Для пустой подстроки всегда `true`.
...
Заканчивается ли `s` на `suffix`. Для пустого `suffix` всегда `true`.
...
Возвращает: при нахождении — индекс; при отсутствии — `-1`.

> Возвращается байтовое смещение. Для строк с многобайтовыми символами можно сначала преобразовать
> через `chars`, а затем искать индекс символа.
...
Извлекает подстроку по **символьным** индексам в диапазоне `[start, end)`.

- `start` — начальный символьный индекс, по умолчанию `0`
- `end` — конечный символьный индекс (не включая), по умолчанию — конец строки

Возвращает: результат среза. Выходящие за границы индексы **фиксируются** в допустимом диапазоне без
ошибки; при `start > end` фиксируется как пустая строка.
...
Литеральная фигурная скобка обозначается удвоением: две открывающие фигурные скобки дают одну
литеральную открывающую, и аналогично для закрывающих.

Возвращает: форматированную строку. Аргументы сначала преобразуются в строки (аналогично
`convert.to_string`); при выходе индекса за границы подставляется пустая строка, недопустимая ширина
...
Возвращает: при успехе — `Result.ok(Int)`; при неудаче — `Result.err(Error)` с `code` равным
...
Возвращает: при успехе — `Result.ok(Float)`; при неудаче — `Result.err(Error)` с `code` равным
`E6011`.
