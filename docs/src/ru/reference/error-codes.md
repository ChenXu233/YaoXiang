...
сообщений. Коды ошибок группируются по диапазонам номеров, каждый код соответствует определённому
сценарию ошибки.
...
Ошибки, возникающие на этапах лексического анализа (Lexer) и синтаксического анализа (Parser).

| Код   | Шаблон                                                                                 | Описание                        |
| ----- | -------------------------------------------------------------------------------------- | ------------------------------- |
| E0001 | `Invalid character: '{char}'`                                                          | Недопустимый символ             |
| E0002 | `Invalid number literal: '{literal}'`                                                  | Недопустимый числовой литерал   |
| E0003 | `Unterminated string starting at line {line}`                                          | Незакрытая строка               |
...
Ошибки этапа проверки типов, охватывающие типы переменных, вызовы функций, сопоставление с образцом,
инстанцирование дженериков, параллельную семантику и распространение ошибок.
...
| E1030 | `Pattern non-exhaustive: missing patterns {patterns}`                                                 | Неисчерпывающий шаблон                                                         |
| E1031 | `Unreachable pattern: '{pattern}'`                                                                    | Недостижимый шаблон                                                            |
...
| E1055 | `Constraint type '{type}' can only be used in generic context`                                        | Ограничение применимо только в обобщённом контексте                            |
...
| E1091 | `Generic meta-type self-reference is not allowed: '{decl}'`                                           | Недопустимый обобщённый мета-тип                                               |
| E1062 | `Const generic constraint violation: {reason}`                                                        | Нарушение ограничения const-дженерика                                          |
...
| E1096 | `Interface '{name}' expects {expected} type argument(s), found {found}`                               | Несоответствие числа аргументов при инстанциации интерфейса                    |
| E1097 | `Interface member '{member}' conflicts with field of type '{type}'`                                   | Конфликт имени члена интерфейса с полем                                        |
...
| E1100 | `Duplicate implementation of '{type}.{method}' (override is not allowed)`                             | Дублирование реализации метода с той же сигнатурой (переопределение запрещено) |
| E1101 | `Type '{type}' does not implement interface '{interface}' and cannot enter this existential position` | Тип не реализует интерфейс (проверка члена экзистенциального типа)             |

## E2xxx — Семантический анализ

Ошибки этапа семантического анализа, охватывающие область видимости, время жизни переменных,
владение и разбор сигнатур функций.
...
| E2010 | `Cannot assign to immutable variable '{name}'`                           | Присвоение неизменяемому                      |
...
| E2092 | `Invalid signature: missing '->'`                                        | Отсутствует '->' в сигнатуре                  |
...
| E2095 | `Invalid signature: parameter '{name}' shadows generic`                  | Затенение обобщённого типа параметром         |
...
| Код   | Шаблон                                                         | Описание                        |
| ----- | -------------------------------------------------------------- | ------------------------------- |
| E4001 | `Type '{type}' does not satisfy the trait bound '{trait}'`     | Нарушение ограничения дженерика |
...
| E4005 | `Associated type '{assoc_type}' not found in '{container}'`    | Связанный тип не найден         |
...
Внутренние ошибки компилятора, обычно свидетельствующие о баге в самом компиляторе. При обнаружении
таких ошибок, пожалуйста, сообщайте о них в
[GitHub Issues](https://github.com/yaoxiang/yaoxiang/issues).

| Код   | Шаблон                                      | Описание                      |
| ----- | ------------------------------------------- | ----------------------------- |
| E8001 | `Internal compiler error: {message}`        | Внутренняя ошибка компилятора |
| E8002 | `Unexpected compiler panic: {reason}`       | Неожиданный панический сбой   |
...
| W1063 | ``const generic constraint cannot be evaluated: `{constraint}` ({var} = {value})`` | Невозможно вычислить ограничение const-дженерика |
| W1080 | `Constraint cannot be proven at compile-time, degraded to runtime check`           | Понижение доказательства времени компиляции      |

---

Всего **118** диагностических кодов (111 кодов ошибок + 7 кодов предупреждений), авторитетным
источником является реестр `define_codes!` (`src/util/diagnostic/codes/`); полный зарегистрированный
список каждого кода определяется реестром; на этой странице перечислены распространённые коды по
семействам.
