...
Модуль `std.assert` предоставляет единый механизм утверждений — runtime `assert` и уточняющий тип
compile-time `Assert` являются двумя сторонами одной и той же примитивы.
...
`assert(false, "msg")` эквивалентно raise — нет необходимости в отдельном ключевом слове
throw/raise.
