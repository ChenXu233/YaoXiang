...
| Присутствуют свободные переменные времени выполнения       | Вставляется проверка времени выполнения, инжектируется потокочувствительный набор допущений Γ |
...
```yaoxiang
// Итератор диапазона (пункт I №300: Range — значение первого класса, step — третья компонента)
Range: Type = {
    start: Int,
    end: Int,
    step: Int,
...
> **Пункт I №300**: Range — значение первого класса — `r = 1..10` допустимо, проверка членства
> `x in r` и итерация `for i in r` раскрываются в соответствии со статическими типами. Литерал
> step=0 отвергается во время компиляции; динамический step=0 приводит к ошибке времени выполнения
> (после введения будущей системы ошибок будет повышен до Result, №301).
