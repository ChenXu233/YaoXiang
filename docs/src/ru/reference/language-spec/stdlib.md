...
```
// В случае успеха возвращает значение, в случае неудачи — возвращает err наверх
data = fetch_data()?
...
`assert` и уточняющий тип времени компиляции `Assert` являются двумя сторонами одной и той же
примитивы.
...
// assert: утверждение времени выполнения (value introducer для Assert)
...
| Присутствуют свободные переменные времени выполнения       | Вставляется проверка во время выполнения, инжектируется потоко-чувствительный набор предположений Γ |
...
```yaoxiang
// Итератор диапазона (#302: Range — полноценный тип, его идентичность во время выполнения
// представляет собой неизменяемую запись из трёх скаляров, без оболочки Tuple;
// выводится как `1..10` / `1..10..2`, структурное равенство, именованные поля)
Range: Type = {
    start: Int,
    end: Int,
    step: Int,
...
// for выполняет статическую диспетчеризацию по типу)
for i in 0..10 {
    print(i)
}

// Форма с шагом (две точки, без нового ключевого слова)
for i in 0..10..2 {
    print(i)
}
```
...
> `r.start`/`r.end`/`r.step`; во время выполнения `x in r` обращается к `std.range.contains`
> (проверка границ + выравнивание шага), что доказывает, что пайплайн распознаёт предикат интервала
> `x >= r.start && x < r.end && (x - r.start) % r.step == 0` (интервал сохраняется как интервал, без
> материализации). Литерал `step=0` отвергается на этапе компиляции, динамический `step=0` — ошибка
> времени выполнения (после внедрения будущей системы ошибок будет повышен до Result, #301).
> Инстанциация интерфейса (объявление `Iterator(Int)` в теле типа) ожидает принятия RFC-011a (#307);
> на данный момент протокольный аспект предоставляется `std.range`.
...
| Модуль           | Описание                                                                |
| ---------------- | ----------------------------------------------------------------------- |
| `std.assert`     | Механизм утверждений — runtime assert + уточняющий тип Assert           |
...
| `std.iterator`   | Итераторы (на данный момент протокол предоставляется `std.range`, #302) |
| `std.range`      | Итератор Range, предикаты интервалов, адаптеры (#302)                   |
...
| `std.assert` | Единый механизм: `Assert(C)` во время компиляции и `assert(x > 0)` во время выполнения (RFC-030) |
