...
В данном документе определяется спецификация FFI (Foreign Function Interface, внешний интерфейс функций) для языка программирования YaoXiang, включая определения типов, объявления функций, привязки методов и обработку непрозрачных типов.
...
Непрозрачные типы определяются в блоке `unsafe {}`, через `return` возвращаются в вышестоящую область видимости:
...
Привязка методов может осуществляться в любом месте, поскольку типы являются контейнерами данных:
...
### 5.2 Не ресурсные типы могут выполняться параллельно
