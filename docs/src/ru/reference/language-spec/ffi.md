...
Настоящий документ определяет спецификацию FFI (интерфейса внешних функций) языка программирования
YaoXiang, включая определение типов, объявление функций, привязку методов и обработку непрозрачных
типов.

> **Подробное описание**: полное описание FFI, мотивация и компромиссы подробно изложены в
...
Непрозрачный тип определяется в блоке `unsafe {}` и возвращается в вышестоящую область видимости
через `return`:
...
Для типов параметров FFI-функций используются типы YaoXiang напрямую, а отображение на C-типы
компилятор выполняет автоматически:
