...
**Spawnモデル**は、《易・復卦》の「万物spawn
action、吾れ以って復を観察す」に由来します。これはプログラミング言語のコンカレンシーパラダイムであり、開発者が同期・順序的な思考でロジックを記述できるようにし、言語ランタイムがその中の計算ユニットを万物spawn般に自動かつ効率的にコンカレントに実行させ、最終的に統一的に協調させます。
...
**Void 自動即時評価ルール：**
Void を返す関数は自動的に即時評価されます（デコレータ不要）。なぜならば副作用は実行されなければならないからです。
...
戻り値の型が `-> T spawn` とマーキングされた関数は、即座に `Async[T]`
...
`spawn`
キーワードは同期思考と非同期実装を接続する唯一の橋渡しであり、3重のセマンティクスを持っています：
...
> _"万物spawn action、吾れ以って復を観察す。"_ —— 《易・復卦》
>
