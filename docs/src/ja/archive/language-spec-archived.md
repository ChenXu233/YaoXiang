...
> **注意：本文書はアーカイブ済みでありメンテナンスされていません。**
...
本書では YaoXiang プログラミング言語の構文と意味論を定義する。これは言語の権威あるリファレンスであり、コンパイラおよびツール実装者を対象としている。

チュートリアルとサンプルコードについては、[YaoXiang ガイド](../guide/YaoXiang-book.md) および [tutorial/](../tutorial/) ディレクトリを参照されたい。

### 1.2 準拠

プログラムまたは実装が本書に定義されたすべての規則を満たす場合、YaoXiang 仕様に準拠しているとみなす。
...
これらのキーワードはあらゆるコンテキストで特殊意味を持ち、識別子として使用することはできない。
...
| `false` | Bool | 偽値 |
| `void` | Void | 空値 |
| `some(T)` | Option | Option 値値variant |
| `ok(T)` | Result | Result 成功値variant |
| `err(E)` | Result | Result エラー値variant |

### 2.5 識別子

識別子は文字またはアンダースコアで始まり、後続の文字は文字、数字、またはアンダースコアにできる。識別子は大小文字を区別する。

特殊識別子：
- `_` はプレースホルダーとして使用され、ある値を無視することを表す
...
コードは4つのスペースを使用してインデントする必要がある。Tab 文字の使用は禁止。これは強制的な構文規則である。
...
**インターフェースへの直接代入**：具象型は直接インターフェース型変数に代入可能（構造的サブタイピング）

```yaoxiang
// 直接代入（コンパイル時に具象型を特定可能 → ゼロオーバーヘッド呼び出し）
d: Drawable = Circle(1)
d.draw(screen)        # コンパイル後：vtable なしで circle_draw を直接呼び出し

// 関数戻り値（コンパイル時に特定不可 → vtable 呼び出し）
...
| シナリオ | 推論結果 | 呼び出し方式 |
|------|----------|----------|
| 具象型への直接代入 | 具象型を特定可能 | 直接呼び出し（ゼロオーバーヘッド） |
| 関数戻り値 | 不明 | vtable |
...
**構文**：型積 `A & B` は A と B の両方を同時に満たす型を表す
...
**コア理解**：名前付き関数と Lambda 式は同じものである！唯一の違いは、名前付き関数が Lambda に名前を付けたものである。
...
**重要ポイント**：シグネチャでパラメータ型を完全に宣言すると、Lambda ヘッダーのパラメータ名は冗長になり、省略可能になる。

### 3.18 パラメータスコープ規則

**パラメータが外側変数をシャドウする**：シグネチャ内のパラメータスコープは関数ボディをオーバーライドし、内部スコープの方が高優先度である。

```yaoxiang
x = 10  # 外側変数
double: (x: Int) -> Int = x * 2  # ✅ パラメータ x が外側の x をシャドウ、結果は 20
```

### 3.19 型注釈の位置

型注釈は以下のいずれかの位置に配置可能で、**少なくとも1箇所に注釈を付ける必要がある**：

| 注釈位置 | 形式 | 説明 |
|----------|------|------|
| シグネチャのみ | `double: (x: Int) -> Int = x * 2` | ✅ 推奨 |
| Lambda ヘッダーのみ | `double = (x: Int) => x * 2` | ✅ 有効 |
| 両方に注釈 | `double: (x: Int) -> Int = (x: Int) => x * 2` | ✅ 冗長だが許容 |
...
#### 5.9.1 意味論：各反復は新しい値のバインディングである

YaoXiang の for ループの意味論は従来の言語と異なる：**各反復は新しい値のバインディングであり、同じ変数を変更するのではない**。
...
| 反復 | ループ変数の動作 |
|------|----------------|
| 1回目 | 新しいバインディングを作成 `i = 1`、ループ本体を実行、1 を印刷 |
| 2回目 | 新しいバインディングを作成 `i = 2`（以前のバインディングは破棄済み）、ループ本体を実行、2 を印刷 |
| 3回目 | 新しいバインディングを作成 `i = 3`、ループ本体を実行、3 を印刷 |
| 4回目 | 新しいバインディングを作成 `i = 4`、ループ本体を実行、4 を印刷 |
| 終了 | ループ本体終了、バインディング破棄 |

**重要ポイント**：各反復の終了後、その反復で作成されたバインディングは破棄される。次に反復では前回反復のバインディングとは完全に無関係な新しいバインディングとなる。
...
// ✅ 有効：for mut を使用するとバインディングの変更が許可される
for mut i in 1..5 {
    i = i + 1  # 変更が許可される
}
```
...
for ループ変数は外側スコープに既に存在する変数をシャドウできない：
...
YaoXiang は**統一宣言モデル**を採用している：変数、関数、メソッドはすべて同じ形式 `name: type = value` を使用する。
...
```yaoxiang
// Void 以外の戻り値型 - return の使用必須
add: (a: Int, b: Int) -> Int = {
    return a + b
}

// Void 戻り値型 - return は省略可能
...
| 規則 | 説明 |
|------|------|
| 位置は 0 から開始 | `func[0]` は最初のパラメータ（インデックス 0）にバインディング |
| 最大位置 | 関数パラメータ数未満である必要がある |
| 負数インデックス | `[-1]` は最後から数えたパラメータ |
| プレースホルダー | `_` はその位置をスキップし、ユーザーが提供する |

### 6.8 カレリングサポート

バインディングは本質的にカレリングをサポートする。呼び出し時に提供されたパラメータが残りパラメータより少ない場合、残りのパラメータを受け取る関数を返す：
...
明示的に宣言された並行領域で、ブロック内のタスクは並作実行される：
...
データ並列ループで、ループ本体がすべてのデータ要素に並作実行される：
...
モジュールはファイルを境界とする。各 `.yx` ファイルがモジュールである。
...
YaoXiang は**所有権モデル**を使用してメモリを管理し、各値は唯一つの所有者を持つ：
...
| 制約 | 意味論 | 説明 |
|------|------|------|
| **Send** | スレッド境界を越えて安全に転送可能 | 値を別のスレッドに移動可能 |
| **Sync** | スレッド境界を越えて安全に共有可能 | 不変参照を別のスレッドに共有可能 |
...
```
Send ──► スレッド境界を越えて安全に転送可能
  │
  └──► Sync ──► スレッド境界を越えて安全に共有可能
       │
...
| `Ref[T]` | 可変参照 | ⚠️ 同期必要 | 並行変更可能とマーク、コンパイル時にロック使用検査 |
| `Atomic[T]` | アトミック型 | ✅ 安全 | 低レベルアトミック操作、ロックフリー並行 |
| `Mutex[T]` | 相互排除ロック包装 | ✅ 安全 | 自動ロック解除解除、コンパイル時保証 |
| `RwLock[T]` | 読み書きロック包装 | ✅ 安全 | 読み取り多い書き込み少ないシナリオ最適化 |
...
| キーワード | 仕様ステータス | コード実装 | 説明 |
|--------|---------|---------|------|
| `struct` | 削除済み | ❌ なし | 統一構文 `Name: Type = {...}` を使用 |
| `enum` | 削除済み | ❌ なし | variant 構文 `Name: Type = { A \| B \| C }` を使用 |
...
以下の Rust スタイル機能は**実装しない**：

| 機能 | 理由 |
|------|------|
| ライフタイム `'a` | 参照概念がないため不要 |
| 借用チェッカー | ref = Arc で代替 |
| `&T` 借用構文 | Move 意味論を使用 |
...
YaoXiang は**純粋関数型設計**を採用しており、すべての操作は関数で実装される。バインディングメカニズムは関数を型に関連付け、呼び出し元がメソッドのように関数を呼び出せるようにする。

```
バインディング宣言 ::= 型 '.' 識別子 '=' 関数名 '[' 位置リスト ']'
位置リスト ::= 位置 (',' 位置)* ','?
位置     ::= 整数（0 から開始） | 負整数 | プレースホルダー
```
...
**意味論**：
- `Type.method = func[0]` は `obj.method(arg)` の呼び出し時、`obj` を `func` の 0 番目のパラメータにバインディングすることを意味する
- 残りパラメータは元の順序で埋める
...
Point.wrong = distance[0, 0]          # 位置的重复（許容されない場合）
Point.wrong = distance[-2]            # -2 が範囲外
```

### 10.5 自動バインディング

モジュールで定義され最初のパラメータがモジュール型である関数について、メソッド呼び出し構文を自動的にサポートする：
...
**自動バインディング規則**：
- 関数定義はモジュールファイルにある
- 関数の 0 番目のパラメータ型とモジュール名が一致する
- 関数はモジュール外で自動バインディングするために `pub` である必要がある

### 10.6 バインディングとカレリングの関係
...
```
// 単一位置バインディング（デフォルトで 0 番目にバインディング）
Type.method = func[0]
...
| v1.1.0 | 2025-01-04 | 沫郁酱 | match arm が `=>` を使用するように修正（`->` ではなく）；関数定義構文を更新；型定義構文を更新；コード実装との違いの説明を追加 |
| v1.2.0 | 2025-01-05 | 沫郁酱 | 純粋仕様に精简化、サンプルコードは tutorial/ ディレクトリに移動 |
| v1.3.0 | 2025-01-05 | 沫郁酱 | 並作モデル仕様を追加（3層並行アーキテクチャ、spawn 構文、アノテーション）；型システム制約を追加（Send/Sync）；並行安全型を追加（Mutex、Atomic）；エラー処理を更新（? 演算子）；未実装機能リストを更新 |
| v1.4.0 | 2025-01-15 | 晨煦 | 所有権モデルを更新（デフォルト Move + 明示的 ref=Arc）；unsafe キーワードを追加；ライフタイム `'a` と借用チェッカーを削除；未実装機能リストを更新 |
| v1.5.0 | 2025-01-20 | 晨煦 | メソッドバインディング仕様を追加（RFC-004）：位置インデックスが 0 から開始；デフォルトで 0 番目にバインディング；負数インデックスと複数位置バインディングをサポート |
| v1.6.0 | 2025-02-06 | 晨煦 | RFC-010（統一型構文）を統合：`type Name = {...}` 構文を更新；シグネチャ内のパラメータ名の関数定義；Type.method メソッド構文；RFC-011（ジェネリックシステム）を統合：ジェネリック型 `[T]`、型制約 `[T: Clone]`、関連型 `Item: T`、コンパイル時ジェネリック `[N: Int]`、条件型 `If[C, T, E]`、関数オーバーロード特殊化、プラットフォーム特殊化を追加 |
| v1.7.0 | 2026-02-13 | 晨煦 | RFC-010 更新：`Name: Type = {...}` で `type Name = {...}` を置換；`Type`（大文字）のみがメタ型キーワード；すべての宣言が統一構文を使用 |
| v1.8.0 | 2026-02-18 | 晨煦 | RFC-010 新規機能：デフォルト値初期化、組み込みバインディング構文；RFC-004 新規機能：組み込みバインディング、匿名関数バインディング |
| v1.8.1 | 2026-02-20 | 晨煦 | メタ型はキーワードには属さない。 |

---

> 本仕様は YaoXiang プログラミング言語のコア構文と意味論を定義する。
> チュートリアルとサンプルコードについては、[YaoXiang ガイド](../guide/YaoXiang-book.md) および [tutorial/](../tutorial/) ディレクトリを参照されたい。