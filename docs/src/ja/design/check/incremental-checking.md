...
`CheckSession` を使用して增量チェックの状態を管理し、`ModuleDependencyGraph::affected_modules`
を利用して影響を受けるファイルのみを再チェックする。
...
- watch モードでは依然として busy-wait debounce を使用（`command.rs` の `Instant::now()` +
  `recv_timeout`）
