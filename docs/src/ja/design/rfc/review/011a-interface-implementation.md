...
**コア戦略：所有権追跡によるインクリメンタル構築。** インターフェースを実装する全型をコンパイル時にスキャンするのではなく、各 `List(Animal)` の**所有権操作点で**インクリメンタルに収集：
...
**コンパイラ処理：** 継承チェーンを展開します。`Pet` は `{ Animal の全メソッド, name: ... }` に展開されます。`Dog` が `Pet` を宣言すると、コンパイラは `Dog` が `Animal` と `Pet` の全メソッドを同時に満たすことを検証します。
...
**継承は下位実装を仮定できる：** インターフェース `Pet` が `Animal` を継承する際、`Pet` のデフォルトメソッドは `Animal` で宣言されたメソッドを使用できます——継承しているので保証があるからです。
