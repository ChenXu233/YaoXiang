...
YaoXiang 言語の統一設定システムを設計し、ユーザーレベルとプロジェクトレベルの二つの階層をサポートし、パッケージマネージャー、コンパイラ、REPL、LSP などのコンポーネントに共有される設定インフラを提供する。
...
設定の優先度（高 → 低）：
┌─────────────────────────────────────────────┐
│ 1. プロジェクトレベル yaoxiang.toml           │ ← プロジェクトチームが制御
├─────────────────────────────────────────────┤
│ 2. ユーザーレベル ~/.config/yaoxiang/config.toml │ ← ユーザー設定
├─────────────────────────────────────────────┤
│ 3. コンパイラのデフォルト値                    │ ← 妥当な初期値
└─────────────────────────────────────────────┘
```

**ルール**：上位層が下位層を上書きし、未設定のオプションは下位層にフォールバックする。

### 設定階層の制限

| 設定セクション | ユーザーレベル | プロジェクトレベル | 消費者 |
|--------|--------|--------|--------|
| `[package].*` | ❌ | ✅ | パッケージマネージャー |
| `[yaoxiang]` | ❌ | ✅ | コンパイラ |
...
| `[fmt].indent-width` | Number | 4 | インデント幅 |
...
**初回実行**：ユーザーが初めて `yaoxiang` コマンドを実行する際、自動的にユーザーレベル設定の有無を検出する。存在しない場合は、デフォルトオプションで自動生成される。

**設定ファイルの場所**：
- プロジェクトレベル：`./yaoxiang.toml`（プロジェクトルート）
- ユーザーレベル：`~/.config/yaoxiang/config.toml`

### 設定マージのセマンティクス

異なる階層の設定は以下のルールでマージされる：

| タイプ | 戦略 | 説明 |
|------|------|------|
| スカラー (String/Number/Boolean) | 置換 | プロジェクトレベルがユーザーレベルを上書き |
| 配列 (Array) | 置換 | プロジェクトレベルがユーザーレベルを完全に置換 |
| オブジェクト (Object) | ディープマージ | フィールドごとにマージし、未定義フィールドは下位層を継承 |
...
- ✅ 既存の設定ファイルなしモードを引き続きサポート（全コンポーネントが組み込みデフォルト値を使用）
- ✅ 新規設定項目にはすべて妥当なデフォルト値を用意
- ✅ ユーザーが初めてコマンドを実行した際、デフォルトオプションで設定を自動生成
- ✅ 設定解析失敗時は、具体的な行番号とエラー理由を示す分かりやすいエラーを表示

## トレードオフ

### 利点

- 統一された設定インフラにより、重複コードが削減される
...
| 案 | 採用しなかった理由 |
|------|-----------|
| 各コンポーネントが独立した設定 | コードの重複、用户体验の分断 |
| コマンドライン引数のみサポート | ユーザー設定の永続化不可 |
...
| **Phase 3** | コマンドライン/環境変数オーバーライド、`platform` プラットフォーム制約、`[tool.*]` 拡張 |
...
| リスク | 緩和策 |
|------|----------|
| 設定項目が多すぎる | 妥当なデフォルト値を提供し、ユーザーが意識する必要がないようにする |
| パーサーが複雑 | 既存の toml ライブラリを使用 |

## 未解決問題

- [x] `features` 条件コンパイル構文？ → **別 RFC に分離**、RFC-011 ジェネリクスシステムに依存
- [x] `workspace` ワークスペース設計？ → **別 RFC に分離**、複雑度が高く独立設計が必要

### 受領済み機能（第三フェーズ）

#### `platform` プラットフォーム制約

> **注意**：以下の構文は `yaoxiang.toml` **設定ファイル**用であり、YaoXiang ソースコード（`.yx` ファイル）の構文では**ない**。ユーザーはコード内に `cfg(...)` を記述する**必要がない**。
...
**説明**：
- この構文は `yaoxiang.toml` 設定ファイルにのみ出現する
- ビルド時に `--target` パラメータに応じて対応する設定を選択する
- ユーザーは `.yx` ソースコード内に `cfg(...)` 構文を記述する**必要はない**し、記述する**べきでもない**
...
サードパーティツールが `[tool.<名前>]` 配下に設定を保存できるようにする：
...
**動作**：
- YaoXiang は未知の `[tool.*]` セクションを無視するが、設定ファイル内には保持する
- サードパーティツールは `yaoxiang tool run <名前>` で統合するか、直接アクセス可能
- ツール固有設定は検証されない
