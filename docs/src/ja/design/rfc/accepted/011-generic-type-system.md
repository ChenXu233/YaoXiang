...
> **著者**: 晨煦
> **作成日**: 2025-01-25
> **最終更新**: 2026-04-22（Type 自己記述メカニズムを更新し、ジェネリック呼び出し構文を統一）

## 概要

このドキュメントは YaoXiang 言語の**ジェネリックシステム設計**を定義し、強力なジェネリック機能によりゼロコスト抽象を実現し、コンパイル時最適化によりマクロへの依存を軽減し、デッドコード排除メカニズムを提供します。

**コア設計**：
- **統一シグネチャ構文**：`(T: Type, R: Type) -> ...` ジェネリックパラメータと通常パラメータの統一
- **Type 自己記述メカニズム**：`Type` は言語レベルの特殊存在であり、シグネチャ内の `Type` 位置は自動推論で填充
...
**価値**：
- ゼロコスト抽象：コンパイル時単態化、実行時オーバーヘッドなし
- デッドコード排除：インスタンス化グラフ分析 + LLVM 最適化
- マクロ代替：ジェネリックでマクロ使用シナリオの90%を代替
- 型安全性：コンパイル時チェック、IDE に優しい
- **明示的优于隐式**：`Type` 自己記述、コンパイラが自動推論

## 参照ドキュメント

このドキュメントの設計は次のドキュメントに基づいています：

| ドキュメント | 関係 | 説明 |
|------|------|------|
| [RFC-010: 統一型構文](./010-unified-type-syntax.md) | **構文基盤** | ジェネリック構文と統一 `name: type = value` モデルの統合 |
| [RFC-010: 統一型構文](./010-unified-type-syntax.md) | **呼び出し構文** | 第6節：ジェネリック呼び出し構文——統一 `()` 適用、`[]` を完全に削除 |
| [RFC-009: 所有権モデル](./accepted/009-ownership-model.md) | **型システム** | Move セマンティクスとジェネリックの自然な組み合わせ |
| [RFC-001: 並作モデル](./accepted/001-concurrent-model-error-handling.md) | **実行モデル** | DAG 分析とジェネリック型チェック |
...
| [RFC-022: ホーア論理静的検証](./draft/022-hol-logic-verification.md) | **終了チェック** | decreases 契約とコンパイル時評価安全保障 |

## 型宇宙思想と値依存型

YaoXiang のジェネリックシステムは**型宇宙思想**に基づいており、このメンタルモデルは言語内のすべての概念を階層構造に統一し、核心的な革新は**値依存型**を Type2 層の第一級市民として昇格させた点です。

### 値依存型とは？

**値依存型**とは、1つ以上の**値**（他の型だけでなく）に依存する型のことです。これらの値はコンパイル時に評価可能であり、したがってコンパイル段階で型安全保証を提供します。
...
従来のジェネリック相比、YaoXiang の値依存型は以下のコア優位性を持ちます：

| 特性 | 従来のジェネリック (C++/Rust) | YaoXiang 値依存型 |
|------|-------------------|---------------------|
| 型が依存する値 | 型パラメータのみに依存 | 関数呼び出し結果を含む任意の値に依存可能 |
| コンパイル時評価 | C++ テンプレート手動特殊化、Rust なし | 自動コンパイル時評価、終了保証 |
| 型レベル計算 | テンプレートメタプログラミング（複雑/危険） | 統一された型レベル計算エンジン |
| 型安全性 | C++ なし、Rust 制限的 | 完全な型安全性、コンパイル時チェック |
| 次元検証 | 実行時チェックまたは手動特殊化 | コンパイル時次元検証、実行時オーバーヘッドなし |

### 型宇宙階層と値依存型

型宇宙思想は言語概念をセマンティックロールによって異なる階層に分類し、値依存型は **Type2 層** に位置します：
...
**重要な設計**：Type2 層の関数、型構築子、値依存型は**統一的構文**を持ち、すべて `(params) -> result` の形式です：
- 通常関数：`(Int, Int) -> Int` → 戻り値は値
- 型構築子：`(T: Type) -> Type` → 戻り値は型
- 値依存型：`(n: Int) -> Type` → 戻り値は型だが、値パラメータに依存

> **Curry-Howard 同形**：この統一は偶然ではありません。Curry-Howard 同形は「型即ち命題、プログラム即ち証明」を指摘します——関数型 `A → B` は論理的含意「A ならば B」に対応し、ジェネリック `(T: Type) -> Type` は全称量化「すべての型 T について」に対応し、値依存型 `(n: Int) -> Type` は「各整数 n に対してある型が存在する」に対応します。YaoXiang が関数、型構築子、値依存型を Type2 層に統一することは、本質的に「証明」と「計算」を同一概念——**構成的証明**——に統一することです。これは Curry-Howard 同形の言語設計における直接的な具現化です：論理的命題と計算過程の両方を1つの形式（`(params) -> result`）で同時に担います。

### コンパイル時決定性保証

YaoXiang の型宇宙思想は要求します：**Type 階層のすべてはコンパイル時に決定される**。
...
コンパイラは自動的に以下を行います：
1. 型位置の関数呼び出しを検出
2. 関数に `decreases` 契約があるかを検証（下部の終了チェックメカニズムを参照）
3. コンパイル時に評価を実行
4. 結果を生成された型に埋め込み
...
| 終了保証 | ❌ | ❌ | ❌（危険） | ✅（decreases 契約） |
| 型安全性 | ❌（マクロ展開） | ✅ | ✅ | ✅ |
| 統一構文 | ❌ | ❌ | ❌ | ✅ |
| コンパイル時次元検証 | 手動特殊化 | 実行時チェック | 型族 | コンパイル時自動検証 |
| decreases 契約 | ❌ | ❌ | ❌ | ✅ |

### 終了チェックメカニズム（RFC-022 との統合）

値依存型のコンパイル時評価は**終了を保証**する必要があります、さもなくば型システムは無限ループに陥ります。YaoXiang は **decreases 契約** を通じてこれを保証し、RFC-022 とシームレスに統合します。
...
┌─────────────────────────────────────────────────────────────┐
│  1. decreases 契約をチェック                                   │
│     - decreases 有り：すべての再帰パスで递减条件が成立かを検証   │
│     - decreases なしだが明らかに終了可能：直接評価              │
│     - decreases なしかつ終了しない可能性：コンパイルエラー     │
└─────────────────────────┬───────────────────────────────────┘
                          ▼
┌─────────────────────────────────────────────────────────────┐
...
- **統一性**：終了チェックと部分的正確性検証が同じ契約メカニズムを共有
- **漸進的強化**：実行時チェックから完全に静的証明へ段階的に移行可能

## 動機

### なぜ強力なジェネリックシステムが必要なのか？

現在の主流言語のジェネリックには限界があります：

| 言語 | ジェネリック能力 | 問題点 |
|------|----------|------|
| Java | 境界型 | コンパイル時単態化、ジェネリック特殊化なし |
| C# | ジェネリック制約 | 実行時型チェック、パフォーマンスオーバーヘッドあり |
| Rust | ジェネリック + Trait | Trait システム複雑、学習曲線が険しい |
...
YaoXiang の**値依存型**は従来のジェネリックに対するコア優位性です：

| 優位性 | 説明 |
|------|------|
| **型が値に依存** | `Vec: (n: Int) -> Type` 型が具体的な値に依存する |
| **コンパイル時評価** | 型位置の関数呼び出しはコンパイル時に評価され、結果を型に直接埋め込み |
| **次元検証** | `Matrix(Float, 3, 3)` コンパイル時に行列次元を検証 |
| **型レベル計算** | `If`, `Match` などの条件型が型レベル計算をサポート |
| **終了保証** | decreases 契約によりコンパイル時評価が必ず終了することを保証 |
...
2. **デッドコード排除** - コンパイル時分析、使用されているジェネリックのみインスタンス化
3. **マクロ代替** - ジェネリックでマクロ使用シナリオの90%を代替
4. **型安全性** - コンパイル時チェック、実行時型オーバーヘッドなし
5. **IDE に優しい** - インテリジェントなヒント、明確なエラー情報
6. **値依存型** - 型は値に依存可能、コンパイル時次元検証をサポート
7. **コンパイル時評価の安全性** - decreases 契約によりコンパイル時評価の終了を保証
...
> **重要なルール**：ジェネリック型定義は**必ず `: Type` を明示的に标注**でなければならず、さもなくば HM 推論により関数として扱われます。
>
> | 書き方 | 意味 |
> |------|------|
> | `List: (T: Type) -> Type = {...}` | ✅ 型構築子 |
> | `List = {...}` | ❌ HM 推論により関数として扱われ、型ではない |
...
`Type` は言語レベルの特殊存在であり、コンパイラは本能的にシグネチャ内の `Type` 位置を識別し、実引数の型から自動推論で填充します。

```yaoxiang
# コンパイラは自動推論でジェネリックパラメータを填充
...
| **Dup** | 浅いコピー：ハンドラ/トークンをコピー、基底データは共有 | 代入/引数渡しが自動 | `&T` トークン、`ref T`、String/Bytes |
| **Clone** | 深いコピー：完全に独立したコピーを作成 | `value.clone()` | Clone を実装する任意の型 |

**Dup のセマンティクス**：Dup を実装する型は、代入/引数渡しの際に所有権を转移しません——コンパイラはハンドラ/トークンをコピーし、複数の所有者が同じ基底データを指します。これは RFC-009 所有権モデルの Move デフォルトセマンティクスの相補です。
...
```
1. 原語値型（Int, Float, Bool, Char） — コンパイラの組み込み値コピー、Dup に属さない
2. Dup  — 参照/トークン型と内部参照カウントの型のみに適用
...
| 型 | Dup | 理由 |
|------|-----|------|
| `&T`（借用トークン） | ✅ | ゼロサイズトークン、トークンコピー = 複数視点が同じデータを指す |
| `ref T` | ✅ | Rc/Arc コピー = 参照カウント+1、ヒープデータを共有 |
| String, Bytes | ✅ | 内部参照カウント、ハンドラコピーにより基底 buffer を共有 |
...
**Int/Float/Bool/Char は Dup ではありません**——これらは値型であり、代入時にコンパイラが自動値コピーを行います（2つの値が完全に独立）。これは「浅いコピー」ではなく、コンパイラの原語に対する組み込み処理であり、Dup 型属性で表現する必要もすべきもありません。

```yaoxiang
# 原語値型：コンパイラが自動値コピー（Dup ではない）
x: Int = 42
...
> **注意**：`Send`/`Sync` はユーザー可視 trait ではありません。タスク間安全保障は `ref` キーワードとコンパイラの全自动処理により保障されます——`ref` は Rc または Arc を自動選択し、ユーザーは Send/Sync を理解する必要がありません。
...
**コア設計**：ジェネリックシグネチャ内の `Type` はコンパイル時型パラメータをマークし、`Int` などの値パラメータはジェネリックコンテキストではデフォルトでコンパイル時に確定可能です。`const` キーワードは不要です。
...
# - C が True のとき、Void に推論
# - C が False のとき、compile_error("Assertion failed") に推論
...
```yaoxiang
# 小配列最適化：関数オーバーロードを使用してコンパイル時ジェネリック特殊化を実装
...
> **Curry-Howard 同形**：条件型は Curry-Howard 視点から 보면、論理における **case 分析** です。`Bool` 型は2つの可能な値を持つ命題（True/False）に対応し、`If` はその命題の真偽に応じて異なる結果を選びます——これは論理における case 選言そのものです。`match C { True => T, False => E }` は实际上、「命題 C が True のとき結論は T であり、C が False のとき結論は E である」と表現しています。
...
> **Curry-Howard 同形**：型族は「命題即ち型」の最も直接的な体現です。`Add: (A: Type, B: Type) -> Type` は「型レベルで加法関数を記述した」ではなく、**自然数加法に関する命題を構成しています**。`(Zero, B) => B` は「命題 Add(Zero, B) は B と同値である」を意味し、`(Succ(A'), B) => Succ(Add(A', B))` は「Add(A', B) が成立するなら、Add(Succ(A'), B) も成立する」を意味しています。これは Peano 公理における加法定義そのものです。型チェッカーがこの match 式を検証することは、この定義の論理的整合性を検証することと同値です。
...
# 型レベル加算（Curry-Howard：これも自然数加算の帰納定義）
...
```yaoxiang
# RFC-010 構文に完全に準拠した特殊化方式：関数オーバーロード
...
**コア特性**：関数オーバーロードとインライン最適化は自然に組み合わせ、ゼロコスト抽象を実現します。
...
- ✅ **コード再利用** → 1つの関数名、複数実装
- ✅ **ゼロコスト抽象** → コンパイル時多态化、実行時オーバーヘッドなし
...
2. **デッドコード排除**
   - コンパイル時分析、使用されているジェネリックのみインスタンス化
   - コード膨胀が制御可能
...
| 方案 | なぜ選択しなかったか |
|------|--------------|
| 基本ジェネリアスのみ | 複雑なマクロを代替できない |
| 純粋マクロシステム | 型安全性なし、エラー情報が悪い |
| 制約のみ依存 | 柔軟性が不十分 |
| 実行時ジェネリック | パフォーマンスオーバーヘッドあり |
...
| 議題 | 説明 | ステータス |
|------|------|------|
| インスタンス化戦略 | Eager vs Lazy vs Threshold | 审议待ち |
| キャッシュサイズ | LRU キャッシュ容量設定 | 审议待ち |
| エラー診断 | ジェネリックエラー情報の詳細度 | 审议待ち |
...
# ジェネリックパラメータ内の Type はコンパイラが実引数の型から自動填充
...
- [デッドコード排除](https://en.wikipedia.org/wiki/Dead_code_elimination)