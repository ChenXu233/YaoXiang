...
author: "晨煦"
created: "2025-01-25"
updated: "2026-04-22（Type自己記述メカニズムに更新、ジェネリクス呼び出し構文を統一）"
issue: "#128"
issues_impl:
  - "#45"
  - "#46"
...
本文書はYaoXiang言語の**ジェネリクスシステム設計**を定義する。強力なジェネリクス能力によってゼロコスト抽象を実現し、コンパイル時最適化を利用してマクロへの依存を削減し、デッドコード除去機構を提供する。

**核心設計**:
- **統一シグネチャ構文**: `(T: Type, R: Type) -> ...` ジェネリクスパラメータと通常パラメータの統一
- **Type自己記述メカニズム**: `Type` は言語レベルの特別な存在であり、シグネチャ内の`Type`位置は自動的に推論・充填される
...
| [RFC-001: 並作モデル](./accepted/001-concurrent-model-error-handling.md) | **実行モデル** | DAG解析とジェネリクス型検査 |
| [RFC-008: ランタイムモデル](./accepted/008-runtime-concurrency-model.md) | **コンパイラアーキテクチャ** | ジェネリクス単相化とコンパイル時最適化戦略 |
| [型ユニバース思想](../reference/plan/ongoing/型ユニバース思想.md) | **理論核心** | 型ユニバース階層モデルと値依存型設計 |
| [RFC-022: ホーア論理静的検証](./draft/022-hol-logic-verification.md) | **停止性検査** | decreases 仕様とコンパイル時評価安全保証 |

## 型ユニバース思想と値依存型

YaoXiangのジェネリクスシステムは**型ユニバース思想**の上に構築されている。このメンタルモデルは言語のすべての概念を階層構造として統一するもので、核心となる革新は**値依存型**を Type2 層の一級市民に昇格させたことにある。

### 値依存型とは？

**値依存型**は、一つ以上の**値**（他の型だけではなく）に依存する型である。これらの値はコンパイル時に評価され、コンパイル段階で型安全保証を提供できる。
...
伝統的なジェネリクスと比較して、YaoXiangの値依存型には以下の核心的な優位性がある:

| 特性 | 伝統的なジェネリクス (C++/Rust) | YaoXiang値依存型 |
|------|-------------------------------|-----------------|
| 型が依存する値 | 型パラメータのみ | 任意の値（関数呼び出し結果を含む）に依存可能 |
| コンパイル時評価 | C++テンプレートは手動特殊化、Rustは非対応 | 自動コンパイル時評価、停止性を保証 |
| 型レベル計算 | テンプレートメタプログラミング（複雑/危険） | 統一された型レベル計算エンジン |
| 型安全性 | C++はなし、Rustは制限付き | 完全な型安全性、コンパイル時チェック |
| 次元検証 | 実行時チェックまたは手動特殊化 | コンパイル時次元検証、実行時オーバーヘッドなし |

### 型ユニバース階層と値依存型

型ユニバース思想は言語概念を意味的役割によって異なる階層に分割する。値依存型は**Type2 層**に位置する:
...
**重要な設計**: Type2層の関数、型コンストラクタ、値依存型は**構文が統一**されており、いずれも`(params) -> result`の形式である:
- 通常関数: `(Int, Int) -> Int` → 戻り値は値
- 型コンストラクタ: `(T: Type) -> Type` → 戻り値は型
- 値依存型: `(n: Int) -> Type` → 戻り値は型だが、値パラメータに依存する

> **Curry-Howard対応**: この統一は偶然ではない。Curry-Howard対応は「型は命題、プログラムは証明」であることを指摘する——関数型 `A → B` は論理的含意「A ならば B」に対応し、ジェネリクス `(T: Type) -> Type` は全称量化「すべての型 T に対して」に対応し、値依存型 `(n: Int) -> Type` は「各整数 n に対して型が存在する」に対応する。YaoXiangは関数、型コンストラクタ、値依存型をType2層に統一することで、本質的に「証明」と「計算」を**構成的証明**という同一概念に統一している。これがまさにCurry-Howard対応の言語設計における直接的な現れである: 一つの形式 `(params) -> result` が論理的命題と計算過程を同時に担う。
...
コンパイラは自動的に以下を行う:
1. 型位置での関数呼び出しを検出する
2. 関数が`decreases`仕様（後述の停止性検査メカニズムを参照）でマークされているかを検証する
3. コンパイル時に評価を実行する
4. 結果を生成された型に埋め込む
...
| 停止性保証 | ❌ | ❌ | ❌（危険） | ✅（decreases仕様） |
| 型安全性 | ❌（マクロ展開） | ✅ | ✅ | ✅ |
| 統一構文 | ❌ | ❌ | ❌ | ✅ |
| コンパイル時次元検証 | 手動特殊化 | 実行時チェック | 型族 | コンパイル時自動検証 |
| decreases仕様 | ❌ | ❌ | ❌ | ✅ |

### 停止性検査メカニズム（RFC-022との統合）

値依存型のコンパイル時評価は**停止性を保証**しなければならない。さもなければ型システムが無限ループに陥る。YaoXiangは **decreases仕様** によってこれを保証し、RFC-022とシームレスに統合する。
...
- **統一性**: 停止性検査と部分正当性検証が同一の仕様機構を共有
- **段階的強化**: 実行時チェックから完全静的証明へと漸進的に移行可能
...
| C# | ジェネリクス制約 | 実行時型チェック、パフォーマンスオーバーヘッドあり |
...
1. **パフォーマンス vs 柔軟性**: 実行時の柔軟性 vs コンパイル時最適化
...
YaoXiangの**値依存型**は伝統的なジェネリクスに対する核心的な優位性である:

| 優位性 | 説明 |
|------|------|
| **型が値に依存** | `Vec: (n: Int) -> Type` によって型が具体的な値に依存できる |
...
# ジェネリクスアプローチ: 一つのジェネリクス関数で全型をカバー
...
5. **IDE対応** - スマートヒント、明確なエラー情報
6. **値依存型** - 型が値に依存可能、コンパイル時次元検証をサポート
7. **コンパイル時評価の安全性** - decreases 仕様によってコンパイル時評価の停止を保証

### 設計原則

- **コンパイル時決定**: ジェネリクスパラメータはコンパイル時に決定
- **単相化優先**: 具象コードを生成、仮想関数呼び出しを回避
- **制約駆動**: 型制約がインスタンス化を指導
- **プラットフォーム最適化**: プラットフォーム固有最適化の特殊化をサポート
- **型ユニバース統一**: 関数/型コンストラクタ/値依存型をType2層に統一
- **停止性保証**: 型位置の関数呼び出しは停止性を証明しなければならない
...
> **重要なルール**: ジェネリクスタイプ定義は**明示的に `: Type` を标注しなければならない**。さもないと HM 推論により関数として扱われる。
>
> | 書き方 | 意味 |
> |------|------|
> | `List: (T: Type) -> Type = {...}` | ✅ 型コンストラクタ |
> | `List = {...}` | ❌ HM 推論により関数になり、型ではない |
...
# ジェネリクス関数（: Type なし、HM 推論により関数になる）
...
# ジェネリクス制約（直接式、単一行では return 省略可能）
...
`Type` は言語レベルの特別な存在であり、コンパイラはシグネチャ内の `Type` 位置を自然に認識し、実引数型から自動的に推論・充填する。

```yaoxiang
# コンパイラがジェネリクスパラメータを自動推論
numbers: List(Int) = List(Int)
#         ^^^^^^^^   ^^^^^^
#         型宣言      コンストラクタ呼び出し: Int が T に充填
...
| **Clone** | ディープコピー: 完全な独立副本を作成 | `value.clone()` | Clone を実装するすべての型 |

**Dup のセマンティクス**: Dup を実装した型は、代入/引数渡し時に所有権を移転しない——コンパイラがハンドル/トークンをコピーし、複数の所有者が同じ基底データを指す。これは RFC-009 所有権モデルのデフォルト Move セマンティクスの補完関係にある。

**Dup と Clone は直交する概念**:

```
Dup = ハンドルをコピー、データを共有（変更が相互に影響する）
Clone = データをコピー、副本は独立（変更が相互に影響しない）
```
...
| String, Bytes | ✅ | 内部参照カウント、ハンドルコピー = 基底バッファを共有 |
| `&mut T`（可変トークン） | ❌ | 線形排他、コピー不可 |
| struct | 派生 | 全フィールドが Dup → struct も Dup |
| enum | 派生 | 全 variant の全フィールドが Dup → enum も Dup |
| tuple | 派生 | 全要素が Dup → tuple も Dup |
| Fn（クロージャ） | ❌ | 捕捉環境が Dup でない可能性あり |
| `*T`（生ポインタ） | ❌ | unsafe、所有権システムに参加しない |

**Int/Float/Bool/Char は Dup ではない**——これらは値型であり、代入時にコンパイラが自動で値コピーする（二つの値は完全に独立）。これは「シャローコピー」ではなく、プリミティブに対するコンパイラの組み込み処理であり、Dup 型属性で表現すべきものではない。
...
> **注意**: `Send`/`Sync` はユーザー可視 trait とはしない。タスク横断安全保証は `ref` キーワードとコンパイラの全自動処理によって行われる——`ref` は自動で Rc または Arc を選択し、ユーザーは Send/Sync を理解する必要がない。
...
**核心設計**: ジェネリクスシグネチャ内の `Type` マーカーはコンパイル時型パラメータを示し、`Int` などの値パラメータはジェネリクスコンテキストでデフォルトでコンパイル時に決定可能。`const` キーワードは不要。
...
StaticArray: (T: Type, N: Int) -> Type = {
    data: Array(T, N),  # コンパイル時に既知サイズの配列
    length: N,
}
...
> **Curry-Howard対応**: 条件型は Curry-Howard の視点から論理の **ケース分析** である。`Bool` 型は二つの可能な値（True/False）を持つ命題に対応し、`If` はその命題の真偽に応じて異なる結果を選択する——これはまさに論理におけるケース分解そのものである。`match C { True => T, False => E }` は実際には「命題 C が True のとき結論は T、False のとき結論は E」を表現している。
...
> **Curry-Howard対応**: 型族は「命題即型」の最も直接的な体現である。`Add: (A: Type, B: Type) -> Type` は「型レベルで加算関数を書いた」のではなく、**自然数加算に関する命題を構成している**。`(Zero, B) => B` は「命題 Add(Zero, B) は B と等価である」といい、`(Succ(A'), B) => Succ(Add(A', B))` は「Add(A', B) が成立するなら、Add(Succ(A'), B) も成立する」という。これは Peano 公理における加算定義そのもの。型検査器がこの match 式の通過を検証することは、この定義の論理的一貫性を検証することと等価である。
...
# 型レベル加算（Curry-Howard: これは自然数加算の帰納的定義でもある）
...
```yaoxiang
# RFC-010 構文に完全準拠した特殊化方法: 関数オーバーロード
...
1. **コンパイラのスマート選択**
   ```yaoxiang
   sum(int_arr)      # 自動で sum: (Array(Int)) -> Int を選択
   sum(float_arr)    # 自動で sum: (Array(Float)) -> Float を選択
   sum(custom_arr)   # 自動で sum: (T: Type) -> ((arr: Array(T)) -> T) を選択
   ```

2. **インライン化最適化**
   - 小さな関数は自動で呼び出し点にインライン化
   - 関数呼び出しオーバーヘッドゼロ
   - 手書き最適化コードと完全に等価
...
# 使用
p = Point { x: 1, y: 2 }
p.debug_fmt(&formatter)  # 呼び出しが自動生成される
```
...
List: (T: Type) -> Type = {
    ...
    # ジェネリクスメソッド（T は外側の List(T) から自動でスコープに導入）
...
# ======== 2. ジェネリクスメソッドを実装 ========
# 関数は List 名前空間の下に定義（List. プレフィックス = 名前空間所属）
...
```bnf
# ジェネリクスパラメータは統一された () 構文を使用し、関数型の一部
