...
status: '承認済み'
author: '晨煦'
updated: '2026-07-15（型本体コードブロック + コンパイル時仕様 + エフェクト種子が実装済み）'
issue: '#128'
issues_impl:
  - '#45'
  - '#46'
...
本ドキュメントでは、YaoXiang言語の**ジェネリクスシステム設計**を定義します。強力なジェネリクス機能によりゼロコスト抽象を実現し、コンパイル時最適化によってマクロへの依存を削減し、死コード除去機構を提供します。

**核心設計**：

- **統一シグネチャ構文**：`(T: Type, R: Type) -> ...` ジェネリックパラメータと通常パラメータの統一
- **Type自己記述機構**：`Type` は言語レベルの特別な存在であり、シグネチャ内の `Type`
  の位置は自動的に推論・充填される
...
- ゼロコスト抽象：コンパイル時モノモーフィゼーション、実行時オーバーヘッドなし
...
| ドキュメント                                                                      | 関係                         | 説明                                                                   |
| --------------------------------------------------------------------------------- | ---------------------------- | ---------------------------------------------------------------------- |
| [RFC-010: 統一型構文](./010-unified-type-syntax.md)                               | **構文基礎**                 | ジェネリック構文と統一 `name: type = value` モデルの統合               |
| [RFC-010: 統一型構文](./010-unified-type-syntax.md)                               | **呼び出し構文**             | 第6節：ジェネリック呼び出し構文——統一された `()` 適用、`[]` の完全除去 |
| [RFC-009: 所有権モデル](./accepted/009-ownership-model.md)                        | **型システム**               | Moveセマンティクスとジェネリクスの自然な結合                           |
| [RFC-024: spawnベースの並行ランタイムセマンティクス](./024-concurrency-model.md)  | **実行モデル**               | DAG解析とジェネリック型チェック                                        |
| [RFC-008: ランタイムモデル](./accepted/008-runtime-concurrency-model.md)          | **コンパイラアーキテクチャ** | ジェネリクスモノモーフィゼーションとコンパイル時最適化戦略             |
...
YaoXiangのジェネリクスシステムは**型宇宙思想**の上に構築されています。このメンタルモデルでは、言語内のすべての概念を階層構造として統一し、核心となる革新は**値依存型**をType2層のファーストクラス市民として昇格させることです。

### 値依存型とは何か？

**値依存型**とは、一つ以上の**値**（他の型だけでなく）に依存する型です。これらの値はコンパイル時に評価され、コンパイル段階で型安全保証を提供します。
...
従来のジェネリクスと比較して、YaoXiangの値依存型には以下の核心的な利点があります：

| 特性             | 従来のジェネリクス (C++/Rust)               | YaoXiang 値依存型                              |
| ---------------- | ------------------------------------------- | ---------------------------------------------- |
| 型が依存する値   | 型パラメータのみ                            | 関数呼び出し結果を含む任意の値                 |
| コンパイル時評価 | C++テンプレートの手動特殊化、Rust なし      | 自動コンパイル時評価、停止性を保証             |
| 型レベル計算     | テンプレートメタプログラミング（複雑/危険） | 統一された型レベル計算エンジン                 |
| 型安全性         | C++なし、Rust限定                           | 完全な型安全性、コンパイル時チェック           |
| 次元検証         | 実行時チェックまたは手動特殊化              | コンパイル時次元検証、実行時オーバーヘッドなし |

### 型宇宙階層と値依存型

型宇宙思想は、言語概念を意味的役割によって異なる階層に分割し、値依存型は **Type2層** に位置します：
...
> **Curry-Howard同型**：この統一は偶然ではありません。Curry-Howard同型は「型は命題、プログラムは証明」であると指摘しています——関数型
> `A → B` は論理的含意「A ならば B」に対応し、ジェネリクス `(T: Type) -> Type`
> は全称量化「すべての型 T について」に対応し、値依存型 `(n: Int) -> Type`
> は「各整数 n について型が存在する」に対応します。YaoXiangは関数、型コンストラクタ、値依存型をType2層に統一することで、本質的に「証明」と「計算」を**構成的証明**という同一の概念に統一しています。これがまさに、言語設計におけるCurry-Howard同型の直接的な具現化です：一つの形式（`(params) -> result`）が論理的命題と計算プロセスの両方を同時に担います。

### コンパイル時決定性の保証

YaoXiangの型宇宙思想は、**Type階層のすべてがコンパイル時に決定される**ことを要求します。
...
# 使用時は完全に透過的、型は自動推論
numbers = List(Int)()   # 訂正：値構築の二層形式（§9.1を参照）；要素は push で埋める
numbers.push(1)
numbers.push(2)
numbers.push(3)
...
| 型安全性                                          | ❌（マクロ展開）           | ✅               | ✅           | ✅                           |
| 統一構文                                          | ❌                         | ❌               | ❌           | ✅                           |
| コンパイル時次元検証                              | 手動特殊化                 | 実行時チェック   | 型族         | コンパイル時自動検証         |
| 半自動停止性アノテーション（decreases/invariant） | ❌                         | ❌               | ❌           | ❌（コンパイル時全自动のみ） |

### 停止性検査機構（RFC-027と統一）

値依存型のコンパイル時評価は**停止性を保証**しなければならず、そうでなければ型システムが無限ループに陥ります。停止性検査はRFC-027のコンパイル時証明パイプラインによって**全自动**で実行されます——コンパイラが自動的に度量を合成し、証明可能な再帰/ループは通過し、証明不可能なものは直接コンパイルエラーを報告します。**半自動アノテーションの余地は与えません**：RFC-022の
`//! decreases`、`/*! invariant !*/`
はRFC-022の廃止と共に廃止され、仕様本身就是型アノテーションそのものです。

#### 再帰関数の停止性検査

コンパイラはコンパイル時評価の前に、再帰呼び出しの引数がすべての再帰パスで厳密に減少しているかを検査します（RFC-027
§6.7）。仕様コメントは一切不要です：
...
| シーン                                          | 動作             |
| ----------------------------------------------- | ---------------- |
| コンパイラが再帰の減少を解析できる（例：`n-1`） | コンパイル時評価 |
| 減少しない/減少の判定不能                       | コンパイルエラー |
| 実行時呼び出し（非型位置）                      | 停止性検査不要   |

#### ループの停止性検査

ループは `: Invariant(...)` や `: decreases(...)`
アノテーションを必要としません。変数の精化型アノテーション（例：`UpTo(n)`）がループ不変式と度量境界の両方を提供し、コンパイラは優先順位に従って4つの度量合成戦略を試し、見つかれば停止します（RFC-027
§7）：

1. **線形ランク関数の自動合成**——型アノテーションから変数の境界を抽出し、線形結合を列挙、SMT が m ≥
   0 とすべてのパスで m' < m を検証
2. **述語違反カウント**（実験的）——目標型定義（例：`Sorted`）から violation_count を抽出し、隣接交換/移動をカバー
3. **有界増減パターン**——`v += const` → 度量 `upper - v`（戦略1の縮退、最速パス）
4. **乗法スケール度量テンプレート**——`v *= const`（const > 1）→ 度量 `ceil(log_const(upper / v))`

```yaoxiang
sum: (arr: Array(Int, n)) -> Int = {
    mut i: UpTo(arr.len) = 0   # 型アノテーションが上界 arr.len と下界 0 を与える
    while i < arr.len {
        # コンパイラ自動推論：度量 arr.len - i、各反復で厳密に1減少 → 停止が証明される
        s += arr[i]; i += 1
    }
    return s
}
...
└─────────────────────────┬───────────────────────────────────┘
                          ▼
┌─────────────────────────────────────────────────────────────┐
│  1. 停止性検査（RFC-027 証明パイプライン、全自动）          │
│     - 再帰関数：引数がすべての再帰パスで厳密に減少するか検査│
...
- **統一性**：停止性検査と正当性検証（VC生成）が同一のコンパイル時証明パイプライン（RFC-027）を共有、独立した仕様構文なし
- **全自动**：コンパイラが型アノテーションから自動的に度量を合成し、証明できれば通過、できなければエラー——プログラマの手書き
  `decreases` に依存しない
...
| 言語         | ジェネリクス能力     | 問題                                                       |
| ------------ | -------------------- | ---------------------------------------------------------- |
| Java         | 境界型               | コンパイル時モノモーフィゼーション、ジェネリクス特殊化なし |
| C#           | ジェネリック制約     | 実行時型チェック、パフォーマンスオーバーヘッドあり         |
...
1. **性能 vs 柔軟性**：実行時柔軟性 vs コンパイル時最適化
...
YaoXiangの**値依存型**は従来のジェネリクスに対する核心的優位性です：
...
| **停止性保証**       | コンパイル時停止性検査（自動度量合成）がコンパイル時評価の終了を保証       |
...
# ジェネリクスアプローチ：1つのジェネリック関数ですべての型をカバー
...
2. **死コード除去** - コンパイル時解析、使用されるジェネリクスのみをインスタンス化
3. **マクロ代替** - ジェネリクスが90%のマクロ使用シーンを代替
4. **型安全性** - コンパイル時チェック、実行時型オーバーヘッドなし
...
> を标注しなければならない**。さもないとHMによって関数として推論される。
>
> | 記述                              | 意味                              |
> | --------------------------------- | --------------------------------- |
...
#### 2.4 組み込みmarker trait：Dup と Clone

**3種類のコピーセマンティクス**：

| 型                       | 意味                                                        | 起動方式            | 適用シーン                           |
| ------------------------ | ----------------------------------------------------------- | ------------------- | ------------------------------------ |
| **プリミティブ値コピー** | 代入時に自動的に値コピーされ、2つの値は完全に独立           | 代入/引数渡しで自動 | Int, Float, Bool, Char               |
...
**Dup のセマンティクス**：Dup を実装した型は、代入/引数渡し時に所有権を移動しない——コンパイラがハンドル/トークンをコピーし、複数の保持者が同じ基底データを指します。これはRFC-009所有権モデルにおけるMoveデフォルトセマンティクスの補完関係です。

**Dup と Clone は直交する概念である**：

```
Dup = ハンドルをコピー、データを共有（変更が相互に影響する）
Clone = データをコピー、複製は独立（変更が相互に影響しない）
```
...
| String, Bytes            | ✅   | 内部参照カウント、ハンドルをコピーして基底バッファを共有          |
...
**Int/Float/Bool/Char は Dup ではない**——これらは値型であり、代入時にコンパイラが自動的に値コピーします（2つの値は完全に独立）。これは「シャローコピー」ではなく、プリミティブに対するコンパイラ組み込み処理であり、Dup 型属性で表現する必要はなく、表現すべきでもありません。
...
# Dup：シャローコピー、ハンドルをコピーしてデータを共有
view: &Point = &point
view2 = view    # ✅ Dup：トークンをコピー、両者は同じ point を指す
print(view.x)   # ✅
...
> **注意**：`Send`/`Sync` はユーザー可視の trait とはしません。タスク横断の安全保証は `ref`
> キーワードとコンパイラ全自动処理によって行われます——`ref`
> は自動的に Rc または Arc を選択し、ユーザーは Send/Sync を理解する必要がありません。
...
**核心的設計**：ジェネリックシグネチャ内の `Type` マーカーはコンパイル時型パラメータを示し、`Int`
などの値パラメータはジェネリックコンテキストではデフォルトでコンパイル時に決定可能です。`const`
キーワードは不要です。

```yaoxiang
# ════════════════════════════════════════════════════════
# コンパイル時定数パラメータ：ジェネリクス内の Int はデフォルトでコンパイル時決定
# ════════════════════════════════════════════════════════
...
YaoXiang の型システムは、Curry-Howard同型において ⊥（偽/空型）と ⊤（真/Unit）の両方を備え、`Never`
と `Void` という2つの組み込み型名でそれを担います：

**Never（⊥）** — 交渉不可能な3つの内核的性質：

1. **ゼロコンストラクタ**：`Never`
   型の値を生成するリテラルや式は一切ありません。これはメタレベルの性質であり、組み込みでなければなりません。
2. **爆発原理**：`Never <: T` は任意の型 `T` に対して成立します。`Never`
   値は任意の型として使用できます——これが `assert(false)`
   以降のコードが型チェックを通る理由です（決して実行されませんが）。
3. **発散マーカー**：`f: (...) -> Never` は `f`
   が返らないことを保証します。コンパイラはこれに基づいて dead code 解析を行います。

`Never`
は組み込み型名でありキーワードではないため、parser は感知しません。空和型リテラル構文は開放しません。

**Void（⊤、すなわち Unit）**
— 唯一の居住者（デフォルト void 値）を持ち、真命題「恒真」の担い手です。`Void`
はゼロフィールドの積型の単位元であり、`Never`
はゼロバリアントの和型の単位元——両者は双対です。`x: Void = <デフォルト>`
は合法であり、`x: Never = ...` は右辺に何も書けません。
...
> **Curry-Howard同型**：条件型をCurry-Howardの視点で見ると、論理における**ケース分析**です。`Bool`
> 型は2つの可能な値（True/False）を持つ命題に対応し、`If`
> はその命題の真偽に応じて異なる結果を選択します——これはまさに論理におけるケースの選言です。`match C { True => T, False => E }`
> は実際には「命題 C が True のとき結論は T、C が False のとき結論は E」ということを表現しています。
...
> **Curry-Howard同型**：型族は「命題即型」の最も直接的な具現化です。`Add: (A: Type, B: Type) -> Type`
> は「型の層に加法関数を書いた」のではなく、**自然数の加法に関する命題を構成している**のです。`(Zero, B) => B`
> は「命題 Add(Zero, B) は B と等価である」と述べ、`(Succ(A'), B) => Succ(Add(A', B))` は「Add(A',
> B) が成立するなら Add(Succ(A'),
> B) も成立する」と述べています。これがまさに Peano 公理における加法定義そのものです。型チェッカーがこの match 式が通過することを検証することは、この定義の論理的一貫性を検証することと等価です。
...
# 型レベル加算（Curry-Howard：ケース分析 + 再帰呼び出し、完全な帰納のためには停止性検査が必要）
...
**重要な特性**：関数オーバーロードとインライン最適化は自然に結合し、ゼロコスト抽象を実現します。
...
fibonacci(10.5)    # Float版を選択、Binetの公式を使用
```
...
# 訂正：初期バージョンでは Array(Int, 10)(1, 2, 3, ...)（要素を直接展開）と記述していたが、
# §9.3 の権威パターン（Type(パラメータ)(フィールド構築パラメータ)/空構築）と一致しないため、
# フィールド名式の構築パラメータに統一（SPEC type-system.md §4.3 参照）。
arr_100_int = Array(Int, 100)()   # 空構築、データは後で代入

# コンパイル後は使用されるSizeのみが生成される
...
# - コンパイル後のバイナリにはこれら2つのインスタンスのみが含まれる
```
...
# 使用
p = Point { x: 1, y: 2 }
p.debug_fmt(&formatter)  # 自動生成された呼び出し
```
...
List: (T: Type) -> Type = {
    ...
    # ジェネリックメソッド（T は外側の List(T) から自動的にスコープに取り込まれる）
...
# ======== 2. ジェネリックメソッドの実装 ========
# 関数定義は List 名前空間配下（List. プレフィックス = 名前空間所属）
...
1. **ゼロコスト抽象**
   - コンパイル時モノモーフィゼーション、実行時オーバーヘッドなし
   - 仮想関数不要、RTTI 不要
...
| リスク           | 影響                 | 緩和策                            |
| ---------------- | -------------------- | --------------------------------- |
| 制約解決の複雑さ | コンパイル時間過大   | インクリメンタル解決 + キャッシュ |
...
# 型定義（統一 Binding 構文）
# ジェネリック型は List: (T: Type) -> Type = { ... } のような形式
generic_type ::= identifier ':' type '=' type_expression

# ジェネリクスパラメータ内の Type はコンパイラが実引数型から自動充填
# 例：map(numbers, f) では、T は numbers: List(Int) から、R は f: (Int) -> String から抽出される
```
...
- [モノモーフィゼーション最適化](https://llvm.org/docs/Monomorphization.html)
