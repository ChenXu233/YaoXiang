...
> **著者**: 晨煦
> **作成日**: 2025-01-25
> **最終更新**: 2026-04-22（Type 自己記述機構を更新し、泛型呼び出し構文を統一）

## 概要

このドキュメントはYaoXiang言語の**泛型システム設計**を定義し、強力な泛型能力を通じてゼロコスト抽象化を実現し、编译期最適化によりマクロへの依存を軽減し、デッドコード消除機構を提供します。

**コア設計**：
- **統一署名構文**: `(T: Type, R: Type) -> ...` 泛型パラメータと通常パラメータの統一
- **Type 自己記述機構**: `Type` は言語レベルの特殊な存在であり、署名内の `Type` 位置は自動推論により填充される
- **型制約**: `T: Dup + Add` 複数制約、関数型制約
- **関連型**: `Iterator: (Item: Type) -> Type = { next: () -> Option(Item), has_next: () -> Bool }`
- **编译期泛型**: `N: Int` 泛型値パラメータ、编译期定数インスタンス化
- **条件型**: `If: (C: Bool, T: Type, E: Type) -> Type` 型レベル計算、型族

**価値**：
- ゼロコスト抽象化：编译期モノモーフィゼーション、运行时オーバーヘッドなし
- デッドコード消除：インスタンス化グラフ分析 + LLVM最適化
- マクロ代替：泛型により90%の宏使用シナリオを代替
- 型安全性：编译期チェック、IDEフレンドリー
- **明示的优于隐式**: `Type` 自己記述、コンパイラ自動推論

## 参考ドキュメント

このドキュメントの設計は以下のドキュメントに基づいています：

| ドキュメント | 関係 | 説明 |
|------|------|------|
| [RFC-010: 統一型構文](./010-unified-type-syntax.md) | **構文基盤** | 泛型構文と統一 `name: type = value` モデルの統合 |
| [RFC-010: 統一型構文](./010-unified-type-syntax.md) | **呼び出し構文** | 第6節：泛型呼び出し構文——統一 `()` 適用、`[]` の完全移除 |
...
| [RFC-008: 実行時モデル](./accepted/008-runtime-concurrency-model.md) | **コンパイラアーキテクチャ** | 泛型モノモーフィゼーションと编译期最適化戦略 |
| [型宇宙思想](../reference/plan/ongoing/タイプ宇宙思想.md) | **理論コア** | 型宇宙階層モデルと値依存型設計 |
| [RFC-022: 霍尔論理静的検証](./draft/022-hol-logic-verification.md) | **終了チェック** | decreases規定と编译期評価安全保障 |

## 型宇宙思想と値依存型

YaoXiangの泛型システムは**型宇宙思想**に基づいており、このメンタルモデルは言語内のすべての概念を階層構造に統一し、核心的な革新点は**値依存型**をType2層の一級市民として昇格させたことです。

### 値依存型とは？

**値依存型**とは、1つまたは複数の**値**に依存する型です（他の型にのみ依存するのではなく）。これらの値は编译期に評価でき thereby 型安全性を编译段階で提供します。
...
従来の泛型と比較して、YaoXiangの値依存型は以下のコア優位性を持ちます：

| 特性 | 従来の泛型 (C++/Rust) | YaoXiang 値依存型 |
|------|-------------------|---------------------|
| 型が依存する値 | 型パラメータのみに依存 | 関数呼び出し結果を含む任意の値に依存可能 |
| 编译期評価 | C++テンプレート手動特化、Rustなし | 自動编译期評価、終了保証 |
| 型レベル計算 | テンプレートメタプログラミング（複雑/危険） | 統一された型レベル計算エンジン |
| 型安全性 | C++なし、Rust制限付き | 完全な型安全性、编译期チェック |
| 次元検証 | 运行时チェックまたは手動特化 | 编译期次元検証、オーバーヘッドなし |

### 型宇宙階層と値依存型

型宇宙思想は言語概念をセマンティックロール別に異なる階層に分類し、値依存型は **Type2層** に位置します：
...
**重要設計**: Type2層の関数、型構築子、値依存型は**統一式語**であり、すべて `(params) -> result` の形式です：
- 通常関数：`(Int, Int) -> Int` → 返り値は値
- 型構築子：`(T: Type) -> Type` → 返り値は型
- 値依存型：`(n: Int) -> Type` → 返り値は型だが、値パラメータに依存

> **Curry-Howard 同型**: この統一は偶然ではありません。Curry-Howard 同型は「型即ち命題、プログラム即ち証明」を指摘します——関数型 `A → B` は論理蕴含「AならばB」に対応し、泛型 `(T: Type) -> Type` は全称量化「すべての型Tについて」に対応し、値依存型 `(n: Int) -> Type` は「各整数nに対して型が存在する」に対応します。YaoXiangは関数、型構築子、値依存型をType2層に統一しますが、本質的には「証明」と「計算」を同一の概念——**構成的証明**——に統一することです。これはCurry-Howard同型の言語設計への直接的な反映です：1つの形式（`(params) -> result`）が論理命題と計算過程を同時に担います。

### 编译期決定性保証

YaoXiangの型宇宙思想は要求します：**Type階層はすべて编译期に決定される**。

```yaoxiang
# 编译期次元検証の例
Matrix: (T: Type, Rows: Int, Cols: Int) -> Type = {
    data: Array(Array(T, Cols), Rows),
    # 编译期チェック：次元は正でなければならない
...
# 编译期計算：factorial(3) = 6、ベクトルサイズは编译期に確定
vec: Vec(factorial(3)) = Vec(6)()
```

コンパイラは自動的に：
1. 型位置の関数呼び出しを検出
2. 関数が `decreases` 規定でマークされているか検証（以下参照）
3. 编译期に評価を実行
4. 結果を生成された型に埋め込み
...
multiply: (T: Add + Multiply + Zero,
    ...
    # 编译期チェック：a.Cols == b.Rows，否则编译エラー
    result = Matrix(T, Rows, M)()
    # ...
}

# エラーは编译期にキャッチ：
# multiply(matrix_2x3, matrix_4x2)  # 编译エラー：2 != 4
```
...
| 型安全性 | ❌（宏展開） | ✅ | ✅ | ✅ |
| 統一式語 | ❌ | ❌ | ❌ | ✅ |
| 编译期次元検証 | 手動特化 | 运行时チェック | 型族 | 编译期自動検証 |
| decreases規定 | ❌ | ❌ | ❌ | ✅ |

### 終了チェック機構（RFC-022との統合）

値依存型の编译期評価は**終了を保証**する必要があります、さもなくば型システムは無限ループに陥ります。YaoXiangは **decreases規定** を通じてこれを確保し、RFC-022とシームレスに統合します。
...
factorial: (n: Int) -> Int = {
    //! requires: n >= 0
    //! ensures: result == n!
    //! decreases: n    # 各再帰でnは厳密に減少
    if n <= 1 { return 1 }
    return n * factorial(n - 1)
}

# 使用：型位置で呼び出し
vec: Vec(factorial(5)) = Vec(120)()  # 编译期に factorial(5) = 120 を評価
```
...
┌─────────────────────────────────────────────────────────────┐
│  1. decreases規定のチェック                                    │
│     - decreasesあり：すべての再帰パスで減少条件が成立するか検証   │
│     - decreasesなしでも終了明白：直接評価                      │
│     - decreasesなしで終了可能性不明：编译エラー               │
└─────────────────────────┬───────────────────────────────────┘
                          ▼
┌─────────────────────────────────────────────────────────────┐
│  2. 编译期評価（組み込みインタープリタが実行）                  │
│     - 純粋関数：直接評価                                       │
│     - 副作用あり：编译エラー（型位置は副作用禁止）              │
└─────────────────────────┬───────────────────────────────────┘
                          ▼
┌─────────────────────────────────────────────────────────────┐
│  3. 結果の型への埋め込み                                       │
...
- **安全性**: 编译期評価が必ず終了し、型システムの無限ループを回避
- **統一性**: 終了チェックと部分的正確性検証は同一の規定機構を共有
- **漸進的強化**: 运行时チェックから完全静的証明へ段階的に移行可能
...
| 言語 | 泛型能力 | 問題 |
|------|----------|------|
| Java | 境界型 | 编译期モノモーフィゼーション、泛型特化なし |
| C# | 泛型制約 | 运行时型チェック、パフォーマンスオーバーヘッドあり |
| Rust | 泛型 + 特質 | 特質システムが複雑、学習コストが高い |
| C++ | テンプレート | テンプレート特化が複雑、编译エラー情報が悪い |
| **YaoXiang** | **値依存型** | **型は値に依存可能、编译期次元検証、終了保証** |

### コアトレードオフ

1. **パフォーマンス vs 柔軟性**: 运行时柔軟性 vs 编译期最適化
2. **複雑さ vs 簡潔さ**: 強力な型システム vs 使いやすさ
3. **宏 vs 泛型**: 宏コード生成 vs 泛型型安全性
4. **値依存 vs 型安全性**: 従来の泛型では编译期に次元を検証できない

### 値依存型のコア優位性

YaoXiangの**値依存型**は従来の泛型に対するコア優位性です：

| 優位性 | 説明 |
|------|------|
| **型が値に依存** | `Vec: (n: Int) -> Type` で型が具体的な値に依存可能 |
| **编译期評価** | 型位置の関数呼び出しは编译期に評価され、結果を型に直接埋め込み |
| **次元検証** | `Matrix(Float, 3, 3)` は编译期に行列次元を検証 |
| **型レベル計算** | `If`, `Match` などの条件型が型レベル計算をサポート |
| **終了保証** | decreases規定が编译期評価の終了を保証 |

```yaoxiang
# C++/Rust ではできない编译期検証
matrix: Matrix(Float, factorial(3), factorial(2)) = ...
# 编译期に計算：factorial(3) = 6, factorial(2) = 2
# 型は Matrix(Float, 6, 2)

# 次元不一致は编译期にキャッチ
identity: Matrix(Float, 3, 3) = ...
# multiply(matrix_2x3, identity_3x3)  # 编译エラー：2 != 3
```
...
2. **デッドコード消除** - 编译期分析、使用されている泛型のみをインスタンス化
3. **マクロ代替** - 泛型で90%の宏使用シナリオを代替
4. **型安全性** - 编译期チェック、运行时型オーバーヘッドなし
5. **IDEフレンドリー** - インテリセンス、明確なエラー情報
6. **値依存型** - 型は値に依存可能、编译期次元検証をサポート
7. **编译期評価安全性** - decreases規定により编译期評価の終了を保証

### 設計原則

- **编译期確定**: 泛型パラメータは编译期に確定
- **モノモーフィゼーション優先**: 具象コードを生成、仮想関数呼び出しを回避
- **制約駆動**: 型制約がインスタンス化を指導
...
> **重要規則**: 泛型型定義は**必ず `: Type` を明示的にマーク**する必要があります、さもなくばHM推論により関数として扱われます。
>
> | 書き方 | 意味 |
> |------|------|
...
```yaoxiang
# 泛型関数は統一された (T: Type, R: Type) 署名構文を使用
...
`Type` は言語レベルの特殊な存在であり、コンパイラは署名内の `Type` 位置を本能的に識別し、実引数型から自動推論により填充します。
...
**コア設計**: 泛型署名内の `Type` マークは编译期型パラメータを示し、`Int` などの値パラメータは泛型コンテキストではデフォルトで编译期に確定可能です。`const` キーワードは不要です。

```yaoxiang
# ════════════════════════════════════════════════════════
# 编译期定数パラメータ：泛型内の Int はデフォルトで编译期確定
# ════════════════════════════════════════════════════════

# 编译期階乗：N は编译期に既知の字句量でなければならない
...
StaticArray: (T: Type, N: Int) -> Type = {
    data: Array(T, N),  # 编译期に既知のサイズの配列
    length: N,
}

# 使用方法
arr: StaticArray(Int, factorial(5))  # StaticArray(Int, 120)、コンパイラが编译期に計算
```
...
# コンパイラは编译期に字句量型の関数呼び出しを計算
SIZE: Int = factorial(5)  # 编译期の値は 120
...
# 使用：编译期に計算、Matrix(Float, 3, 3) を生成
...
Array: (T: Type, N: Int) -> Type = {
    data: Array(T, N),
    # 编译期チェック：N は 0 より大きい必要がある（Assert は型位置）
    length: Assert(N > 0),
}
...
```yaoxiang
# 小配列最適化：関数オーバーロードを使用して编译期泛型特化を実装
...
> **Curry-Howard 同型**: 条件型はCurry-Howardの観点からは論理の **case分析** に相当します。`Bool` 型は2つの可能な値を持つ命題（True/False）に対応し、`If` はその命題の真偽に応じて異なる結果を選びます——これは論理におけるcase選言そのものです。`match C { True => T, False => E }` は実際には「命題 C が True のとき結論は T、C が False のとき結論は E」を表現しています。
...
> **Curry-Howard 同型**: 型族は「命題即ち型」を最も直接的に体現しています。`Add: (A: Type, B: Type) -> Type` は「型レベルで加法関数を書いた」のではなく、**自然数加算に関する命題を構成**しています。`(Zero, B) => B` は「命題 Add(Zero, B) は B と同値である」を言い、`(Succ(A'), B) => Succ(Add(A', B))` は「Add(A', B) が成立するなら、Add(Succ(A'), B) も成立する」を言います。これはPeano公理における加法の定義そのものです。型チェッカーがこのmatch式を検証することは、この定義の論理的整合性を検証することと同値です。
...
```yaoxiang
# RFC-010構文に完全準拠した特化方法：関数オーバーロード
...
#### 6.3 関数オーバーロードとインライン最適化の完璧な組み合わせ

**重要特性**: 関数オーバーロードとインライン最適化は自然に組み合わせ、ゼロコスト抽象化を実現します。
...
# ======== コンパイル後（等価コード）=======
# コンパイラは自動で最も良い特化を選択し、次にインライン化
result = native_sum_int(int_arr.data, int_arr.length)

# 手書きの最適化コードと完全等価！関数呼び出しオーバーヘッドなし！
```
...
- ✅ **泛型特化** → 関数オーバーロードで自然に解決
- ✅ **パフォーマンス最適化** → インライン化が自動完了
- ✅ **コード再利用** → 1つの関数名、複数の実装
- ✅ **ゼロコスト抽象化** → 编译期多態、运行时オーバーヘッドゼロ
- ✅ **新しいキーワード不要** → RFC-010統一式語に完全準拠
```
...
# - コンパイル後のバイナリはこれらの2つのインスタンスのみを含む
```
...
Matrix: (T: Type, Rows: Int, Cols: Int) -> Type = {
    ...
    # 编译期次元検証：Assert 標準ライブラリ型を利用
    _assert: Assert[Rows > 0],  # Rows > 0、さもなくば编译エラー
    _assert: Assert[Cols > 0],  # Cols > 0、さもなくば编译エラー
...
# 编译期検証：resultの型は Matrix(Float, 2, 2)
# 2x2 単位行列
identity_3x3 = identity(Float, 3)()

# 次元不一致：编译エラー
# bad_multiply = matrix_2x3.multiply(identity_3x3)  # 编译エラー：3x3 != 2x3
```
...
1. **ゼロコスト抽象化**
   - 编译期モノモーフィゼーション、runtimeオーバーヘッドなし
   - 仮想関数不要、RTTI不要

2. **デッドコード消除**
   - 编译期分析、使用されている泛型のみをインスタンス化
   - コード膨張は制御可能

3. **マクロ代替**
   - 型安全なコード生成
   - IDEフレンドリー、 error情報が明確
...
|------|--------------|
| 基礎泛型のみ | 複雑な宏を代替できない |
| 純粋宏システム | 型安全性がない、 error情報が悪い |
| 制約のみ依存 | 柔軟性が不足 |
| runtime泛型 | パフォーマンスオーバーヘッドあり |
...
# 型定義（統一 Binding 構文）
# 泛型型如 List: (T: Type) -> Type = { ... }
generic_type ::= identifier ':' type '=' type_expression

# 泛型パラメータの Type はコンパイラが実引数型から自動填充
