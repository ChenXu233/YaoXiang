...
本文書はYaoXiang言語の**ジェネリクスシステム設計**を定義する。強力なジェネリクス能力によってゼロコスト抽象を実現し、コンパイル時最適化によってマクロへの依存を削減し、デッドコード除去機構を提供する。
...
- **条件型**：`If: (C: Bool, T: Type, E: Type) -> Type` 型レベル計算、type family
...
| 文書                                                                              | 関係                         | 説明                                                           |
| --------------------------------------------------------------------------------- | ---------------------------- | -------------------------------------------------------------- |
| [RFC-010: 統一型構文](./010-unified-type-syntax.md)                               | **構文基礎**                 | ジェネリクス構文と統一 `name: type = value` モデル統合         |
| [RFC-010: 統一型構文](./010-unified-type-syntax.md)                               | **呼び出し構文**             | 第6節：ジェネリクス呼び出し構文——統一 `()` 適用、`[]` 完全除去 |
| [RFC-009: 所有権モデル](./accepted/009-ownership-model.md)                        | **型システム**               | Move意味論とジェネリクスの自然な結合                           |
| [RFC-024: spawn ベースの並行ランタイム意味論](./024-concurrency-model.md)         | **実行モデル**               | DAG解析とジェネリクス型検査                                    |
| [RFC-008: ランタイムモデル](./accepted/008-runtime-concurrency-model.md)          | **コンパイラアーキテクチャ** | ジェネリクス単態化とコンパイル時最適化戦略                     |
| [型ユニバース思想](../reference/plan/ongoing/类型宇宙思想.md)                     | **理論核心**                 | 型ユニバース階層モデルと値依存型設計                           |
| [RFC-027: コンパイル時述語と統一静的検証](./027-compile-time-evaluation-types.md) | **停止性検査**               | 自動度量合成とコンパイル時評価の安全保証                       |

## 型ユニバース思想と値依存型

YaoXiang のジェネリクスシステムは**型ユニバース思想**の上に築かれており、このメンタルモデルにより言語内のすべての概念を階層構造として統一する。核心となる革新は、**値依存型**を Type2 層のファーストクラス市民に引き上げたことにある。

### 値依存型とは？

**値依存型**は、一つまたは複数の**値**（他の型だけでなく）に依存する型である。これらの値はコンパイル時に評価可能であり、コンパイル段階で型安全保証を提供する。
...
従来のジェネリクスと比較して、YaoXiang の値依存型は以下の核心的優位性を持つ：
...
型ユニバース思想は言語概念を意味的役割によって異なる階層に分割する。値依存型は
**Type2 層**に位置する：
...
> は「各整数 n に対してある型が存在する」に対応する。YaoXiang は関数、型コンストラクタ、値依存型を Type2 層に統一することで、本質的に「証明」と「計算」を**構成的証明**という同一概念に統一している。これはまさに Curry-Howard 同型が言語設計において直接体現されたものである：一つの形式（`(params) -> result`）が同時に論理的命題と計算過程を担う。

### コンパイル時決定性保証

YaoXiang の型ユニバース思想は要求する：**Type 階層のすべてはコンパイル時に決定される**。
...
1. 型位置での関数呼び出しを検出
2. 関数に対してコンパイル時停止性検査を実行（後述の停止性検査機構を参照）
3. コンパイル時に評価を実行
4. 結果を生成された型に埋め込む
...
#### 境界失敗のコンパイル時カバレッジ目標（#299 方向アンカー）

> **実装状態説明**（#299）：コンテナ型は脱特殊化済み——`Array(T, N)`
> は const ジェネリクスコンストラクタであり、リテラルコンテキスト落点と `in`
> メンバーシップ述語は既に実装済み。（#300）：`Array(T, N)`
> リテラル落点の N と要素型はコンパイル時検証で強制され（E1002）、**N は信頼可能**——本節の目標機構は「注釈 N
> == ランタイム長」の上に構築可能。現在の `[]`
> インデックス範囲外（E6003）と Dict キー欠落（E6008）は
> **ランタイムエラー過渡状態**；本節の値依存型はこれらの境界失敗を
> **コンパイル時**に押し下げる目標機構である：
>
> - const インデックス：`a[5]`（5 はコンパイル時定数）は `a: Array(Int, 3)`
>   のとき直接コンパイル時拒否；
> - 値インデックス：`a[i]` は前置条件 `i < len(a)` を要求し、値依存型契約により証明；
> - `in` 述語は Hoare 論理前置条件の基底：`n in 1..10`、`x in some_set`
>   はいずれもコンパイル時証明可能命題。
>
> 精錬型の完全設計は実装時に別途本節を補足する。
...
# 使用時は完全に透明、型は自動推論
numbers = List(Int)()   # 訂正：値コンストラクタ二層形式（§9.1 参照）；要素は push で充填
numbers.push(1)
numbers.push(2)
numbers.push(3)
...
| 型安全性                                | ❌（マクロ展開）           | ✅                | ✅           | ✅                           |
| 統一構文                                | ❌                         | ❌                | ❌           | ✅                           |
| コンパイル時次元検証                    | 手動特殊化                 | ランタイム検査    | type family  | コンパイル時自動検証         |
| 半自動停止性注釈（decreases/invariant） | ❌                         | ❌                | ❌           | ❌（コンパイル時全自動のみ） |

### 停止性検査機構（RFC-027 と統一）

値依存型のコンパイル時評価は**停止を保証**しなければならない。さもないと型システムが無限ループに陥る。停止性検査は RFC-027 のコンパイル時証明パイプラインによって**全自動**で完了する——コンパイラが自動的に度量を合成し、証明可能な再帰/ループは通過し、証明不可能なものは直接コンパイルエラーを報告する。**半自動注釈には余地を与えない**：RFC-022 の
`//! decreases`、`/*! invariant !*/` は RFC-022 廃止とともに廃止され、規約とは型注記そのものである。

#### 再帰関数の停止性検査

コンパイラはコンパイル時評価の前に、再帰呼び出しのパラメータがすべての再帰経路で厳密に減少していることを検査する（RFC-027
§6.7）。規約コメントは一切不要：
...
注釈を必要としない。変数上の精錬型注記（例：`UpTo(n)`）が同時にループ不変式と度量境界を提供し、コンパイラは優先度に従って四つの度量合成戦略を試し、見つかった時点で停止する（RFC-027
§7）：

1. **線形ランク関数の自動合成**——型注記から変数の境界を抽出し、線形組み合わせを列挙、SMT が m ≥
   0 かつすべての経路で m' < m を検証
2. **述語違反カウント**（実験的）——目標型定義（例：`Sorted`）から violation_count を抽出し、隣接交換/移動をカバー
3. **有界増加/減少パターン**——`v += const` → 度量 `upper - v`（戦略 1 の縮退、最速経路）
4. **乗法スケール度量テンプレート**——`v *= const`（const > 1）→ 度量 `ceil(log_const(upper / v))`

```yaoxiang
sum: (arr: Array(Int, n)) -> Int = {
    mut i: UpTo(arr.len) = 0   # 型注記が上界 arr.len と下界 0 を与える
    while i < arr.len {
        # コンパイラ自動推論：度量 arr.len - i、各反復で厳密に 1 減少 → 停止証明
        s += arr[i]; i += 1
    }
    return s
}
...
│     - 再帰関数：パラメータがすべての再帰経路で厳密減少か検査  │
│     - ループ：四つの度量合成戦略（線形ランク/違反カウント/    │
│       有界パターン/乗法スケール）、SMT が減少を検証          │
...
- **統一性**：停止性検査と正当性検証（VC 生成）が同一のコンパイル時証明パイプライン（RFC-027）を共有、独立した規約構文なし
- **全自動**：コンパイラが型注記から自動的に度量を合成し、証明できれば通過、できなければエラー——プログラマの手書き
  `decreases` に依存しない
...
| Rust         | ジェネリクス + Trait | Trait システムが複雑、学習曲線急峻                   |
| C++          | テンプレート         | テンプレート特殊化が複雑、コンパイルエラーが不明瞭   |
...
YaoXiang の**値依存型**は従来ジェネリクスに対する核心的優位性である：

| 優位性               | 説明                                                                         |
| -------------------- | ---------------------------------------------------------------------------- |
| **型の値依存**       | `Vec: (n: Int) -> Type` により型が具体的な値に依存可能                       |
| **コンパイル時評価** | 型位置での関数呼び出しをコンパイル時評価し、結果は直接型に埋め込まれる       |
| **次元検証**         | `Matrix(Float, 3, 3)` がコンパイル時に行列次元を検証                         |
| **型レベル計算**     | `If`, `Match` などの条件型が型レベル計算をサポート                           |
| **停止保証**         | コンパイル時停止性検査（自動度量合成）がコンパイル時評価の必然的な停止を保証 |
...
5. **IDE フレンドリ** - スマートヒント、明確なエラー情報
6. **値依存型** - 型が値に依存可能、コンパイル時次元検証をサポート
7. **コンパイル時評価の安全性** - コンパイル時停止性検査（RFC-027 自動度量合成）によりコンパイル時評価の停止を保証
...
> **重要規則**：ジェネリクス型定義は**明示的に `: Type`
> を标注しなければならない**。さもないと HM に推論されて関数になる。
>
> | 書き方                            | 意味                           |
> | --------------------------------- | ------------------------------ |
...
List: (T: Type) -> Type = {
    data: Array(T),
    length: Int,
    push: (self: List(T), item: T) -> Void,   # self は単なる慣例名、キーワードではない
...
# ジェネリクス制約（直接式、ワンライナーは return 省略可）
...
位置を自然に認識し、実際引数の型から自動推論して充填する。

```yaoxiang
# コンパイラがジェネリクスパラメータを自動推論
numbers: List(Int) = List(Int)()
#         ^^^^^^^^   ^^^^^^^^
#         型宣言     コンストラクタ呼び出し：Int が T を充填、() 値構築
...
| **Dup**                  | シャローコピー：ハンドル/トークン複製、底层データ共有 | 代入/引数渡し自動 | `&T` トークン、`ref T`、String/Bytes |
| **Clone**                | ディープコピー：完全独立な複製を作成                  | `value.clone()`   | Clone を実装する任意の型             |

**Dup の意味論**：Dup を実装する型は、代入/引数渡し時に所有権を移転しない——コンパイラがハンドル/トークンを複製し、複数の保持者が同一の底層データを指す。これは RFC-009 所有権モデルにおける Move デフォルト意味論の補完である。

**Dup と Clone は直交する概念である**：

```
Dup = ハンドルを複製、データを共有（変更は相互に影響）
Clone = データを複製、複製は独立（変更は相互に影響しない）
```
...
4. デフォルト Move — 他の型はデフォルトの Move 意味論を維持
```
...
| String, Bytes            | ✅   | 内部参照カウント、ハンドル複製で底層バッファ共有          |
| `&mut T`（可変トークン） | ❌   | 線形排他、複製不可                                        |
...
**Int/Float/Bool/Char は Dup ではない**——これらは値型であり、代入時にコンパイラが自動的に値コピーを行う（二つの値は完全に独立）。これは「シャローコピー」ではなく、プリミティブに対するコンパイラ組み込み処理であり、Dup 型属性を通じて表現すべきではない。

```yaoxiang
# プリミティブ値型：コンパイラ自動値コピー（Dup ではない）
x: Int = 42
y = x          # 値コピー、x と y は完全に独立
print(x)       # ✅

# Dup：シャローコピー、ハンドル複製でデータ共有
view: &Point = &point
view2 = view    # ✅ Dup：トークン複製、両者は同一の point を指す
print(view.x)   # ✅

# Clone：明示的ディープコピー、独立な複製を作成
...
> **注意**：`Send`/`Sync` はユーザー可視トレイトとはしない。タスク横断安全保証は `ref`
> キーワードとコンパイラの全自動処理によって行われる——`ref`
...
> **訂正（#296）**：原文では「`Int`
> などの値パラメータはジェネリクスコンテキストでデフォルトでコンパイル時決定可能」とされていたが、この記述は**厳密に誤り**である——`add: (a: Int, b: Int) -> Int = a + b`
> において `a`/`b`
> はランタイム値パラメータである。**型位置で参照される**具体型パラメータのみがコンパイル時値パラメータとなる。正しい定義は以下。

**核心設計**：ジェネリクスシグネチャ内の `Type`
は型パラメータをマークする；具体型（`Int`/`Bool`/`Float`
など）の注釈付きパラメータは**コンパイル時値パラメータ候補**として列挙され、値が**型位置で参照される**（値依存）か否かによってコンパイル時値パラメータとなるかが決定される。`const`
キーワードは不要。

**判定規則（二ステップ）**：

1. **形態粗選別**：非 `Type` の具体型（例：`Int`）で注釈されたパラメータ → 候補として列挙。
2. **用途精選別**：候補名が**型位置**（型体フィールド型、内層 `Fn` パラメータ型、`Assert`
   述語、`Array(T, N)`
   などの型構築実引数位置）に現れる → コンパイル時値パラメータとして確認；それ以外は**ランタイム値パラメータ**と見なす。

| 書き方                                                     | 判定                       | 理由                                                 |
| ---------------------------------------------------------- | -------------------------- | ---------------------------------------------------- |
| `add: (a: Int, b: Int) -> Int = a + b`                     | a/b ランタイム値パラメータ | 値位置にのみ出現、型構築に参加しない                 |
| `Array: (T: Type, N: Int) -> Type = { data: Array(T, N) }` | N コンパイル時値パラメータ | N が `Array(T, N)` の型構築実引数位置に現れる        |
| `factorial: (N: Int) -> (k: N) -> Int`                     | N コンパイル時値パラメータ | N が内層パラメータ `k` の型として機能                |
| `Foo: (T: Type, N: Int) -> Type = { x: T }`                | N 不成立（以下参照）       | N が型体内で参照されず、ランタイム値パラメータに退化 |

> **値依存の本質**：コンパイル時値パラメータはすなわち値依存型である——値が**型の構築**に使用されるときのみ、コンパイル時決定が必要となる。形態（`: Int`）は候補資格のみを決定し、用途（型位置での露出）がコンパイル時値パラメータか否かを決定する。これは §「コンパイル時決定性保証」における「型位置での関数呼び出しはコンパイル時評価」と同一の判定基準である。
...
# 使用方式：factorial(5) は型位置で評価（コンパイル時）、結果 120 が型に埋め込まれる
arr: StaticArray(Int, factorial(5))  # StaticArray(Int, 120)

# ════════════════════════════════════════════════════════
# 値依存：N が内層パラメータ k の型として機能
# ════════════════════════════════════════════════════════
...
> **不成立候補の処理**：具体型で注釈されているが型位置で参照されない候補（例：上表 `Foo` の
> `N`）はランタイム値パラメータ（関数レベル経路）に退化する。型コンストラクタ経路の不成立候補はランタイムスロットを占められない（型コンストラクタはコンパイル時評価される）、宣言側で直接エラー [E1094]：「N はコンパイル時値パラメータとして宣言されたが、型体内で参照されていない」——以前の静かに破棄によりインスタンス化 arity の不一致が生じていた、issue
...
と `Void` 二つの組み込み型名がこれを担う：

**Never（⊥）** — 交渉不可能な三つの内核性質：

1. **ゼロコンストラクタ**：いかなるリテラルや式も `Never`
   型の値を生成できない。これはメタレベル性質であり、組み込みでなければならない。
2. **爆発原理**：`Never <: T` は任意の型 `T` に対して成立する。`Never`
   値は任意の型として使用可能——これが `assert(false)`
   の後のコードも型検査を通過する理由である（決して実行されないが）。
3. **発散マーカー**：`f: (...) -> Never` は `f`
   が決して返らないことを保証する。コンパイラはこれに基づいて dead code 解析を行う。

`Never` は組み込み型名でありキーワードではなく、parser は無関係。空和型リテラル構文は公開しない。

**Void（⊤、すなわち Unit）**
— ちょうど一人の居住者（デフォルト void 値）を持ち、真命題「恒真」の担い手である。`Void`
はゼロフィールド積型の単位元、`Never`
はゼロ変体和型の単位元——両者は対を成す。`x: Void = <デフォルト>` は合法、`x: Never = ...`
は右辺に書けるものがない。
...
> **Curry-Howard 同型**：条件型は Curry-Howard の視点から見ると論理における
> **case 分析**である。`Bool` 型は二つの可能な値（True/False）を持つ命題に対応し、`If`
> はその命題の真偽に応じて異なる結果を選択する——これはまさに論理における case 選言である。`match C { True => T, False => E }`
> は実際には次を表現している：「命題 C が True であることが既知の場合の結論は T、C が False であることが既知の場合の結論は E」。
...
> は「型層に加法関数を書いた」のではなく、自然数加法に関する**命題を構築している**。`(Zero, B) => B`
> は「命題 Add(Zero, B) は B と等価である」と述べ、`(Succ(A'), B) => Succ(Add(A', B))` は「Add(A',
> B) が成立するならば、Add(Succ(A'),
> B) も成立する」と述べた。これは Peano 公理における加法定義そのものである。型検査器がこの match 式の通過を検証することは、この定義の論理的一貫性を検証することと等価である。
...
# 型レベル加法（Curry-Howard：case analysis + 再帰呼び出し、完全帰納のためには停止性検査が必要）
...
**重要特性**：関数オーバーロードとインライン最適化は自然に結合し、ゼロコスト抽象を実現する。
...
# 訂正：初期版では Array(Int, 10)(1, 2, 3, ...)（要素を直接展開）と書かれていたが、
# §9.3 の権威パターン（Type(パラメータ)(フィールド構築パラメータ)/空構築）と一致しないため、
# フィールド名式構築パラメータに統一、SPEC type-system.md §4.3 を参照。
arr_100_int = Array(Int, 100)()   # 空構築、データを後で代入
...
List: (T: Type) -> Type = {
    ...
    # ジェネリクスメソッド（T は外側の List(T) から自動的にスコープに導入される）
...
# self は単なる慣例パラメータ名であり、コンパイラは名前ではなく型を見る
...
| 案                     | 選択しない理由                   |
| ---------------------- | -------------------------------- |
| 基本ジェネリクスのみ   | 複雑なマクロを代替不可           |
| 純粋マクロシステム     | 型安全性なし、エラー情報不良     |
| 依存制約のみ           | 柔軟性不足                       |
...
# ジェネリクスパラメータ内の Type はコンパイラが実引数型から自動充填
# 例：map(numbers, f) において、T は numbers: List(Int) から抽出、R は f: (Int) -> String から抽出
```
