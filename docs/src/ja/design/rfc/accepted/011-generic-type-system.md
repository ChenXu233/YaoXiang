...
- **Type 自己記述機構**：`Type` は言語レベルの特殊存在、シグネチャ中の `Type`
  位置は自動推論で充填可能
...
**値依存型**とは、1つまたは複数の**値**（型だけでなく）に依存する型のことを言う。これらの値はコンパイル時に評価可能であり、 컴파일
단계에서 타입 안전성을 보장할 수 있다.
...
型宇宙の思想は言語概念をセマンティック役割別に異なる階層に分類し、値依存型は **Type2層**
に位置する：
...
**主要設計**：Type2層の関数、型構築子、値依存型は**統一構文**で、すべて `(params) -> result`
の形式：
...
> `A → B` は論理包含「AならばB」に対応し、ジェネリック `(T: Type) -> Type`
> は全称量化「すべての型Tについて」に対応し、値依存型 `(n: Int) -> Type`
> は「各整数nに対して型が存在する」に対応する。YaoXiangが関数、型構築子、値依存型をType2層に統一することは、本質的に「証明」と「計算」を同一概念——**構成的証明**——に統一することである。これはCurry-Howard同型の言語設計への直接的反映である：1つの形式（`(params) -> result`）が論理命題と計算過程を同時に担う。
...
値依存型のコンパイル時評価は**終了を保証**しなければならない、さもなくば型システムは無限ループに陥る。YaoXiangは
**decreases仕様** を通じてこれを保証し、RFC-022とシームレスに統合する。
...
> **重要ルール**：ジェネリック型定義は**必ず `: Type` を明示的に标注** 해야
> 하며、さもなくばHM推論により関数として扱われる。
>
> | 書き方                            | 意味                            |
> | --------------------------------- | ------------------------------- |
...
`Type` は言語レベルの特殊存在であり、コンパイラは本能的にシグネチャ中の `Type`
位置を認識し、実際の引数型から自動推論で充填する。
...
> **注意**：`Send`/`Sync` はユーザー可见traitとしては提供しない。タスク間安全保障は `ref`
> キーワードとコンパイラの完全自動処理により保障——`ref`
> は Rc または Arc を自動選択し、ユーザーは Send/Sync を理解する必要がない。
...
**コア設計**：ジェネリックシグネチャ中の `Type` はコンパイル時型パラメータを标记し、`Int`
などの値引数はジェネリックコンテキストではデフォルトでコンパイル時に確定可能である。`const`
キーワードは不要。
...
YaoXiangの型システムはCurry-Howard同型において⊥（偽/空型）と⊤（真/Unit）の両方を同時に備え、`Never`
と `Void` の2つの組み込み型名で表現する：

**Never（⊥）** — 3つの交渉不可能な内核性質：

1. **ゼロコンストラクタ**：任何のリテラルや式も `Never`
   型の値を生成できない。これはメタレベル性質であり、組み込み必要。
2. **爆発原理**：`Never <: T` は任意の方 `T` に対して成立する。`Never`
   値は任意の方として使用可能——これが `assert(false)`
   以降のコードが型チェックを通過する理由（実行されることはないが）。
3. **発散マーク**：`f: (...) -> Never` は `f`
   が返さないことを保証する。コンパイラはこの情報を基にdead code分析を行う。

`Never`
は組み込み型名でありキーワードではなく、parserには無感知である。空和型リテラル構文は開放しない。

**Void（⊤、即ちUnit）**
— 丁度1つの住人（デフォルトvoid値）を持ち、真命题「恒真」の担体である。`Void`
はゼロフィールド積型の単位元であり、`Never`
はゼロvariant和型の単位元である——二者に対偶関係がある。`x: Void = <デフォルト>`
は合法であり、`x: Never = ...` は右辺に书写できるものが何もない。
...
> **Curry-Howard同型**：条件型はCurry-Howardの視点からは論理における **case分析** である。`Bool`
> 型は2つの可能な値を持つ命題（True/False）に対応し、`If`
> はその命題の真偽に応じて異なる結果を選択する——これは論理におけるcase選言に他ならない。`match C { True => T, False => E }`
> は实际上、「命題 C が True の時結論は T、C が False の時結論は E」を表現している。
...
> **Curry-Howard同型**：型族は「命題即型」最も直接的な体现である。`Add: (A: Type, B: Type) -> Type`
> は「型レベルで加算関数を記述した」ではなく、**自然数加算に関する命題を構成している**。(Zero, B) =>
> B は「命題 Add(Zero, B) は B と同値である」ことを言い、(Succ(A'), B) => Succ(Add(A',
> B)) は「Add(A', B) が成立するなら、Add(Succ(A'),
> B) も成立する」ことを言う。これはPeano公理における加算定義そのものである。型チェッカーがこのmatch式を検証することは、この定義の論理的一貫性を検証することと等価である。
