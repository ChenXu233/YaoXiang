...
# RFC-011：ジェネリクスシステム設計 - ゼロコスト抽象化とマクロ代替

> **状態**: 承認済み
> **著者**: 晨煦
> **作成日**: 2025-01-25
> **最終更新**: 2026-04-22（Type自己記述機構へ更新、ジェネリック呼び出し構文を統一）

## 概要

本書はYaoXiang言語の**ジェネリクスシステム設計**を定義し、強力なジェネリクス機能によるゼロコスト抽象化を実現し、コンパイル時最適化によりマクロへの依存を軽減し、デッドコード消除メカニズムを提供します。

**コア設計**：
- **統一シグネチャ構文**：`(T: Type, R: Type) -> ...` ジェネリック引数と通常引数を統一
- **Type自己記述機構**：`Type`は言語レベルの特殊な存在であり、シグネチャ中の`Type`の位置は自動的に推論・補完可能
- **型制約**：`T: Clone + Add` マルチ制約、関数型制約
- **関連型**：`Iterator: (Item: Type) -> Type = { next: () -> Option(Item), has_next: () -> Bool }`
- **コンパイル時ジェネリクス**：`N: Int` ジェネリック値引数、コンパイル時定数インスタンス化
...
| [型宇宙思想](../reference/plan/ongoing/类型宇宙思想.md) | **理論コア** | 型宇宙階層モデルと値依存型設計 |
| [RFC-022: ホイル論理静的検証](./draft/022-hol-logic-verification.md) | **終了チェック** | decreases契約とコンパイル時評価安全保障 |

## 型宇宙思想と値依存型

YaoXiangのジェネリクスシステムは**型宇宙思想**に基づいており、このメンタルモデルは言語内のすべての概念を階層構造に統一し、コアとなる革新は**値依存型**をType2層の第一級市民として昇格させたことです。

### 値依存型とは？

**値依存型**とは、1つまたは複数の**値**（他の型だけでなく）に依存する型のことです。これらの値はコンパイル時に評価可能であり、编译段階から型安全性を提供します。
...
従来のジェネリクス相比、YaoXiangの値依存型は以下のコア優位性を持っています：
...
| 型安全性 | C++なし、Rust制限的 | 完全な型安全性、コンパイル時チェック |
| 次元検証 | ランタイムチェックまたは手動特殊化 | コンパイル時次元検証、ランタイムオーバーヘッドなし |

### 型宇宙階層と値依存型

型宇宙思想は言語概念を意味役割に従って異なる階層に分類し、値依存型は**Type2層**に位置します：
...
**鍵設計**：Type2層の関数、型コンストラクタ、値依存型は**統一構文**で、`(params) -> result`の形式です：
- 通常関数：`(Int, Int) -> Int` → 戻り値は値
- 型コンストラクタ：`(T: Type) -> Type` → 戻り値は型
- 値依存型：`(n: Int) -> Type` → 戻り値は型だが値引数に依存

> **Curry-Howard同型**：この統一は偶然ではありません。Curry-Howard同型は「型即ち命題、プログラム即ち証明」を指摘します——関数型`A → B`は論理包含「AならばB」に対応し、ジェネリック`(T: Type) -> Type`は全称量化「任意の型Tについて」に対応し、値依存型`(n: Int) -> Type`は「各整数nについてある型が存在する」に対応します。YaoXiangは関数、型コンストラクタ、値依存型をType2層に統一することで、本質的に「証明」と「計算」を同一概念——**構成的証明**に統一します。これはCurry-Howard同型が言語設計に直接具現化されたものであり、一つの形式（`(params) -> result`）が論理命題と計算過程を同時に担います。

### コンパイル時確定的保証

YaoXiangの型宇宙思想は以下の要件があります：**Type階層的一切はコンパイル時に確定**。
...
コンパイラは自動的に以下を実行します：
1. 型位置上にある関数呼び出しを検出
2. 関数が`decreases`契約でマークされているか検証（以下に示す終了チェックメカニズムを参照）
3. コンパイル時に評価を実行
4. 結果を生成された型に埋め込み
...
値依存型のコンパイル時評価は**終了を保証**でなければならず、さもなくば型システムは無限ループに陥ります。YaoXiangは**decreases契約**を通じてこれを保証し、RFC-022とシームレスに統合します。
...
┌─────────────────────────────────────────────────────────────┐
│  型チェックフェーズ                                          │
│  型位置上に関数呼び出しを検出（例：Vec(factorial(5))）        │
└─────────────────────────┬───────────────────────────────────┘
                          ▼
┌─────────────────────────────────────────────────────────────┐
│  1. decreases契約をチェック                                   │
│     - decreasesあり：すべての再帰パスで逓減条件が成立するか検証 │
...
- **安全性**：コンパイル時評価が必ず終了することを保証し、型システムが無限ループに陥ることを 방지
...
| Rust | ジェネリクス + Trait | Traitシステム複雑、学習曲線が険しい |
| C++ | テンプレート | テンプレート特殊化複雑、コンパイルエラーメッセージが悪い |
| **YaoXiang** | **値依存型** | **型は値に依存、コンパイル時次元検証、終了保証** |
...
2. **デッドコード消除** - コンパイル時分析、使用されているジェネリクスのみをインスタンス化
3. **マクロ代替** - ジェネリクスでマクロ使用シナリオの90%を代替
4. **型安全性** - コンパイル時チェック、ランタイム型オーバーヘッドなし
5. **IDEフレンドリ** - インテリセンス、明確なエラーメッセージ
6. **値依存型** - 型は値に依存可能、コンパイル時次元検証をサポート
7. **コンパイル時評価安全性** - decreases契約によりコンパイル時評価の終了を保証

### 設計原則

- **コンパイル時確定**：ジェネリックパラメータはコンパイル時に確定
- **単態化優先**：具象コードを生成、仮想関数呼び出しを避ける
...
> **重要規則**：ジェネリック型定義は**必ず`: Type`を明示的にマーク**でなければならず、さもなくばHM推論により関数とみなされます。
>
> | 書き方 | 意味 |
> |------|------|
...
```yaoxiang
# ジェネリック関数は統一した(T: Type, R: Type)シグネチャ構文を使用
...
`Type`は言語レベルの特殊な存在であり、コンパイラはシグネチャ中の`Type`位置を本能的に識別し、実際の引数型から自動的に推論・補完します。

```yaoxiang
# コンパイラは自動的にジェネリックパラメータを推論
...
```yaoxiang
# Iterator trait（(Item: Type) -> Type構文を使用）
...
# ArrayのIterator実装
# メソッド構文糖衣使用：Array.Item, Array.next, Array.has_next
...
# - CがTrueの時、Voidを導出
# - CがFalseの時、compile_error("Assertion failed")を導出
...
Array: (T: Type, N: Int) -> Type = {
    data: Array(T, N),
    # コンパイル時チェック：Nは0より大きくなければならない（Assertは型位置）
    length: Assert(N > 0),
}
...
> **Curry-Howard同型**：条件型はCurry-Howard観点からは論理における**case分析**です。`Bool`型は2つの可能な値を持つ命題（True/False）に対応し、`If`はその命題の真偽に応じて異なる結果を選びます——これは論理におけるcase選言そのものです。`match C { True => T, False => E }`は実質的に「命題CがTrueの時の結論はT、CがFalseの時の結論はE」を表現しています。
...
> **Curry-Howard同型**：型族は「命題即ち型」最も直接的な具現化です。`Add: (A: Type, B: Type) -> Type`は「型レベルに加法関数を記述した」ではなく、自然数加法に関する命題を**構築している**のです。`(Zero, B) => B`は「命題Add(Zero, B)はBと同値」といい、`(Succ(A'), B) => Succ(Add(A', B))`は「Add(A', B)이 성립하면 Add(Succ(A'), B)도 성립한다」と言います。これがPeano公理における加法の定義そのものです。型チェッカーがこのmatch式を検証を通ることは、この定義の論理的整合性を検証することと同値です。
...
# 型レベル加算（Curry-Howard：これも自然数加法の帰納的定義）
...
**核心的特性**：関数オーバーロードとインライン最適化は天然に結合し、ゼロコスト抽象化を実現します。
...
# ======== コンパイル後（等価コード）=======
# コンパイラが自動的に最適特殊化を選択し затемインライン展開
...
1. **コンパイラインテリジェント選択**
   ```yaoxiang
   sum(int_arr)      # 自動的にsum: (Array(Int)) -> Intを選択
   sum(float_arr)    # 自動的にsum: (Array(Float)) -> Floatを選択
   sum(custom_arr)  # 自動的にsum: (T: Type) -> ((arr: Array(T)) -> T)を選択
   ```
...
# 使用
result_type = Add[Int, Float]  # Floatに推論
```
...
List: (T: Type) -> Type = {
    ...
    # ジェネリックメソッド（Tは外側のList(T)から自動的にスコープに入る）
    push: (self: List(T), item: T) -> Void,
...
# ======== 2. ジェネリックメソッド実装 ========
# Type.method構文糖衣を使用：List型に自動的に関連付け
...
2. **デッドコード消除**
   - コンパイル時分析、使用されたジェネリスのみをインスタンス化
   - コード膨胀が制御可能

3. **マクロ代替**
   - 型安全なコード生成
   - IDEフレンドリ、明確なエラーメッセージ
...
| リスク | 影響 | 軽減措施 |
|------|------|----------|
| 制約解決複雑度 | コンパイル時間が过长 | 增量解決 + キャッシュ |
| コード膨胀 | バイナリファイル过大 | DCE + しきい値制御 |
| 実装複雑度 | 開発周期延长 | 段階的実装 |
...
# ジェネリックパラメータのTypeはコンパイラが実引数型から自動的に補完
# 例：map(numbers, f)、Tはnumbers: List(Int)から、Rはf: (Int) -> Stringから抽出
```
