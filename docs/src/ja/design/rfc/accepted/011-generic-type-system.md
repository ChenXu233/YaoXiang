...
- **Type 自己記述メカニズム**：`Type` は言語レベルの特殊な存在であり、シグネチャ内の `Type` 位置は自動的に推論・充填可能
...
型宇宙思考は言語概念をセマンティックロールごとに異なる階層に分類し、値依存型は **Type2 層** に位置します：
...
**重要な設計**：Type2 層の関数、型構築子、値依存型は**統一構文**であり、すべて `(params) -> result` の形式です：
...
> `A → B` は論理包含「A ならば B」に対応し、ジェネリック `(T: Type) -> Type` は全称量化「すべての型 T に対して」に対応し、値依存型
> `(n: Int) -> Type`
...
値依存型のコンパイル時評価は**終了を保証**する必要があります、さもなくば型システムは無限ループに陥ります。YaoXiang は **decreases 制約**
によりこれを保証し、RFC-022 とシームレスに統合します。
...
> **重要なルール**：ジェネリック型定義は**必ず `: Type` を明示的にマークする**必要があります、さもなくばHM推論により関数として扱われます。
>
> | 書き方                              | 意味                       |
> | --------------------------------- | -------------------------- |
...
`Type` は言語レベルの特殊な存在であり、コンパイラはシグネチャ内の `Type` 位置を自然に識別し、实际引数の型から自動的に推論・充填を行います。
...
**Int/Float/Bool/Char は Dup ではありません**——これらは値型であり、代入時にコンパイラが自動的に値コピーを行います（2つの値が完全に独立）。これは「浅コピー」ではなく、コンパイラのプリミティブに対する組み込み処理であり、`Dup` 型属性を通じて表現する必要も表現すべきもありません。
...
> キーワードとコンパイラの全自动処理により保障されます——`ref` は Rc または Arc を自動的に選択し、ユーザーは Send/Sync を理解必要はありません。
...
などの値パラメータはジェネリックコンテキストではデフォルトでコンパイル時に確定可能です。`const` キーワードは不要です。
...
YaoXiang の型システムは Curry-Howard 同型において ⊥（偽/空型）と ⊤（真/Unit）の両方的同时具备し、`Never` と
`Void` の2つの組み込み型名で担います：

**Never（⊥）** — 3つの交渉不可な内核性質：

1. **零構築子**：いかなるリテラルや式も `Never` 型の値を生成できません。これはメタレベルの性質であり、組み込みが必要です。
2. **爆発原理**：`Never <: T` は任意の型 `T` に対して成立します。`Never` 値は任意の型として使用できます——これが
   `assert(false)` の後のコードが型チェックを通る理由です（ embora 永不実行到）。
3. **発散マーク**：`f: (...) -> Never` は `f` が返回しないと保証します。コンパイラはこれに基づいて dead code 分析を行います。

`Never` は組み込み型名であり 키워드ではなく、parser は無感知です。空和型リテラルの構文は開放しません。

**Void（⊤，即ち Unit）** — 丁度1つの住人（デフォルト void 値）があり、真命題「恒真」の担体です。`Void`
は零フィールド積型の単位元であり、`Never` は零variant和型の単位元——二者対偶です。`x: Void = <デフォルト>`
は合法であり、`x: Never = ...` は右辺に何も書けない。
