...
本 RFC は全新的な**多位置連合バインディング**構文を提案する。関数を型の任意の引数位置に正確にバインディングすることを可能にし、单位置バインディングと多位置連合バインディングをサポートすることで、カレ化されたバインディングにおける「誰が呼び出し側か」という問題を根本的に解決し、`self`
キーワードを導入する必要がない。
...
1. **呼び出し側位置の柔軟性が低い**：従来のバインディングでは `obj.method(args)` の `obj`
   を最初の引数に固定只能
...
**コアルール：`[n]` を書かない = バインディングしない。** `Point.name = func`
は只是名称空間エイリアスであり、任何な暗黙的なバインディングをトリガーしない。`p.name(args)`
のような `.` 呼び出し構文を有効にするには、明示的に指定する必要がある：`Point.name = func[n]`。
...
- `distance = distance[0]` をバインディングする際、元関数シグネチャは
