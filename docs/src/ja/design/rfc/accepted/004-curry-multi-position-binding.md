---
title: "RFC-004: 柯里化メソッドのマルチポジション結合バインディング設計"
status: "承認済み"
author: "晨煦"
created: "2025-01-05"
...
# RFC-004: 柯里化メソッドのマルチポジション結合バインディング設計

## 要約

本 RFC は全新の**マルチポジション結合バインディング**構文を提案する。これにより関数を型の任意のパラメータ位置に正確にバインドでき、単一位置バインディングとマルチポジション結合バインディングの両方をサポートする。柯里化バインディングにおける「呼び出し元は誰か」という問題を、`self` キーワードを導入することなく根本的に解決する。

## 動機

### なぜこの機能が必要なのか？

現在の言語設計では、独立した関数を型のメソッドとしてバインドする際に以下の問題に直面している：

1. **呼び出し元の位置が柔軟でない**：従来のバインディングでは `obj.method(args)` の `obj` を最初のパラメータに固定することしかできない
2. **マルチパラメータバインディングが困難**：メソッドが複数の同型パラメータを受け取る場合、优雅に表現できない
3. **柯里化の意味的曖昧さ**：部分適用時に「どの位置にバインドされているか」の区別が困難

### 設計目標：二つのプログラミング視点を統一

本設計は**関数型と OOP 二つのプログラミング視点を統一**することを目指す：
...
# [positions] 構文糖により二つの書き方は等価となり、本質的にはどちらも関数呼び出し
Point.distance = distance[0]   # this を第0位にバインド
```

**核心的価値**：
- 底层は関数、上層はメソッド構文
- `self` キーワードを導入せず、言語の簡潔さを保持
- 完全関数型：メソッド呼び出しは本質的にパラメータ渡し
- `[0]`, `[1]`, `[-1]` により this バインディング位置を柔軟に制御
- **構文の統一**：関数定義は `name: (params) -> Return = body` の形式を使用
...
**中核ルール：`[n]` を書かない = バインドしない。** `Point.name = func` は単なる名前空間のエイリアスであり、暗黙のバインディングは一切発生しない。`p.name(args)` という `.` 呼び出し構文を有効化させるには、明示的に指定する必要がある：`Point.name = func[n]`。
...
Point.distance = distance            # Point.distance(p1, p2) のみ可能
...
- 柔軟な制御：`[n]` でバインディング位置を正確に制御

#### 柯里化バインディング

関数のパラメータ数がバインディング位置数より多い場合、柯里化関数が自動生成される。**バインディングは常に明示的な操作である。**
...
# 位置 0 に明示的にバインド → 柯里化：残りのパラメータ factor は呼び出し元が提供
Point.scale = scale[0]
...
`[position]` 構文を導入し、関数のパラメータと型のバインディング関係を正確に制御する：
...
# 単一位置バインディング
Point.distance = distance[1]           # 第1パラメータにバインド（インデックスは 0 から開始）
# 使用：p1.distance(p2) → distance(p2, p1)

# マルチポジション結合バインディング（タプル分解）
Point.transform = transform[1, 2]      # 第1,2パラメータにバインド
...
バインディングは個別のバインディング文を必要とせず、型定義本体内に直接記述できる：
...
**柯 curry化セマンティクス**：
- `distance = distance[0]` をバインドする際、元関数シグネチャは `(a: Point, b: Point) -> Float`
- 生成される method シグネチャ：`b: Point -> Float`（第0位は呼び出し元が埋める）
...
# 第1パラメータ（Point 型）のみをバインドし、第3パラメータを保持
...
| マルチポジション（柯 curry化） | `Point.scale = scale[0]` | `p.scale(2.0)` | `scale(p, 2.0)` |
| プレースホルダ | `Type.method = func[1]` | `obj.method(arg)` | `func(arg, obj)` |

**説明**：
- **バインドしない**：`Point.name = func` は単なる名前空間エイリアスで、`.` 呼び出し構文はない
- `[0]`：呼び出し元を第 0 位（最初のパラメータ）にバインド
- `[1]`：呼び出し元を第 1 位（二番目のパラメータ）にバインド
- `[-1]`：呼び出し元を最後の位にバインド（末尾からカウント）

## トレードオフ

### 利点

- **明示的バインディング**：`[n]` が唯一のバインディング機構で、書かない限りバインドされず、暗黙の挙動がない
- **精密な制御**：任意のパラメータ位置にバインドでき、柔軟性が高い
- **型安全**：コンパイル時に完全に型検査され、型が一致する場合のみバインド
- **構文の簡潔さ**：`[position]` 構文は直感的で理解しやすい
- **`self` キーワードなし**：言語の簡潔さを保持
- **柯 curry化フレンドリー**：部分適用とチェーン呼び出しを自然にサポート
- **OOP フレンドリー**：自動柯 curry化により OOP プログラマがそのまま移行可能

### 欠点

- **学習コスト**：位置インデックスの概念を理解する必要がある
- **コンパイルの複雑さ**：バインディング解析と型検査によりコンパイラの複雑さが増す
...
| 案 | 説明 | 採用しない理由 |
|------|------|-----------|
| `self` キーワード | Python/Rust スタイルの `self` を導入 | YaoXiang の暗黙の `self` を持たない設計哲学に反する |
| 名前付きパラメータバインディング | 名前付きパラメータ `func(a=obj)` を使用 | 関数シグネチャ定義の変更が必要で、複雑さが増す |
| マクロシステム | マクロでバインディングを実装 | 実行時オーバーヘッドが大きく、型安全性が低下する |
...
1. **Phase 1: 基本バインディング**（v0.3）
   - 単一位置 `[n]` バインディング構文を実装（n は 0 から開始、負数をサポート）
   - 基本的な型検査とコード生成
   - ユニットテストカバレッジ

2. **Phase 2: 高度な機能**（v0.5）
   - 範囲構文 `[n..m]` のサポート
   - コンパイル時の位置計算最適化
...
| 負数インデックス | サポート | 柔軟性、末尾からカウント |
| プレースホルダ | `_` | 簡潔で汎用的なシンボル |
| 範囲構文 | 実装 | バッチバインディング、例：`[0..2]` |
| 構文スタイル | 中置 `Type.method = func[positions]` | RFC-010 と統一 |
| **バインディングルール** | **明示的に `[n]` を書いた場合のみバインド、書かない場合はバインドしない** | **暗黙の挙動なし、関数定義とバインディングは直交** |
| **名前空間** | **`Type.name` は単なる名前空間の所属であり、バインディングを起動しない** | **定義とバインディングの分離** |
| **関数構文** | **シグネチャ内でパラメータ名を宣言 `name: (params) -> Return`** | **RFC-010 と統一** |
...
| 結合バインディング | 型を複数のパラメータ位置にバインド |
| 部分適用 | パラメータの一部のみを提供し、呼び出し未完了の関数を返す |
| **統一構文** | **`name: (params) -> Return = body`、シグネチャ内でパラメータ名を宣言** |
| **名前空間関数** | **`Type.name` 構文、関数は Type の名前空間に属し、暗黙のバインディングを含意しない** |
