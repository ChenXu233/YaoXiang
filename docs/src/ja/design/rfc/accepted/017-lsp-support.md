...
> **参考**: 完全な例は [EXAMPLE_full_feature_proposal.md](EXAMPLE_full_feature_proposal.md)
> をご覧ください。
...
**現状**：現在の型チェッカーは最初のエラーに遭遇した時点で（`?`
演算子を使用して）直接返されるため、すべてのエラーを収集できません。
...
- **定数値提示**：コンパイル時にすでに計算済みの定数を表示（例：`const MAX = 100 + 200` の横に `300`
  を表示）
