---
title: "RFC-012：F-String テンプレート文字列"
---

# RFC 012: F-String テンプレート文字列

> **ステータス**: 承認済み
> **著者**: Chen Xu
> **作成日**: 2025-01-27
> **最終更新**: 2026-02-12

## 概要

YaoXiang 言語に f-string テンプレート文字列機能を追加し、変数挿入、式評価、フォーマット出力をサポートする。f-string は Python スタイルの構文（`f"..."` 接頭辞）を使用し、文字列中で `{式}` 構文により式を埋め込み、コンパイル時に効率的な文字列操作に変換する。

> **注意**: f-string の構文と動作は Python と一貫しており、詳細については [Python 公式ドキュメント](https://docs.python.org/3/tutorial/inputoutput.html#formatted-string-literals) を参照。

## 動機

### なぜこの機能が必要か

現在の YaoXiang の文字列連結方式是が面倒である：

```yaoxiang
# 現状：+ による連結
name = "Alice"
age = 30
message = "Hello ".concat(name).concat(", age: ").concat(age.to_string())
print(message)

# または format 関数を使用
message2 = format("Hello {}, age: {}", name, age)
```

###  現在の問題

1. **可読性が低い**：文字列の連結とフォーマットに多处の呼び出しが必要で、コード冗長
2. **間違えやすい**：手動で型変換を行い、`.to_string()` を見落としやすい
3. **パフォーマンスの考慮**：複数回の文字列連結はパフォーマンスに影響を与える可能性がある
4. **表現力が不足**：文字列内に複雑な式を直感的に埋め込むことができない

## 提案

### 中核的な設計

f-string を新しい文字列リテラルの接頭辞として導入し、以下のをサポート：
- **変数挿入**：`f"Hello {name}"`
- **式評価**：`f"Sum: {x + y}"`
- **フォーマット指定子**：`f"Pi: {pi:.2f}"`
- **型安全**：コンパイル時に式の型を検査

### 例

```yaoxiang
# 基本的な挿入
name = "Alice"
greeting = f"Hello {name}"  # "Hello Alice"

# 式の挿入
x = 10
y = 20
result = f"Sum: {x + y}"    # "Sum: 30"

# フォーマット指定子
pi = 3.14159
formatted = f"Pi: {pi:.2f}"  # "Pi: 3.14"

# 複雑な式
items = [1, 2, 3]
s = f"Count: {len(items)}, sum: {sum(items)}"  # "Count: 3, sum: 6"

# オブジェクトメソッド呼び出し
user = User("Bob", 25)
bio = f"Name: {user.name}, age: {user.get_age()}"
```

### 構文の変化

| 変更前 | 変更後 |
|--------|--------|
| `"Hello ".concat(name)` | `f"Hello {name}"` |
| `format("Value: {}", value)` | `f"Value: {value}"` |
| `format("Pi: {:.2f}", pi)` | `f"Pi: {pi:.2f}"` |

### 構文仕様

```
FStringLiteral ::= 'f' '"' FStringContent* '"'
FStringContent ::= FStringChar | EscapeSequence | FStringInterpolation
FStringInterpolation ::= '{' Expression (':' FormatSpec)? '}'
FormatSpec      ::= [width] ['.' precision] type
width           ::= digit+
precision       ::= digit+
type            ::= 'b' | 'c' | 'd' | 'e' | 'E' | 'f' | 'F' | 'g' | 'G' | 'n' | 'o' | 's' | 'x' | 'X' | '%'
```

## 詳細な設計

###  字句解析

コンパイラの字句解析段階で `f` 接頭辞の文字列リテラルを認識し、波括弧内の式と省略可能なフォーマット指定子を解析する。

### 変換戦略

f-string はコンパイル時に効率的な文字列操作に変換される：

**単純な挿入**：
```yaoxiang
f"Hello {name}"
```
これは以下に変換される：
```yaoxiang
"Hello ".concat(name.to_string())
```

**式による挿入**：
```yaoxiang
f"Sum: {x + y}"
```
これは以下に変換される：
```yaoxiang
"Sum: ".concat((x + y).to_string())
```

**フォーマット指定子**：
```yaoxiang
f"Pi: {pi:.2f}"
```
これは以下に変換される：
```yaoxiang
format("Pi: {:.2f}", pi)
```

**複数の挿入**：
```yaoxiang
f"Hello {name}, you are {age} years old"
```
これは以下に変換される：
```yaoxiang
"Hello ".concat(name.to_string()).concat(", you are ").concat(age.to_string()).concat(" years old")
```

### 型システムへの影響

- 挿入された式は `Stringable` インターフェースを実装する必要がある（基本型と文字列に автомат で実装）
- フォーマット指定子は型が相应なフォーマットをサポートする必要がある
- コンパイラは式の型とフォーマットルールの整合性を検査する

### コンパイラの修正

| コンポーネント | 修正内容 |
|------|----------|
| lexer | f 接頭辞の認識、文字列内挿入構文の解析 |
| parser | 新規 FStringLiteral 構文ノード |
| typecheck | 挿入式の型検査、フォーマットルールの検証 |
| codegen | 文字列連結またはフォーマット呼び出しコードの生成 |

### 後方互換性

- ✅ 完全な後方互換性
- 既存の文字列リテラル `"..."` はそのまま維持
- f-string は新規追加の構文で、既存のコードに影響しない

## トレードオフ

### 利点

1. **構文が簡潔**：定型コードが減り、可読性が向上
2. **型安全**：コンパイル時に検査し、実行時エラーを減少
3. **パフォーマンスの最適化**：コンパイラが文字列連結を最適化可能
4. **表現力が強い**：任意の式とフォーマットをサポート
5. **学習コストが低い**：Python エコシステムと一貫性がある

### 欠点

1. **コンパイラの複雑さ**：新規の構文解析と変換ロジックが必要
2. **構文の曖昧さ**：既存の文字列構文との区別が必要
3. **デバッグの課題**：コンパイル後のコードがソースコードの構造と異なる

## 代替案

| 案 | 採用しなかった理由 |
|------|--------------|
| 変数の挿入のみサポート | 複雑なフォーマット要件を満たせない |
| 関数型スタイル `format(...)` を使用 | 構文が十分に簡潔でない |
| v2.0 まで延期 | ユーザーが文字列の利便性について明確な需要がある |
| 逆引用符または他の接頭辞を使用 | Python エコシステムと一貫性がない |

## 実装戦略

### フェーズ分け

1. **フェーズ 1 (v0.9)**:
   - 基本的な f-string 構文サポート
   - 変数と単純な式の挿入
   - 基本的な型変換

2. **フェーズ 2 (v1.0)**:
   - フォーマット指定子のサポート
   - 複雑な式の挿入
   - パフォーマンスの最適化

3. **フェーズ 3 (v1.1)**:
   - デバッグ情報の強化
   - エラーメッセージの改善
   - 追加の書式設定オプション

### 依存関係

- 外部依存関係なし
- 基本的な型システムサポートが必要
- 文字列ライブラリの基本機能が必要

### リスク

1. **パフォーマンスリスク**：複数の挿入が过多な文字列オブジェクトを生成する可能性がある
   - **軽減**：コンパイラの最適化で隣接する文字列定数をマージ
2. **型チェックの複雑さ**：フォーマット指定子の型チェック
   - **軽減**：Python の実装を参考に、シンプルで直接的なチェックを使用
3. **構文の曖昧さ**：`{` と `}` のネスト使用
   - **軽減**：明確な構文ルールでネストを制限

##  未解決の問題

- [x] エスケープ 波括弧のサポート？Python と一貫：二重波括弧で単一波括弧を表現，如 <code v-pre>{{</code> は <code v-pre>{</code> を、<code v-pre>}}</code> は <code v-pre>}</code> を表現
- [x] カスタムフォーマット関数のサポート？Python と一貫：`__format__` メソッド 통한カスタムフォーマット動作をサポート
- [x] フォーマット指定子の完全な仕様？Python と一貫，上面の BNF 参照
- [x] パフォーマンス最適化の具体的な戦略？Python と一貫：実行時の連結で，特異な最適化は不要
- [x] エラー診断のベストプラクティス？Python と一貫：错误時に元の f-string の內容と位置を表示

## 付録

### 付録A：フォーマット指定子リファレンス

| 型 | 指定子 | 例 | 出力 |
|------|--------|------|------|
| 整数 | `d` | `f"{42:d}"` | "42" |
| 浮動小数点 | `f` | `f"{3.14:.2f}"` | "3.14" |
| 科学的記数法 | `e` | `f"{1000:e}"` | "1.000000e+03" |
| 文字列 | `s` | `f"{name:s}"` | "Alice" |
| 十六進法 | `x` | `f"{255:x}"` | "ff" |

### 付録B：使用シナリオの例

```yaoxiang
# ログ記録
log(level: String, msg: String, count: Int) = () => {
    timestamp = get_timestamp()
    print(f"[{timestamp}] {level}: {msg} (count: {count})")
}

# JSON 構築
json = "{\n    \"name\": \"".concat(user.name).concat("\",\n    \"age\": ")
    .concat(user.age.to_string()).concat(",\n    \"email\": \"")
    .concat(user.email).concat("\"\n}")

# SQL クエリ構築（SQL インジェクションのリスクに注意）
query = f"SELECT * FROM users WHERE age > {min_age} AND status = '{status}'"

# デバッグ情報
debug_info = f"Point({x:.2f}, {y:.2f}) at {timestamp}"

# 条件付きフォーマット
status_msg = if is_active {
    f"User {name} is active"
} else {
    f"User {name} is inactive"
}
```

---

## 参考文献

- [Python f-strings](https://docs.python.org/3/tutorial/inputoutput.html#formatted-string-literals)
- [Rust format! macro](https://doc.rust-lang.org/std/macro.format.html)
- [JavaScript template literals](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals)
- [C# interpolated strings](https://docs.microsoft.com/en-us/dotnet/csharp/language-reference/tokens/interpolated)

---

## ライフサイクルと帰趣

RFC には以下の状態フローがある：

```
┌─────────────┐
│   草案      │  ← 著者作成
└──────┬──────┘
       │
       ▼
┌─────────────┐
│  審査中     │  ← コミュニティ議論
└──────┬──────┘
       │
       ├──────────────────┐
       ▼                  ▼
┌─────────────┐    ┌─────────────┐
│  承認済み   │    │  拒否済み   │
└──────┬──────┘    └──────┬──────┘
       │                  │
       ▼                  ▼
┌─────────────┐    ┌─────────────┐
│   accepted/ │    │    rfc/     │
│ (正式設計)  │    │ (原件保管)  │
└─────────────┘    └─────────────┘
```