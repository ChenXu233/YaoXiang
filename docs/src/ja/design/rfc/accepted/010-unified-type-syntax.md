...
YaoXiang は唯一の宣言形式を持つ：

```
identifier : type = expression
```

ここで `type` は任意の型式、`expression` は任意の値式である。**`fn` も `struct` も `trait` も `impl`
も、小文字の `type` キーワードも存在しない**（ただし `Type` をメタ型キーワードとして持つ）。

> **核心設計**：`Type` 自体が一つのジェネリック型である。`(T: Type) -> Type`
> は「型引数 T を受け取る型」を意味する。
...
> **名前空間とメソッドバインド**：`Type.name`
> 接頭辞は**名前空間の帰属**を表すだけで、それ以外には何の意味も持たない。`p.draw(screen)` という
> `.`
> 呼び出し構文を有効化するには、明示的なバインドが必要である：`Point.draw = draw[0]`。詳細は後述の「名前空間とメソッドバインド」節を参照。これは型階層の注記として使われ、コンパイラが Type0、Type1、Type2... の区別を自動処理し、ユーザには透過的である。
...
これらの概念の間には統一性がなく、構文の断片化と高い学習コストを招いている。

### 設計目標

1. **極限の統一性**：一つの構文ルールですべての状況をカバー
2. **簡潔でエレガント**：`name: type = value` の対称的美学
3. **新しいキーワード不要**：既存の構文要素を再利用
4. **理論的エレガンス**：型自体も `Type` 型の値
5. **ジェネリック対応**：ジェネリックシステム（RFC-011）とシームレスに統合

### ジェネリックシステムとの統合

RFC-010の統一構文モデルとRFC-011のジェネリックシステム設計は**天然契合**であり、ジェネリック引数は統一モデルにシームレスに溶け込む：
...
- RFC-011 Phase 1（基本ジェネリック）はRFC-010の**強い依存先**
- 基本ジェネリックがなければ、RFC-010のジェネリック例はコンパイル不可
- 推奨：RFC-011 Phase 1 は RFC-010 と同期実装

## 提案

### 核心原則：型コンストラクタと関数/変数の区別
...
- **`: Type` がない** → HM が能動的に `{ ... }` をコードブロックと解釈、関数型と推論
...
# ❌ エラー：: Type がない、コンパイラは { ... } を型として解釈できない
...
│       Point.draw = draw[0]  # 明示的にバインドしてはじめてドット呼び出し構文が有効
│
└── ジェネリック関数
...
correspondence）の直接的な写像である。この同型は深い事実を明らかにする：**型システムと論理システムは同じものの二つの面にすぎない**。
...
**Curry-Howard のもとでの `name: type = value` の意味**：

```yaoxiang
// "x: Int = 42" は「Int 型の証明が存在し、名前は x、値は 42」と読む
x: Int = 42

// "add: (a: Int, b: Int) -> Int = a + b" は次のように読む：
// 「含意証明が存在する：Int の証明 a と b を与えれば、Int の証明を構成できる」
add: (a: Int, b: Int) -> Int = a + b

// "Point: Type = { x: Float, y: Float }" は次のように読む：
// 「Point は命題であり、その証明には Float の証明 x と Float の証明 y の同時提供が必要」
Point: Type = { x: Float, y: Float }
```
...
1. **論理的一貫性 = 型安全性**：もし型システムが型 `T`
   の値の構築を許しながら正当な実行時表現を持たないなら、それは論理において偽命題の証明を許すようなものだ——システムは崩壊する。Curry-Howard が教えてくれる：**型安全な言語は、本質的に一貫した論理システムである**。

2. **宇宙階層は必要条件**：後述するように、もし
   `Type: Type`（すなわち「型の型も型である」）を許せば、Russell パラドックス（型論では Girard パラドックスとして現れる）を生む。YaoXiang の
   `Type₀ : Type₁ : Type₂ : ...`
   階層は、各型が特定の階層にのみ属することを保証し、決して閉環しない上昇の連鎖を形成し、根本からパラドックスを回避する。これは YaoXiang の型システムが Curry-Howard 意味で
   **論理的に一貫している**ことを意味する。

3. **統一構文の理論的基礎**：`name: type = value`
   が一つの構文で変数、関数、型、インターフェース、ジェネリックのすべてをカバーできるのは、これらが Curry-Howard のもとですべて同じもの——**命題に証明を与えること**——だからである。変数は命題の証拠、関数は含意の証拠、記録は連言の証拠、ジェネリックは全称量化の証拠。統一構文は人為的に設計された偶然ではなく、Curry-Howard 同型の自然な帰結である。

> **参考文献**：Wadler, P. (2015). _"Propositions as Types."_ Communications of the ACM, 58(12),
> 75–84. この記事は Curry-Howard 同型の歴史と意義を平易な言葉で解説している。
...
> **設計理由**：`{ ... }`
> は依存駆動計算ユニット（後述）であり、その返却セマンティクスは単一式と異なる。中括弧は複数文のコンテキストを導入するため、「最後の式が戻り値か」という曖昧さを排除するために明示的な
> `return` が必要となる。
...
- 依存が満たされれば即座に実行され、欠落していればブロックして待機する
- `return` で明示的に値を返す（返却ルール参照）
...
> **単一式との違い**：`= expr`（中括弧なし）は直接値を返す単純なバインディング；`= { ... }`（中括弧あり）は依存駆動計算コンテキストを導入し、複数文と明示的な
> `return` を許可する。
...
- `spawn { ... }` 内の直接の子代入が自動的に並列タスクを生成する
- 依存が満たされたタスクは即座に並行実行される
- 呼び出し側はすべての子タスクの完了を待機してブロックする

```yaoxiang
result = spawn {
    a = fetch_data("url1")    # タスク 1
    b = fetch_data("url2")    # タスク 2（a と依存関係なし、並列実行）
    c = process(a, b)         # a, b に依存 → 両方完了後に実行
    return c
}
// 呼び出し側はここでブロック、spawn ブロック内すべてのタスクが完了するまで
```
...
> **詳細定義**：`unsafe` の完全なセマンティクス、FFI 型定義、メソッドバインドについては `ffi.md`
> を参照。
...
**デフォルト値を持つフィールド**：フィールドはデフォルト値を持て、構築時はオプション。
...
1. **キーワード**（パーサ独立トークン）— 制御構造と宣言キーワード、例：`if`、`match`、`pub`、`return`
2. **リテラル予約語**（パーサ独立トークン）—
   `true`、`false`、`void`、`Type`、通常の識別子としては使用不可
3. **内蔵型名**（型チェッカ事前登録）— パーサは通常の識別子として扱い、型チェッカが解釈。**予約語ではなく、シャドウ可能（非推奨）**

`void`（小文字、リテラル予約語）と `Void`（大文字、内蔵型名）の違い：`void`
は値リテラル（Unit の一意の値に等しい）、`Void`
...
| 型       | 論理的対応   | 説明                                                                                                                                                                                                                                   |
| -------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Never`  | ⊥（偽/空型） | ゼロコンストラクタ、いかなる値もこの型に住まない。「不可能」を表す——発散、panic、デッドコード。`Never <: T` は任意の `T` に対して成立（爆発原理）。関数が `Never` を返すのは正常終了しないことを示す。**キーワードではなく内蔵型名。** |
| `Void`   | ⊤（真/Unit） | ちょうど一つの住人（デフォルト void 値）。`x: Void = <デフォルト>` は合法。和型の単位元は積型の単位元に対応——`Void` はゼロフィールド積型（Unit）、`Never` はゼロ変体和型。                                                             |
| `Int`    | —            | 符号付き整数                                                                                                                                                                                                                           |
| `Float`  | —            | 浮動小数点数                                                                                                                                                                                                                           |
...
**`Type.name`
接頭辞は名前空間の帰属**を表すだけであり、それ以外には何の意味も持たない。暗黙のバインドは一切トリガーされない。
...
`p.draw(screen)` のような `.`
メソッド呼び出し構文を有効化するには、**明示的にバインドする必要がある**。 `[position]`
...
// 明示的バインド — これで p.draw(screen) 構文が使えるようになる
...
Point.draw = draw       // バインドしない：Point.draw(p, screen) のみ可
```
...
2. **Type は左、値は右**：`name: type = value`——Type 引数は左で宣言、右側は常に具体的な値。空コンテナ
...
// 明示的バインド — バインドしてはじめてドット呼び出し構文が使える
Point.draw = draw[0]
...
2. 右辺型がコンパイル時に決定不能な場合、vtable 機構にフォールバック
...
> **廃止宣言（2026-07-25、issue #203）**：`|` 変体構文を正式に廃止し実装から削除。
...
統一して記録型で和型（sum
type）を表現する。記録型のフィールドがすべて関数であり、かつすべてが自身の型を返す場合、それは和型である：
...
1. **特殊ケースの排除**：`|` は BNF における唯一の `name: type = value`
   形式でない構文。削除後、`type_expr`
   生成規則は完全に統一され、パーサは変体型のために独立したパスと先読みバックトラックを維持する必要がなくなる。
2. **数学的等価性**：Curry-Howard 同型のもとで、選言 P ⊕
   Q に対応する和型は、「フィールドがすべて自身を返す関数である」記録型と等価である。両者は同じセマンティクスを表現し、二つの構文は不要。
3. **破壊性ゼロ**：削除前の `|`
   構文はパーサで半サポート（引数なし変体は解析可能だが、引数型は単態化時に消失）で、依存するユーザコードは存在しない。
4. **AST 簡素化**：`Type::Variant(Vec<VariantDef>)` ノードを削除、すべての変体型は `Type::Struct`
   経路に統一され、下流の typecheck/mono/formatter の特殊分岐がすべて解消。

> **注**：和型のセマンティック属性（match の網羅性チェック、tagged
> union のメモリレイアウトなど）は、typecheck 層が `Type::Struct`
> 構造から推導する。独立した AST ノードに依存しない。

### 論理演算子：`and` / `or` / `!`（権威的定義、Zig 式）

> **定義宣言（2026-08-03、issue #251 改訂）**：論理演算子の権威的形式はキーワード `and` /
> `or` + 記号単項 `!` である（SPEC `syntax.md`
> §2.2 優先度テーブルと一致）。本設計は Zig に準拠する：**短絡制御フローにはキーワード、純粋な単項演算には記号**。初期実装で C に流れた
> `&&` / `||` や中間状態のキーワード `not` はすべて削除済み。
...
if x != 0 and y / x > 1 { ... }   # x == 0 のとき 0 除算は発生しない

# 密結合：!a == b ≡ (!a) == b（Zig 式；Python の not a == b ≡ not (a == b) と逆）
!3 == 4          # false：(!3) == 4 → false
!(3 == 4)        # true
!x != 0          # ≡ (!x) != 0
...
以下の記法は**もはやサポートされない**（字句解析器がエラーを報告し対応する記法を提示）：
...
**設計理由**（Zig に準拠、ziglang/zig#272 / #6625）：

1. **短絡は制御フロー → キーワード；純粋関数は演算 → 記号**。`and` / `or`
   は評価順序を変更し（必要に応じて右辺をスキップ）、`if` と同性質のためキーワード；`!`
   は評価済みオペランドに純粋な反転を適用し、`-` `+` と同性質のため記号。YaoXiang のエラー伝播は
   `?`（§2.11）を使い、`!` と衝突しない。
2. **密結合による曖昧さ排除**：`!`
   は視覚的にオペランドに「密着」し、高い優先度が一目瞭然；キーワード `not`
   はオペランドとの間にスペースを強要し、どちらに結合するか（`not a == b`）が脳内で曖昧さを生む。
3. **曖昧さ排除**：`&` は二つの役割——借用トークン（`&p` / `&mut p`、RFC-009）とビット AND（SPEC
   §2.2 優先度 8）——を担う。`&&` を導入すると一つの記号が三つの意味を持つことになる。`and` / `or` /
   `!` は借用、ビット演算、論理という三つの概念を視覚的に完全に分離する。
4. **先例**：Zig（同エコポジションの現代的システム言語）がまさに `and` / `or` キーワード + `!`
   記号の組み合わせ；Python / Lua / Ada / SQL は全キーワード（`not`
   を含む）、C ファミリーは全記号——YaoXiang は Zig のミックスを採用し、両者の利点を得る。
5. **Curry-Howard 一貫性**：型は命題（前述の同型節参照）、精錬型における論理結合は `and` / `or`
   と書く（例：`{ 0 <= idx and idx < arr.len }`）のが命題の自然な表現；`!`
   は単項否定記号として ¬ に対応する。

> **実装**：`and` / `or`
> は IR 層で短絡ジャンプ列に展開される（`a and b ≡ if a { b } else { false }`）、 `!`
> は単項密結合として解析される（オペランドは `BP_UNARY + 1` でパース）。回帰テスト：
> `tests/yaoxiang/01-syntax/basics/logical_ops.yx`、`logical_not.yx`。

## 構文設計説明：名前付き関数の本質は Lambda の構文糖衣
...
**要点**：シグネチャが引数型を完全に宣言しているとき、Lambda 頭部の引数名は冗長となり、省略可能。

### 引数スコープルール

**引数が外側の変数を覆う**：シグネチャ内の引数スコープは関数本体を覆い、内部スコープの優先度がより高い。
...
| 両方注釈       | `double: (x: Int) -> Int = (x) => x * 2` | ✅ 冗長だが許可 |
...
| 特性       | 利点                                                              |
| ---------- | ----------------------------------------------------------------- |
| **簡潔性** | シグネチャが完全なら引数名を書き直す必要なし                      |
| **柔軟性** | Lambda 形式を保持、好みのものを使える                             |
| **一貫性** | 変数宣言 `x: Int = 42` と統一パターンを維持                       |
| **直感性** | `name: Type = body` は「名前は name、型は Type、値は body」に対応 |
...
| 理論的エレガンス | 完全に対称的な `name: type = value`      |
| 新キーワード不要 | 既存の構文要素を再利用                   |
| 実装容易         | コンパイラは一つの宣言形式のみ処理       |
| 学習容易         | 一つのパターンを覚えれば全コードが書ける |
| 拡張容易         | 新機能がこのモデルに自然に統合可能       |

### 欠点

| 欠点     | 説明                                       |
| -------- | ------------------------------------------ |
| 命名規約 | メソッドは `Type.method` 命名に従う必要    |
| 冗長性   | 完全構文は簡略構文より長い、ただし推論可能 |
| 学習曲線 | 統一モデルの理解が必要                     |
...
| リスク         | 影響                               | 緩和策                   |
| -------------- | ---------------------------------- | ------------------------ |
| 解析複雑度     | 統一構文が解析を複雑化させる可能性 | 再帰下降パーサを使用     |
| パフォーマンス | vtable 検索に追加オーバーヘッド    | コンパイル時単態化最適化 |
...
> を正しく処理できない（Type0/Type1 宇宙パラドックスを引き起こす）が、我々は意図的にこの「隠し要素」を残している——コンパイルを試みると、言語の創始者からの禅的メッセージが届く。これは単なる技術的境界ではなく、YaoXiang からの型哲学へのオマージュである。
...
# ジェネリック引数：関数型の一部、例 (T: Type, R: Type) -> (...)
...
| メソッドバインド | `Type.name = func[n]`、func の位置 n を呼び出し側にバインドし `obj.name(args)` 構文を使えるようにする |
| ジェネリック関数 | `(T: Type)` 構文を使う関数、型引数を最初の引数グループとして持つ                                      |
