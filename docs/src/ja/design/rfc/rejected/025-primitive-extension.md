...
rejected: '2026-07-03'
reason:
  '業界の実務が証明するように、言語に汎用的なプリミティブ型登録機構は不要です。ドメイン型は標準ライブラリのstructラッパー、FFI不透明ハンドル（RFC-026
  §2.1）、またはコンパイラバックエンドの分岐で解決すべきです。本RFCは存在しない一般的な問題を解決しようとしており、is_copyフィールドはRFC-009の所有権セマンティクスを過度に単純化しています。'
---
