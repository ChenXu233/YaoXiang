...
> RFC（Request for Comments）はYaoXiang言語の機能設計提案の正式な提出形式です。
...
| [EXAMPLE_full_feature_proposal.md](EXAMPLE_full_feature_proposal.md) | 完全な例（パターンマッチ強化） |
...
| RFC-019  | [RFC-019: 型レベル同像性 (Typed Homoiconicity) - 構文がそのまま型](./draft/019-typed-homoiconicity.md)   | 晨煦 | 2026-02-20 | ドラフト   |
...
| RFC-039  | [RFC-039: パターンマッチの完全化](./draft/039-pattern-matching-completeness.md)                          | 晨煦 | 2026-09-03 | ドラフト   |
| RFC-027a | [RFC-027a: 停止検査の明示的な測度](./review/027a-termination-explicit-measure.md)                        | 晨煦 | 2026-09-14 | レビュー中 |
...
| 番号    | タイトル                                                                                             | 著者 | 作成日     | ステータス |
| ------- | ---------------------------------------------------------------------------------------------------- | ---- | ---------- | ---------- |
| RFC-032 | [RFC-032: spawn 統一式修飾 — spawn for の特殊ケースを排除](./review/032-spawn-unified-expression.md) | 晨煦 | 2026-06-16 | レビュー中 |
...
| RFC-008    | [RFC-008：Runtime 並行モデルとスケジューラの脱結合設計](./accepted/008-runtime-concurrency-model.md)                           | 晨煦      | 2025-01-05 | 承認済み           |
...
| RFC-029f   | [RFC-029f: コンパイルターゲットの役割とインポート面セマンティクス](./accepted/029f-target-semantics.md)                        | 晨煦      | 2026-09-12 | 承認済み           |
...
| RFC-022 | [RFC 022: Hoare 論理静的検証サポート（仕様注釈と仕様型）](./deprecated/022-hoare-logic-static-verification.md) | 晨煦 | 2026-03-16 | 廃止済み（RFC-027 に置換） |
...
| RFC-005 | [RFC-005: 自動 CVE セキュリティ検査システム](./rejected/005-automated-cve-scanning.md)              | 晨煦 | 2025-01-05 | 拒否済み   |
| RFC-016 | [RFC 016: 量子ネイティブサポートとマルチバックエンド統合](./rejected/016-quantum-native-support.md) | 晨煦 | 2026-02-13 | 拒否済み   |
| RFC-025 | [RFC-025: 拡張可能プリミティブ型機構](./rejected/025-primitive-extension.md)                        | 晨煦 | 2026-06-05 | 拒否済み   |
...
| **拒否済み**   | `rfc/rejected/`   | 拒否された RFC 文書                            |
...
**RFC 文書には正しい情報のみを含める。**
設計変更時は、原文を直接修正して現在の正しいセマンティクスを表現する；
**誤った内容を残したまま「正誤表」ブロックを追加して修正してはならない。**

「原文 + 正誤表」を残すのは最悪の書き方です：読者が途中まで読んで初めて前の内容がすべて無効だったと気づき、前の部分を読むコストが無駄になる上、廃止された段落を現行のセマンティクスとして誤って引用してしまう恐れがある。正誤表ブロックは一見慎重に感じますが、実際には整理コストを読者に転嫁しているだけです。

### 正しい方法

| 状況                                   | 方法                                                                                                 |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| 実装と原文が一致しない、原文が覆された | その段落を正しい内容に直接書き直し、元の記述を削除する                                               |
| サンプルコードが実行できなくなった     | 古い例を残さず、直接実行可能な形に修正する                                                           |
| 読者に「以前はどうだったか」を伝えたい | **意図的に誤りの比較を行う場合のみ**誤った内容を残し、直ちに「この書き方は誤りである」と理由を添える |
| 設計の変遷を追跡する必要がある         | Git のコミットメッセージや issue に書き、RFC の本文には書かない                                      |

### 例外

以下の誤った情報は保持できます：

- **意図的な対比教育**：正誤対照として明示的に記載し、誤り側には「なぜ誤りか」の説明を隣接させる
- **廃止済み RFC**（`rfc/deprecated/`）：歴史的記録として保持するが、何に置換されたかを明記する

### 補助手段

- RFC 先頭の `status` / `updated`
  フィールドは最新の改訂時間を反映し、本文に「今回の改訂内容」を書く必要はありません
- 実装状態は表の ✅ / ❌ で表現し、本文中に状態の説明を挟み込まないようにする
