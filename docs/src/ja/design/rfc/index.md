...
| RFC-030 | [RFC-030: assert アサーション機構](./draft/030-assert-mechanism.md) | 晨煦 | 2026-06-15 | ドラフト |
| RFC-031 | [RFC-031：最適化レベルと Pass マネージャー](./draft/031-optimization-levels.md) | 晨煦 | 2026-06-16 | ドラフト |
| RFC-032 | [RFC-032: spawn 統一式修飾 — spawn for 特殊ケースの除去](./draft/032-spawn-unified-expression.md) | 晨煦 | 2026-06-16 | ドラフト |
...
| 番号 | タイトル | 著者 | 作成日 | ステータス |
|------|------|------|----------|------|
| RFC-001 | [RFC-001：並作モデルとエラーハンドリングシステム](./deprecated/001-concurrent-model-error-handling.md) | 晨煦 | 2025-01-05 | 廃止（RFC-024 に置換） |
...
| RFC-022 | [RFC 022: ホーア論理静的検証サポート（仕様注釈と仕様型）](./deprecated/022-hoare-logic-static-verification.md) | 晨煦 | 2026-03-16 | 廃止（RFC-027 に置換） |
...
| RFC-005 | [RFC-005: 自動化CVEセキュリティチェックシステム](./rejected/005-automated-cve-scanning.md) | 晨煦 | 2025-01-05 | 拒否 |
| RFC-016 | [RFC 016: 量子ネイティブサポートとマルチバックエンド統合](./rejected/016-quantum-native-support.md) | 晨煦 | 2026-02-13 | 拒否 |
| RFC-025 | [RFC-025: 拡張可能なプリミティブ型機構](./rejected/025-primitive-extension.md) | 晨煦 | 2026-06-05 | 拒否（RFC-026 の不透明ハンドルに包含） |
...
| **レビュー中** | `rfc/review/` | コミュニティの議論とフィードバックを公開 |
| **承認済み** | `rfc/accepted/` | 正式な設計ドキュメントとなり、実装段階へ移行 |
| **廃止** | `rfc/deprecated/` | かつて承認されたが、新しい設計に置換された |
...
1. [RFC_TEMPLATE.md](RFC_TEMPLATE.md) を読み、フォーマット要件を理解する
2. [EXAMPLE_full_feature_proposal.md](EXAMPLE_full_feature_proposal.md) を参考に書き方を学ぶ
3. 新規ファイルを作成し、`番号-説明的なタイトル.md` という命名にする
