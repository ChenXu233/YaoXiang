---
title: 'RFC-019: 型レベル同像性 (Typed Homoiconicity) - 構文即ち型'
status: '草案'
author: '晨煦'
created: '2026-02-20'
updated: 'YYYY-MM-DD'
...
> **⚠️ 永久実験的宣言**: これは「構文即ち型」という言語設計理念の実行可能性を検証するための**探索的実験**です。**本 RFC は決してマージされません**。結果にかかわらず dev/main ブランチには入りません。実験ブランチは完了後に廃棄またはアーカイブされます。
>
> - **実験目標**: 型レベル同像性の実装難易度と潜在価値を検証する
> - **損切りライン**: 6ヶ月間進捗がない場合は放棄
> - **成功基準**: 少なくとも1つのユーザー定義キーワードを完走できる（解析→コンパイル→実行）
>
> **main ブランチへのマージを保証しません**。将来、様々な理由により拒否または放棄される可能性があります。本番環境ではこの機能を使用しないでください。
>
> **⚠️ 位置づけの説明**: 本 RFC は言語設計の思想実験でありエンジニアリング方案を提供しません。実用的な拡張可能パーサー模式が必要な場合は、Rust の `syn::Parse` または Haskell の `parsec` を参照してください。

---

## 摘要

本 RFC は以下の эксперимент，提出了一个激进的语言设计实验：**让语言的语法结构本身成为类型系统的一部分**。
...
| Phase 2 | 实现基本的求值器                  | 2 周     | 关键挑战：if/return 控制流         |
| Phase 3 | 实现 SyntaxRule 类型的解析规则    | 3 周     |                                    |
| Phase 4 | 用户自定义语法扩展                | 3 周     | 核心目标：至少跑通一个自定义关键字 |
| Phase 5 | 优化和文档                        | 2 周     | 实验结束                           |

**超时处理**：如果 Phase 2（控制流实现）超过 4 周无进展，应考虑放弃。
...
└──────┬──────┘
       │
       ▼
       ⚠️ 永久实验性分支 (exp/typed-homoiconicity)
...
> **⚠️ 重要提醒**: 这是一个探索性实验，**永不合并**。请勿在生产代码中依赖此特性。
