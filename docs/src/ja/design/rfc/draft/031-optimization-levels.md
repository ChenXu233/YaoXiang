...
| 日常開発 | 基本最適化、コンパイル速度とのバランス | O1 | | テスト/CI
| 標準最適化、本番動作の検証 | O2 | | 本番リリース | 深い最適化、极致なパフォーマンス | O3 |
...
| 最適化レベル | 実行時動作 | |--------------| | O0 | 最適化なし、すべてのデバッグ情報を保持 | | O1
| 基本最適化、基本的なデバッグ情報を保持 | | O2 | 標準最適化、デバッグ情報なし | | O3 |
gressive最適化、デバッグ情報なし |
