...
---
title: パッケージマネージャー
description: YaoXiang 公式パッケージマネージャーの使い方チュートリアル
---
...
|------|------|------|
| `name` | string | パッケージ名 |
| `version` | string | バージョン番号（省略可能、デフォルト `*`） |
...
- `vendor` ディレクトリが既に存在する場合、確認してキャッシュを再利用
...
1. 現在のロックバージョンをクリア
2. `vendor` ディレクトリ内の旧バージョンをクリーンアップ
3. すべての依存関係を再ダウンロード
4. `yaoxiang.lock` を更新

### 単一更新

パラメータありの場合、指定した依存関係のみ更新：
...
#   http (1.0.0 → 1.1.0)
#   json (2.0.0 → 2.1.0)
# ✅ 2件の依存関係を更新、ロックファイル更新済み

# 単一の依存関係を更新
yaoxiang update http

# 出力
# ✅ http を更新 (1.0.0 → 1.1.0)
```
...
プロジェクトメタデータと依存関係を宣言するプロジェクトマニフェストファイルです。
...
パッケージマネージャーによって自動生成される依存関係ロックファイルです。
...
### ランタイム依存関係 vs 開発用依存関係

- **ランタイム依存関係** (`[dependencies]`)：プロジェクト実行時に必須のパッケージ
- **開発用依存関係** (`[dev-dependencies]`)：開発・テスト時のみ必要なパッケージ
...
- チームメンバーが完全に同一の依存関係バージョンを使用することを確保
- CI ビルドの再現性を確保
- 「自分の環境では動く」問題を回避

### vendor ディレクトリ

依存関係のダウンロード先は `vendor` ディレクトリです：

- `yaoxiang install` と `yaoxiang update` によって自動管理
- 削除后再び `install` を実行すると再構築可能
- `.gitignore` に追加推奨、チームメンバーが個別に管理
...
YPM は依存関係バージョンの競合を検出してエラー表示します。解決策：
...
できます。削除後 `yaoxiang install` を実行すると、すべての依存関係を再ダウンロードします。

### Q: 特定のパッケージ情報を確認するには？

`yaoxiang list` で全依存関係を確認するか、`yaoxiang.toml` を確認してください。
```