...
Loop）はインタラクティブなコード実行環境で、YaoXiang コードを行ごとに入力して実行することができます。学習、テスト、デバッグに非常に適しています。
...
シンボル `x` の型情報を確認します。短縮形 `:t` も使用できます。
...
現在の REPL で定義されているすべてのシンボル（変数と関数）を一覧表示します。短縮形 `:i` または
`:info` も使用できます。
...
REPL は複数行コードの入力をサポートしています。コードが不完全な場合（閉じられていない括弧など）を検出すると、自動的に継続行モードに入ります：
...
REPL はインテリジェントな自動補完機能を提供し、コードの迅速な入力をサポートします。
...
エラーは REPL セッションを終了させないので、新しいコードを入力し続けることができます。
...
REPL の状態が乱れたときは、`:clear` を使用してリセットします：
...
最初の数文字を入力した後 Tab を押して、変数名と関数名をすばやく補完します。
...
- `Bool`：ブール値
- `Void`：空型
- カスタム record type と value variant
