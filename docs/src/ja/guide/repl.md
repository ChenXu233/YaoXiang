...
Loop）は、YaoXiang コードを行単位で入力・実行できるインタラクティブなコード実行環境で、学習、テスト、デバッグに最適です。
...
YaoXiang には `fn` キーワードがありません。関数はシグネチャを持つ単なる値です：
...
REPL は複数行コードの入力をサポートしています。コードが不完全であることが検出された場合（例：閉じられていない括弧）、自動的に継続行モードに入ります：
...
エラーが発生しても REPL セッションは終了せず、新しいコードを引き続き入力できます。

### 履歴

REPL はコマンド履歴を自動的に保存し、以下をサポートします：

- **上下矢印キー**：履歴コマンドの参照
- **検索**：一部の内容を入力後、上下矢印キーで検索
...
最初の数文字を入力してから Tab を押し、変数と関数名を迅速に補完します。
...
### Q: 長時間実行されるコードを中断するにはどうすればよいですか？
