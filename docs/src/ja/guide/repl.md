...
YaoXiang REPL（Read-Eval-Print Loop）はインタラクティブなコード実行環境であり、YaoXiang コードを行ごとに入力・実行することができます。学習、テスト、デバッグに非常に適しています。
...
または `yaoxiang` を直接実行します（サブコマンドなし）：
...
プロンプト `>>` の後に YaoXiang コードを入力し、Enter キーで実行します：
...
REPL は有効な YaoXiang 式なら何でも実行できます：
...
YaoXiang には `fn` キーワードがなく、関数はシグネチャ付きの値です：
...
REPL は複数行コードの入力をサポートしています。コードが不完全であることが検出された場合（閉じられていない括弧など）、自動的に継続行モードに入ります：
...
REPL はスマートな自動補完機能を提供し、コードの迅速な入力をサポートします。
...
1. **キーワード補完**：YaoXiang 言語キーワード（Tab を押すと展開）
2. **シンボル補完**：定義済みの変数と関数名
...
エラーは REPL セッションを終了させないので、新しいコードを引き続き入力できます。
...
最初の数文字を入力した後、Tab を押して変数と関数名を迅速に補完します。
...
A: 閉じられていない括弧、引用符、中括弧がないか確認してください。REPL は完全なコード入力を待ちます。
