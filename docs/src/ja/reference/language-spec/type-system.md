...
Curry-Howard 同型（Curry-Howard correspondence）は YaoXiang 型システムの理論的基礎である。これはプログラミング言語の型システムと数理論理の間の深層対応を明らかにする：
...
- **型検査は証明の検証である**。あるプログラムが型検査を通過するとは、論理的命題が構成的に証明されたことに相当する。
- **停止する型レベル計算は正しい帰納的推論に対応する**。YaoXiang の型族（例えば `Nat` 上の `Add` のパターンマッチング）は本質的に数学的帰納法の型レベル符号化である。

### 0.3 言語設計への影響

Curry-Howard 同型の YaoXiang における具体的な体現：

1. **宇宙階層**（RFC-010）：`Type₀ : Type₁ : Type₂ …` は `Type: Type` に起因する論理的パラドックス（Girard のパラドックス）を回避する
2. **型族**（RFC-011）：自然数 `Nat(Zero/Succ)` の型レベルパターンマッチングは Peano 公理下の帰納的証明に対応する
3. **条件型**（RFC-011）：`If: (C: Bool, T: Type, E: Type) -> Type` は論理における case による選言に対応する
4. **値依存型**（RFC-011）：`Vec: (n: Int) -> Type` は「各整数 n に対し一つの型が存在する」という有界量化に対応する
...
> これは暗黙的なバインディングを引き起こさない。`p.draw()` のような `.` 呼び出し構文を有効にするには、明示的なバインディングが必要である：
> `Point.draw = draw[0]`。
> 詳細は RFC-004 および RFC-010 を参照。

#### 3.1.1 フィールドのデフォルト値

型フィールドにはデフォルト値を指定でき、構築時には任意の指定が可能：
...
ジェネリクスパラメータは関数型の一部であり、通常のパラメータと統一的に `()` 構文を用いる：
...
ジェネリック関数において、型パラメータも同様にシグネチャで宣言され、コンパイラが実引数から自動的に推論する：
...
Container: (T: Type) -> Type = {
    Item: T,
    IteratorType: Iterator(T),  // 関連型もジェネリックになり得る
    iter: () -> IteratorType
}
```
...
**核心設計**：`(n: Int)` ジェネリクスパラメータ + `(n: n)` 値パラメータにより、コンパイル時定数と実行時値を区別する。
...
YaoXiang には区別すべき型属性が一つだけある：線形性 vs 複製可能性。これはコンパイラにより自動推論される。
...
### 11.2 Dup（浅いコピー：ハンドルの複製、データの共有）

**Dup 属性は参照/トークン型に使用される**。Dup 型の代入 = 浅いコピー（ハンドル/トークンの複製、基底データを共有）。複数の所有者が同一のデータを指す。

| 型 | 属性 | 説明 |
|------|------|------|
| `&T` | Dup | ゼロサイズの読み取りトークン、トークン複製 = 同一データを指す複数の視点 |
| `ref T` | Dup | Rc/Arc の複製 = 参照カウント+1、ヒープデータの共有 |
| `&mut T` | Linear | ゼロサイズの書き込みトークン、独占、コピー不可 |
| 他のすべての型 | Move | デフォルトの所有権移転 |

**プリミティブ値型**（Int, Float, Bool, Char）はコンパイラ組み込みの特殊処理である：代入時に自動的に値がコピーされ、二つの値は完全に独立である。これはコンパイラのネイティブ動作であり、Dup 型属性には含まれない。
...
**Clone** は明示的な深いコピーインターフェースである。すべての型は Clone を実装でき、`.clone()` メソッドを提供する。
...
| | Dup | Clone |
|---|---|---|
| **セマンティクス** | 浅いコピー：ハンドル/トークン複製、基底データ共有 | 深いコピー：完全で独立した複製を作成 |
| **呼び出し方式** | 暗黙的（代入/引数渡しで自動） | 明示的（`.clone()`） |
| **変更の影響** | 相互に影響（基底データ共有） | 互いに影響しない（独立した複製） |
| **適用型** | `&T` トークン、`ref T` | Clone インターフェースを実装する任意の型 |
| **コスト** | ゼロオーバーヘッド（トークンはゼロサイズ型） | 型による |

**Dup は Clone を蕴含せず、Clone は Dup を蕴含しない**—これらは直交する二つの概念である：
...
**設計意図**：
- Dup はトークン/参照型に使用され、「同一データを複数の視点で見る」問題を解決する
- Clone は独立した複製が必要なシナリオに使用され、明示的な呼び出しによりコストを可視化する
...
```
&T      →  ゼロサイズ、Dup（複製可能）、読み取り権限を付与
&mut T  →  ゼロサイズ、Linear（Dup ではない）、排他的読み書き権限を付与
```
...
// 複数の &T トークンの共存—Dup 型は自由なコピーを許可する
distance: (a: &Point, b: &Point) -> Float = {
...
トークンは通常の型であるため、すべての通常の型操作をサポートする：
...
**クロージャによるキャプチャ**—クロージャは他の値をキャプチャするのと同じようにトークンをキャプチャする：
...
```
1. 実引数が後に使用される場合 → トークン作成を優先（メソッドシグネチャに従い &T または &mut T）
...
コンパイラはトークン値に対して**フロー敏感的生存解析**を行い、各トークンの状態（生存/移動済み）を追跡する：
...
// ❌ 同一の実引数から同時に &mut トークンと他のトークンを作成できない
...
ユーザはブランドに触れることがない。コンパイラは内部で各トークンにコンパイル時一意識別子を割り当てる：
...
- **競合検出**：同源の WriteToken と派生 ReadToken は同時に生存できない
...
| エスケープ | 可能（トークンは戻り値/構造体/クロージャで伝播） | 元々エスケープ用 |
| タスク間 | 不可（トークンはタスク間渡し未実装） | 可能（コンパイラが自動 Arc 選択） |
...
// === Dup（浅いコピー：ハンドル複製、基底データ共有） ===
&T              // ゼロサイズ読み取りトークン、トークン複製 = 同一データを指す複数の視点
...
// ❌ タスク間をまたぐことはできない（トークンはタスク間渡し未実装）
```