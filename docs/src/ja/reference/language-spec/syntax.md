...
本ファイルでは、YaoXiang プログラミング言語の構文仕様を定義しており、字句構造、構文規則、演算子の優先順位を含みます。
...
これらのキーワードはどのようなコンテキストにおいても特別な意味を持ち、識別子として使用することはできません。
...
コードは4スペースのインデントを使用しなければならず、タブ文字の使用は禁止されています。これは強制的な構文規則です。
...
`ref` は共有所有権を作成します。コンパイラが Rc（単一タスク）または Arc（タスク間）を自動的に選択し、ユーザーが実装の詳細を気にする必要はありません。
...
YaoXiang の for ループのセマンティクスは従来の言語とは異なります：**各イテレーションは新しい束縛であり、同じ変数を変更するのではありません**。
...
| 第2回 | 新しい束縛 `i = 2` を作成し（以前の束縛は破棄済み）、ループ本体を実行、2 を出力 |
| 第3回 | 新しい束縛 `i = 3` を作成し、ループ本体を実行、3 を出力 |
| 第4回 | 新しい束縛 `i = 4` を作成し、ループ本体を実行、4 を出力 |
| 終了 | ループ本体が終了し、束縛が破棄 |

**重要なポイント**：各イテレーション終了後、そのイテレーションで作成された束縛は破棄されます。次のイテレーションは前回のイテレーションの束縛とは無関係な、全く新しい束縛です。
...
YaoXiang は変数のシャドーイングを禁止しています。for ループ変数は外側のスコープの変数と同じ名前にできません：
...
この規則はすべてのコードブロックに適用されます。[4.3 シャドーイング規則](./modules.md#43-シャドーイング規則)を参照してください。

#### 3.9.4 他の言語との比較

| 言語 | for ループ変数のセマンティクス |
|------|------------------|
| YaoXiang | 各イテレーションが新しい束縛 |
...
1. **より自然なセマンティクス**
   自然言語では「集合内の各要素 x について」とは、各 x が独立した存在であることを意味します。YaoXiang の `for i in 1..5` は「1 から 5 の中の各 i について」と読み、各イテレーションの i は全く新しい束縛であり、人間の直感的な理解と一致します。

2. **偶発的な変更を避ける**
   デフォルトで不変の束縛セマンティクスにより、ループ本体内でループ変数を偶発的に変更することがなくなります。複雑なループ本体内でうっかり `i = ...` と書いて追跡困難なバグが発生する心配がありません。

3. **高性能な解決策が簡単に得られる**
   イテレーション間で変数を再利用する必要がある場合（例：アキュムレータ、キャッシュ）、`for mut` 宣言を使用するだけで可変束縛モードに切り替えることができます。これは暗黙の共有状態よりも明確です—意図が構文で明示的に表現され、実行時の動作に隠されません。
...
**spawn ブロック**：并发疆域を明示的に宣言し、ブロック内の式并发に実行されます。
