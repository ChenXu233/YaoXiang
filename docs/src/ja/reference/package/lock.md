---
title: 'yaoxiang.lock 形式'
description: 依存関係ロックファイル形式の説明
---

# yaoxiang.lock 形式

`yaoxiang.lock` は YaoXiang の依存関係ロックファイルで、すべての依存関係の正確なバージョン情報を記録します。

## 概要

- **自動生成**：`yaoxiang install` と `yaoxiang update` によって自動的に生成・更新されます
- **手動編集禁止**：このファイルはパッケージマネージャーによって自動的に管理されます
- **バージョン管理に含める**：チームメンバーと CI ビルドが同じ依存関係バージョンを使用することを確保します
...
- `yaoxiang.toml`：**欲しい**依存関係を宣言（範囲指定可以使用）
