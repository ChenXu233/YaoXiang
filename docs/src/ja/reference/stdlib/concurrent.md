...
description: 'スリープ、スケジューリングの譲与、スレッド識別'
---
...
> 本モジュールはオペレーティングシステムのスレッドに依存するため、`wasm32`
> ターゲットでは**エクスポートされません**。
...
- `millis` —— スリープするミリ秒数。`Int` でない、または省略された場合は `0`
  として扱われます（エラーは発生しません）。
...
戻り値：`ThreadId(1)`
形式の文字列です。具体的な値はプラットフォームやスケジューリングによって変化するため、**存在確認のみに使用し**、その具体的な内容や形式に依存してはいけません。
...
現在のスレッドのスケジューリング時間枠を自発的に譲与し、他のスレッドに実行の機会を与えます。
...
- [言語仕様：並行モデル](../language-spec/concurrency.md) —— `spawn` と並作（spawn）のセマンティクス
