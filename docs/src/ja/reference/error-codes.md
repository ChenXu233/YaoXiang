...
YaoXiang コンパイラはエラーコードを使用してさまざまな種類の診断情報を識別します。エラーコードは番号範囲ごとにグループ化され、各エラーコードは特定のエラーシナリオに対応します。

---

## E0xxx -- 字句解析と構文解析

字句解析器（Lexer）と構文解析器（Parser）段階で生成されるエラーです。
...
型チェック段階で生成されるエラーで、変数の型、関数呼び出し、パターンマッチング、generics のインスタンス化、並行セマンティクス、エラー伝播などをカバーします。
...
| E1051 | `Logical NOT requires boolean operand, found '{type}'` | 論理 NOT にブールオペランドが必要 |
...
意味解析段階で生成されるエラーで、スコープ、変数のライフタイム、所有権、関数シグネチャの解析などをカバーします。
...
| E2095 | `Invalid signature: parameter '{name}' shadows generic` | ジェネリックをシャドーイングするパラメータ名 |

## E4xxx -- ジェネリックと Trait

ジェネリック制約と Trait システムに関連するエラーです。
...
| E6002 | `Null pointer dereference at {location}` | null ポインタのデリファレンス |
| E6003 | `Array index out of bounds: valid range is 0..{max}, found {index}` | 配列インデックス範囲外 |
...
| E7002 | `Permission denied: '{path}'` | アクセス拒否 |
| E7003 | `I/O error: {reason}` | I/O エラー |
| E7004 | `Network error: {reason}` | ネットワークエラー |

## E8xxx -- 内部コンパイラエラー

コンパイラの内部エラーで、通常はコンパイラ自体のバグを示します。この種のエラーに遭遇した場合は [GitHub Issues](https://github.com/yaoxiang/yaoxiang/issues) で報告してください。
...
| W1063 | `Const generic constraint not evaluable at compile time` | const ジェネリック制約はコンパイル時に評価できない |
