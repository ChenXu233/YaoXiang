...
**非常重要！！！！絶対に忘れないでください！！！** すべての提交メッセージは次の形式に従います：
...
> ⚠️ **重要**:
> emoji 文字を直接入力せず、**emoji コード**（例：`:sparkles:`）を使用する必要があります。
>
...
スコープはプロジェクトの `src/`
ディレクトリ構造に基づいており、**次の定義済み scope を使用する必要があります**：
...
バージョン番号はプロジェクトのルートディレクトリ `Cargo.toml` の `version`
フィールドで定義されています：
...
> のバージョン番号を更新し**、PR を通じて main にマージすると CI が自動的に tag と Release を作成します。**tag を手動でプッシュしないでください**。さもなくば CI が release フローをスキップします。
