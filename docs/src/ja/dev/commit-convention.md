...
**非常重要！！！！絶対に忘れないでください！！！** すべてのコミットメッセージは以下の形式に従います：
...
> ⚠️ **重要**: 直接的な emoji 文字ではなく、**emoji コード**（例：`:sparkles:`）を使用する必要があります。
>
...
バージョン番号はプロジェクトのルートディレクトリにある `Cargo.toml` の `version` フィールドで定義されます：
