...
従来の連結方式と比較すると、f-string の優位性が一目瞭然です：
...
println(f"Pi: {pi:.2f}")   # Pi: 3.14（小数点以下2桁）
println(f"Pi: {pi:.4f}")   # Pi: 3.1416（小数点以下4桁）
```
...
リテラルの `{` や `}` を出力する必要がある場合は、**二重に記述**するだけです：
...
コンパイラが f-string を検出すると、効率的な文字列連結へと変換します：
...
これは、f-string が書きやすく簡潔なだけでなく、ランタイムパフォーマンスも手書きの連結と同等であることを意味します——**ゼロオーバーヘッド**です。
