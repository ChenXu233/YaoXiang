...
データ構造はプログラムの骨格です。YaoXiangは2つの組み込みコレクション型を提供しています：リストと辞書です。

## リスト

リストは**順序付け**された値のシーケンスで、すべての要素は同じ型です。`[]` を使って作成します：
...
`[]` を使って位置で要素にアクセスし、インデックスは0から始まります：
...
リスト内包表記はリストを作成する強力なツールで、既存のリストから新しいリストを生成します：
...
辞書は**キーと値のペア**のコレクションで、キーは文字列、値は任意の型にできます。`{}`
を使って作成します：
