...
| 规范 | 状态 | 差异说明 |
|------|------|----------|
| §2.2 换行策略优先级 | ✅ 已实现 | 已实现完整的优先级链 |
| §4.2 参数列表换行（尾随逗号） | ✅ 已实现 | 参数列表超过行宽时自动换行，使用尾随逗号 |
| §6.2 单行代码块 | ✅ 已实现 | 单语句代码块且不超过行宽时使用单行格式 |
| §6.3 空代码块 | ✅ 已实现 | 空代码块输出 `{}` 而非 `{\n}` |
| §8.3 单引号模式 | ✅ 已实现 | `single_quote` 配置已生效 |
| §E2 退出码 | ✅ 已实现 | 已按规范使用特定退出码 |

---

## 测试覆盖

**77 个测试 + 1 个 proptest 幂等性测试**：

| 测试组 | 数量 | 覆盖内容 |
|--------|------|----------|
| handlers/tests/expr | 50 | 字面量、二元运算、函数调用、列表、字典、return、cast、match、f-string、try、unsafe、语法错误容错、单引号模式、空代码块、单行代码块、参数列表换行、**变量引用、return/break/continue、元组、索引访问、字段访问、ref、错误恢复** |
...
|------|------|------|
| 未完成事项 | 0 | — |
| 测试覆盖 | 良好 | 77 个测试 + 1 个 proptest，覆盖所有 Expr 变体 |
