# 编译期计算引擎（CTE Engine）与霍尔逻辑静态验证 — 实现计划

> **任务**：实现编译期求值引擎 + 霍尔逻辑静态验证通道，支撑值依赖类型和编译期维度验证  
> **基于 RFC**：RFC-010（统一类型语法）、RFC-011（泛型系统/值依赖类型）、RFC-022（霍尔逻辑静态验证）  
> **日期**：2026-05-10  
> **状态**：设计中  
> **目标里程碑**：
> - M1：常量折叠 + 纯度分析骨架
> - M2：纯函数编译期求值 + 终止检查
> - M3：类型级计算（`If`/`Assert`/`match` 类型族）
> - M4：霍尔逻辑验证（`//!` 规约解析 → VC 生成 → SMT 对接）

---

## 摘要

YaoXiang 的值依赖类型（RFC-011）要求类型可以依赖编译期已知的值（如 `Vec(factorial(5))` → `Vec(120)`），而霍尔逻辑静态验证（RFC-022）要求对纯函数进行编译期规约检查。两者共享同一核心需求：**在编译期安全地执行/分析纯函数**。

本计划提出**统一编译期计算引擎（CTE Engine）**，将纯度分析、终止检查、表达式求值抽象为公共基础设施，分别服务于类型级求值和霍尔逻辑验证两个消费者。

---

## 核心设计原则

1. **复用所有权系统做纯度分析**：YaoXiang 的 `&mut` 就是副作用标记——借用检查已经告诉我们什么会被修改
2. **纯函数即编译期可求值**：不加 `const fn` 关键字，编译器自动推断纯度
3. **终止证明 = 类型安全的基石**：类型位置的求值必须证明终止（`decreases` 规约），否则类型系统不可判定
4. **部分求值优于全量求值**：已知多少参数就算多少，算不出来的留到运行时
5. **编译期求值和霍尔逻辑共享解释器**：同一个表达式求值核心支撑两类消费者
6. **双模式求值**：具体求值（已知参数 → 产出 `CTValue`）和符号求值（未知参数 → 产出 SMT 表达式）共享同一个解释器框架，仅求值环境不同
...
src/middle/
├── cte/                          # 编译期计算引擎
│   ├── mod.rs                    # CTE 入口，协调三个子系统
│   ├── value.rs                  # CTValue 定义 + 基本操作
│   ├── eval.rs                   # 统一 AST 遍历器（Interpreter trait + eval_ast）
│   ├── concrete.rs               # 具体求值实现（ConcreteInterpreter → CTValue）
│   ├── symbolic.rs               # 符号求值实现（SymbolicInterpreter → SMTExpr）
│   ├── env.rs                    # EvalEnv（求值环境 + 步数限制）
│   ├── purity.rs                 # 纯度分析器
│   ├── termination.rs            # 终止检查器（decreases 验证）
│   ├── type_level.rs             # 类型级计算（If/Assert/match 类型族）
│   └── cache.rs                  # 求值结果缓存
│
├── verification/                 # 霍尔逻辑静态验证
│   ├── mod.rs                    # 验证入口
│   ├── spec_parser.rs            # //! 规约解析
│   ├── spec_types.rs             # 内置规约类型定义
│   ├── vcgen.rs                  # 验证条件生成（WP 演算）
│   ├── smt.rs                    # Z3 SMT 求解器接口
│   └── counterexample.rs         # 反例格式化
│
└── passes/
    └── mono/                     # 现有单态化（增强 CTE 集成）
        └── ...                   # 使用 CTE 结果进行实例化
```
...
| 决策 | 选项 | 选择 | 理由 |
|------|------|------|------|
| 纯度判定方式 | 显式标注 vs 自动推断 vs 两者结合 | **自动推断** | 所有权系统已提供足够信息；不提供显式标注逃生舱 |
| 编译期求值器 | 受限子集 vs 完整语言 | **完整语言（带步数限制）** | 与统一类型语法的"一切皆 `name: type = value`"一致 |
| 终止证明 | 强制标注 vs 自动推断 | **类型位置强制，其他自动** | 类型位置不可判定 = 编译错误，其他地方可以宽松 |
| VC 生成 | WP 演算 vs SP 演算 | **WP 演算** | WP 更简单直接，错误定位更清晰 |
| SMT 求解器 | Z3 vs CVC5 vs 自研 | **Z3** | 最成熟、Rust binding 最完善、社区最大 |
| 缓存策略 | 无缓存 vs 跨模块缓存 | **LRU 缓存 + 增量失效** | 编译期求值结果是确定性纯函数，天然适合缓存 |
| 编译模式 | 统一模式 vs Debug/Release 分离 | **Debug 验证 → Release 零开销** | 验证成本高，Release 不应承担 |
...
| 风险 | 影响 | 缓解 |
|------|------|------|
| Z3 集成复杂 | Phase 4 延迟 | 使用成熟的 `z3` crate；先支持简单算术，逐步扩展 |
| 编译期求值超时 | 用户体验差 | 步数限制 + 清晰的超时错误信息 + 建议简化表达式 |
| 纯度误判 | 编译期求值结果与运行时不一致 | 所有权系统提供强保证；若误判则是编译器 bug，应修编译器 |
| SMT 验证失败难以调试 | 用户不理解为什么规约不成立 | 反例提取 + 具体输入值展示 + 执行路径高亮 |
| 编译时间显著增加 | CI 变慢 | 增量验证 + 模块级缓存 + 验证结果文件（类似 `.o` 文件） |
