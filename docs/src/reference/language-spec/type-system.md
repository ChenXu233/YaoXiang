...
> **勘误**：原文称编译期值参数「默认编译期确定」，该表述**严格错误**——
> `add: (a: Int, b: Int) -> Int = a + b` 中 `a`/`b`
> 是运行时值参数。只有**在类型位置被引用**的具体类型参数才是编译期值参数。正确定义见下。
...
**方法接收者跟随签名语义**（勘误 2026-08-30，RFC-011a 接收者拼写约定同款）：接收者是 `&T`
→ 只读借用令牌；`&mut T` → 可变借用令牌；按值 →
