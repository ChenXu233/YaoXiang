...
> **字典字面量要求至少一个键值对**：`{}` **不是**空字典——它是空块（值 `Void`，见
> [§2.9](#_2-9-块表达式)）。空字典请用构造子 `dict.new()`：
>
> ```yaoxiang
> empty = dict.new()                  // ✅ 空字典
> d = { "a": 1 }                       // ✅ 字典字面量
...
> ```
>
> **判据是内容**：`Dict` 文法要求至少一个 `String ':' Expr`，`{}`
> 无内容可依据，故取块结构的零形态。非空形态由内容自描述（`{ "k": v }` 有键值对 → 字典）——这与
> `f = { 5 }` 是 `Int` 值而非函数同源：**类型由内容决定**。
...
命名实参用 `name = value`（RFC-010 §函数定义、RFC-011
§构造形式）。位置实参必须在命名实参之前；顺序指定的参数在命名实参中可任意排列：
...
命名实参写错名字报 **E1014**，同一形参被位置与命名同时指定报 **E1015**，个数不符报
**E1010**（RFC-013）。
...
> **为何 `{}` 不是空字典**：空字典的“空”无法自描述（它既可以是 `Dict(K, V)`
> 也可以是空块），而字典文法 [§1.6.4](#_1-6-4-集合)
> 要求至少一个键值对。无内容可依据时取块结构的零形态：这与 `unsafe {}` / `spawn {}`
> 一致，不引入特例。空字典用 `dict.new()`。
>
> **为何函数需要注解**：`f = { stmt }`
...
`main` 在此模式下**不特殊**——它只是一个普通绑定。想让 `main` 跑，必须显式调用：
...
> **为何不自动调用 `main`？** 顶层语句已经是程序主体。若再隐式调用 `main`，显式写了 `main()`
> 的脚本会执行两次。两种规则不能共存，故 Script 下只有「顶层语句」一个执行入口。
