...
> **行为收紧（迁移记录）**：迭代变量文法本就是 `'for' Identifier 'in'`，
...
> `in` 是二元关系运算符，返回 `Bool`——命中 `true`、未命中
...
> （Set 从右操作数列表除名——Set 无运行时表示，见 §1.6.4）
...
> 动态 `c` 运行时零检查（E6001 家族；错误系统落地后升格 Result）。
...
- **只出最近一层**：`break` 恒作用于包含它的最内层循环。嵌套循环中需要一次跳出多层时，
  把内层循环提取为函数用 `return` 返回，或使用标志位（break/continue 不带标签；
