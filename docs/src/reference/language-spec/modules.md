...
x = value:
  沿作用域链向外查找 x
    ...
    → 找到 x（已 moved）   ：视为"未找到有效绑定"，在当前作用域重新声明
...
> **块是真作用域**：「沿作用域链查找」的前提是每个 `{}` 块确实建立一层——
> 内层新声明的名字**不泄漏到外层**（见 §2.15）。
...
```
赋值优先查找的"已 moved"分支：
  x 在当前作用域存在，但处于 moved 状态
    → 编译器视为"未找到有效绑定"
    → 在当前作用域重新声明 x（覆盖旧的 moved 槽位）
```

**核心机制：** 旧值被消耗后，绑定失效，名字回归"可声明"状态。这不是遮蔽——旧绑定已经不存在了。

```yaoxiang
// 管道式数据流：每一步消耗旧值，产生新值
data = fetch()           // 不可变，持有所有权
data = transform(data)   // move data → 旧 data 失效，新 data 重新绑定
data = filter(data)      // 同上
process(data)
...
data3 = filter(data2)     // data2 被 move，不可再用
process(data3)
```
