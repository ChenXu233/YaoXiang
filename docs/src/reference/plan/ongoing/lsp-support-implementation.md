...
**测试项目**：
- [ ] didOpen/didChange/didClose 完整流程测试
- [ ] 增量变更测试
- [ ] 多文档管理测试
- [ ] 并发变更测试
...
- [ ] 所有 17 个关键字出现在补全列表
- [ ] 7 个保留字出现在补全列表
- [ ] 2 个函数注解（@block, @eager）出现在补全列表
- [ ] 关键字按类别正确分类（关键字/保留字/注解）

**测试项目**：
- [ ] 关键字补全测试（pub, use, spawn, ref, mut, if, elif, else, match, while, for, return, break, continue, as, in, unsafe）
- [ ] 保留字补全测试（Type, true, false, void, some, ok, err）
- [ ] 函数注解补全测试（@block, @eager）
- [ ] 上下文相关关键字测试（如 if/elif/else 成组出现）
...
- [ ] 当前文件符号可补全
- [ ] 导入模块符号可补全
- [ ] 补全项包含 kind 信息（keyword, function, variable, type）
