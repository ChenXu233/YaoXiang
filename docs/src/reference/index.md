...
> #299：容器类型（`List(T)` / `Vec(T)` / `Array(T, N)` / `Dict(K, V)`）
> 不是内置原语——它们是泛型类型构造器，与用户自定义泛型同一待遇，
> 经由统一的泛型实例化路径处理。字面量语法（`[...]` / `{...}`）保留在核心，
> 落点由上下文注解决定。Set 已除名（#300），详见 [语言规范](language-spec/syntax.md)。
>
> 三个容器概念按长度信息归属区分：`Array(T, N)` 长度在类型中（定长）、
> `Vec(T)` 长度是运行时值（原始缓冲原语）、`List(T)` 是标准库类型
> （`{ data: Vec(T), length: Int }`，策略全在库）。
...
|        | `unary -` 一元负号     |
|        | `* / %` 乘除取模       |
|        | （运算符的优先级与结合性是语言固定的；`+ - * / %` `== !=` `[]` 的语义可由实现对应接口（RFC-011b）的类型重载） |
|        | `+ -` 加减             |
|        | `== != < > <= >=` 比较 |
|        | `and or` 逻辑运算      |
| 最低   | `=` 赋值               |
