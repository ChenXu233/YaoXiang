...
│   ├── 代码块无 return → 默认返回 Void ✅
│   └── 无法推断 → 拒绝 ❌
│
└── 完全无法推断 → 拒绝 ❌
```

**注意**：代码块内无 `return` 时默认返回 `Void`。例如：
- `() => { 42 }` 推断为 `() -> Void`（块无 return，默认返回 Void）
- `() => { return 42 }` 推断为 `() -> Int`（有 return，从 return 推断）
- `() => 42` 推断为 `() -> Int`（表达式形式，直接返回值）
...
// 代码块无 return
no_return = (x: Int) => { x } // ✓ 推断为 Void（块无 return 默认返回 Void）
...
// === 零参数构造器（枚举风格）===
Color: Type = { red | green | blue } // 等价于 red() | green() | blue()

// === 多参数构造器（结构体风格）===
Point: Type = { x: Float, y: Float } // 构造器就是类型
...
get_num: () -> Int = 42
get_num2: () = 42 // 返回类型可推断
get_void = () => { 42 } // ✓ 推断为 Void（块无 return 默认返回 Void）
