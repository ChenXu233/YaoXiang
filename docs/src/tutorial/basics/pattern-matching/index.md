...
println(handle(a))  # 成功！得到的值是: 42
println(handle(b))  # 出错啦: 连接超时
```
...
println(describe(some(100)))  # 有值: 100
println(describe(none))       # 什么也没有
```
...
println(describe_point(Point(x: 3.0, y: 4.0)))  # 坐标 (3.0, 4.0)
```
