...
YaoXiang 采用纯函数式设计，通过柯里化实现类似对象方法调用的语法糖，无需引入 `class` 和 `method`
关键字。
...
| **当前热门议题**： | | | | - 宏系统的设计与实现 | | - 接口类型机制 | | - 错误处理语法优化 |
| - 标准库 API 设计 |
...
> 一元前缀运算符（`!` `-` `+`）紧绑定，高于所有二元运算符（Zig 式，见 SPEC §2.2）。
...
A:
YaoXiang 保留了 Rust 的内存安全和零成本抽象，但采用更简单的语法和更低的认知负担。**并作模型**比 Rust 的
`async/await` 更简洁——只需一个 `spawn`
标记，无需手动管理 Future 和 Pin。「万物并作，吾以观复」，让并发编程如同描述自然规律般直观。**所有权模型**（RFC-009
v9）用 Move + &T/&mut
T 令牌替代生命周期标注，用类型属性（Dup/Linear）替代借用检查器。统一类型语法消除了
`enum`/`struct`/`trait`/`impl` 的概念碎片。
...
A:
4 空格提供了清晰的代码块视觉分隔，减少了嵌套深度带来的混淆。这是经过深思熟虑的「AI 友好」设计决策。
