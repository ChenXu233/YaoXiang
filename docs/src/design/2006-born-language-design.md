...
我从未经历过需要手动管理线程池的时代。我第一次写并发代码时，用的是JavaScript的`async/await`。当我后来学习Rust的`async/await`时，我惊讶于为什么一个简单的"等待"操作需要如此复杂的`Future`
trait和`Pin`、`Context`。
...
编程语言设计长期以来是"前辈"的话语领域。Linus Torvalds在21岁时开始Linux，Graydon
Hoare在设计Rust时已经是资深工程师。
