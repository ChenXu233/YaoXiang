...
yx-bindgen 将 C 头文件机械转换为 `.yx`
FFI 绑定文件，生成库绑定、类型两分（不透明句柄 / 透明类型）、函数声明和方法绑定。
...
手工编写 FFI 绑定枯燥且易错——C 库有几十上百个函数。更危险的是**透明类型的布局**：手写
`Timespec: Type = { tv_sec: Int64, tv_nsec: Int64 }`
时，字段大小、对齐、padding 必须精确匹配目标平台的 C
`struct timespec`，否则 C 按错误布局读写会越界（RFC-026 §2.2 的信任担保）。yx-bindgen 从 `.h`
机械计算布局，消除这个手工担保的风险。
...
不完整类型（`typedef struct sqlite3 sqlite3;` 无定义体）→ 必然是黑盒句柄。完整定义的 struct
→ 透明类型，字段布局机械计算。
...
**平台差异**：`time_t`、`long`、`size_t` 大小随平台变化。yx-bindgen 按 `--target`
选择正确映射，保证生成的透明类型与目标平台 C struct 逐字节匹配。这是消除 RFC-026
§2.2 手工布局担保风险的核心价值。
...
**关键**：布局由 yx-bindgen 机械保证（平台正确），但所有权语义（是否 `.drop`、`char*`
是复制还是裸指针）由用户根据 C 文档确认。工具负责机械正确，用户负责语义正确。
