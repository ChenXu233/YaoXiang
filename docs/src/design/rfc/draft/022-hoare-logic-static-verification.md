---
title: RFC 022：可选的霍尔逻辑静态验证（规约注释与规约类型）
---

# RFC 022: 可选的霍尔逻辑静态验证（规约注释与规约类型）
...
本文提出为 YaoXiang 语言引入一种**可选的霍尔逻辑静态验证机制**，允许开发者在注释中以 `//!` 或 `/*! ... !*/` 的形式编写前置条件、后置条件和循环不变式。这些规约注释在默认编译下被忽略（零开销），但可通过编译器选项启用静态证明或运行时检查。规约本身被视为类型系统的一部分，形成"规约类型"（如 `Requires[P]`、`Ensures[P]`），并可被用户扩展。该设计旨在保持语言简洁的同时，为关键代码提供高可靠性保障，且与 YaoXiang 的统一类型模型完美融合。
...
我们的目标是设计一种**轻量、可选、与语言一体**的静态验证方案：

- **可选性**：开发者仅在需要高可靠性的模块中编写规约，不影响其他代码
...
| 模式 | 行为 | 选项 |
|------|------|------|
| **普通编译** | 忽略所有 `//!` 注释，不生成任何代码 | `yaoxiangc source.yx` |
| **验证模式** | 解析规约，生成验证条件，调用 SMT 求解器证明 | `yaoxiangc --verify source.yx` |
