...
>
> - **实验目标**：验证类型级同像性的实现难度和潜在价值
> - **止损线**：6 个月无进展则放弃
> - **成功标准**：能跑通至少一个用户自定义关键字（完整解析→编译→运行）
>
> **不保证会合并到 main 分支**，未来可能因为各种原因被拒绝或放弃。请勿在生产环境中使用此特性。
>
> **⚠️ 定位说明**: 本 RFC 是语言设计思想实验，不提供工程方案。如需实用的可扩展解析器模式，参见 Rust `syn::Parse` 或 Haskell `parsec`。
