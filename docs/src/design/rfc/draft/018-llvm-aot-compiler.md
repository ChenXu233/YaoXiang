...
自底向上执行：
  print(a) ← 从"需要结果的地方"开始
       ↑
  fetch(url0) ← 分析依赖，反向查找

  fetch(url1) ← 孤岛，独立并行执行
```

**关键区别**：
- 不是"遇到函数调用就生成 Future"
- 而是从"最终需要的结果"反向分析依赖
- 没有消费者的节点（孤岛）不执行或独立并行
- 无限循环作为后台 DAG，调度器切片执行
...
process(a)   process(b)                        │
    │             │                             │
compute(x)   compute(y)  ←── 孤岛 DAG ──────────┤
    │                                           │
fetch(url0)  fetch(url1)  fetch(url2)          │
    (已执行)                                    │

同时还有一个后台 DAG（while True）：
    ┌─────────────────────────────────────────┐ │
    │  while True:                            │ │
    │      update_ui()                        │ │
...
Phase 1: 自底向上分析（编译时）
─────────────────────────────────────────
从 save_results(results) 开始：
    "需要 results" → 依赖 parse_page(results)
    "需要 page0" → 依赖 fetch(url0)
...
─────────────────────────────────────────
调度器找到所有叶子节点：
    - fetch(url0), fetch(url1), fetch(url2) 无依赖 → 并行执行
    - 控制并发数（比如 16 个）

Phase 3: 向上遍历
─────────────────────────────────────────
当 parse_page 需要 page0 时：
    - 检查 page0 是否就绪
    - 就绪 → 执行 parse_page
    - 未就绪 → 等待，完成后继续
