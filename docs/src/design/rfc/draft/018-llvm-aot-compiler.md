...
本文档设计 YaoXiang 语言的 LLVM AOT 编译器，目标是通过预先编译生成机器码 + DAG 元数据，由运行时调度器根据 DAG 依赖关系调度执行。此设计与 Rust async/await + tokio 运行时模式类似，遵循 RFC-001 的 L3 透明并发设计：默认 @auto（自动并行），@block 同步是特例，解决颜色函数问题。
...
| 注解 | 场景 | 调度策略 |
|------|------|----------|
| `@auto`（默认，L3） | 透明并发 | DAG 自动分析依赖，无依赖并行执行 |
...
| 运行时 | LLVM AOT 行为 |
|--------|---------------|
| **Embedded** | 无 DAG 调度，直接生成顺序机器码，放弃惰性求值 |
| **Standard** | DAG + 单线程调度（num_workers=1），表现为异步执行 |
