...
本文提出引入 `^^` 运算符作为反射入口，用于获取类型和值的元数据。`^^T` 返回类型 `T`
的静态元数据对象，`^^obj` 返回值 `obj`
的动态类型元数据。元数据对象是普通记录类型，包含名称、参数、字段等信息，可在编译期和运行时使用。
...
当前 YaoXiang 没有反射机制，无法在编译期或运行时访问类型的元数据。如果直接使用 `.name`、`.fields`
访问类型元数据，会与用户定义的字段冲突：
