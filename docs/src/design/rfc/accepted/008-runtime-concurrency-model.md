...
> **作者**: 晨煦
> **创建日期**: 2025-01-05
> **最后更新**: 2026-03-21（补充 @eager 语义与示例语法对齐）
...
│  问题 3：异步/并发的实现层级                                   │
│  ├── @block（L1 同步）                                         │
│  ├── spawn（L2 显式并发，仅 @block 内）                         │
│  ├── @auto/无标记（L3 透明并发，Lazy）                          │
│  ├── @eager（L3 急切求值，创建任务后立即 force）                │
│  └── 这些特性应该在哪一层实现？                                │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│  L3    │ 透明并发（默认）  │ 无/@auto/@eager│ Core Runtime  │ Lazy 默认；@eager 立即 force │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  核心要点：                                                              │   │
│  │  ├── L2 spawn 只能在 @block 函数内部使用                                  │   │
│  │  ├── 用于在同步代码中插入部分并发                                        │   │
│  │  ├── L3 默认 @auto（Lazy），编译器自动分析 DAG                           │   │
│  │  └── @eager 仍走 DAG，但会创建任务后立即 force（便于调试/可控等待）      │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│                           Full Runtime（可选）                                  │
