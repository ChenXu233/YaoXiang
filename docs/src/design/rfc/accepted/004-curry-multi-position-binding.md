...
author: '晨煦'
created: '2025-01-05'
updated: '2026-09-16（新增 §柯里化链条终止与括号语义）'
issue: '#132'
---

# RFC-004: 柯里化方法的多位置联合绑定设计

> **补充（2026-09-16，括号语义）**：本文的柯里化链条有一个此前未写明的终止规则。返回位置上的**括号**声明“这是一个完整类型（值）”，链条到此终止：
>
> ```yaoxiang
...
> ```
>
> 依据：注解即类型。`g` 声明了 `g(1) : (b: Int) -> Int`，则 `g(1)` 必须
> **是**那个函数（而非“下一段参数”）。这也是 RFC-010 与语言规范里
> `map: (T: Type) -> ((list: List(T), f: (x: T) -> R) -> List(R))` 的写法本意，与 `stdlib.md`
> 中方法签名 `-> ((self: ...) -> ...)` 一致。
>
> 自反地，不带括号的嵌套 `Fn` 一律是柯里化（含 RFC-011 的
> `(T: Type) -> (x: T) -> T`）。两者行为可区分且均可用。
>
> 实现上：`Type::Paren` 仅在“括号未构成参数组”（即括号后不跟
> `->`）时产生，也只被链条切分与 formatter 两处感知，其余环节视其透明。

## 摘要

本 RFC 提出一种全新的**多位置联合绑定**语法，允许将函数精确绑定到类型的任意参数位置，支持单位置绑定和多位置联合绑定，从根本上解决柯里化绑定中"谁是调用者"的问题，无需引入
`self` 关键字。
...
**核心规则：不写 `[n]` = 不绑定。** `Point.name = func` 只是命名空间别名，不会触发任何隐式绑定。要让
`p.name(args)` 这种 `.` 调用语法生效，必须显式指定：`Point.name = func[n]`。
