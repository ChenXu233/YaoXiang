...
**适用范围：编译期求值。** 终止检查只针对编译期求值涉及的循环与递归（由依赖追踪触发）——编译器自己要执行的代码必须可证全量；运行期代码不要求终止证明（§6.7 作用域表）。范围内编译期全自动优先：编译器能证明的循环通过，自动策略无法证明的直接报编译错误——程序员必须让编译器能自动分析循环的终止性。无**标注语法**口子（不引入 `decreases` 类标注，未支持的永远是语法位，不是函数）。

> **修订（RFC-027a，2026-09-14）**：① 适用范围明确为编译期求值——原文本对「循环」未限定位置，与 §6.7 作用域表存在歧义，实现层的无条件全量检查按此收窄。② 自动策略无法证明的终止义务，程序员可提供测度函数与终止证明函数（`Terminates`）兜底——皆为普通 YaoXiang 函数，零新语法，类型检查器验证。全自动优先原则不变，兜底只发生在 Unproven 之后。见 [RFC-027a: 终止检查的证明函数兜底](../draft/027a-termination-proof-fallback.md)。
...
> 此循环无法自动证明终止。循环变量依赖于不可分析的函数 `f`。请使用可被编译器分析的迭代模式，或提供测度函数与终止证明函数（见 RFC-027a）。
...
**谓词应用的实参必须是编译期常量形态**——字面量、变量（按名绑定）或单参数类型应用（递归提取）。实参形态不可转换为常量表达式报 **E1092**，实参个数与谓词参数不匹配报 **E1093**——精化约束**绝不静默丢弃**（此前不可转换实参会导致约束无声消失，违反约束的绑定静默通过）。
...
- [x] **`forall` 谓词违反计数量度对非相邻操作的扩展？**
      ...
      → **再讨论已触发（2026-09-14，#318）**：非结构递归（gcd 类非直接递减、相互递归、归并分区）即本条款预期的真实场景——现管线无法自动证明其终止，且它们无法在不破坏可读性的前提下改写为可分析的迭代模式。结论：全自动优先不变；Unproven 后增加证明函数兜底——测度与终止证明皆为普通 YaoXiang 函数，零新语法，类型检查器验证。落地于 [RFC-027a: 终止检查的证明函数兜底](../draft/027a-termination-proof-fallback.md)。
