...
编译期全自动优先。编译器能证明的循环通过，自动策略无法证明的直接报编译错误——程序员必须让编译器能自动分析循环的终止性。无**标注语法**口子（不引入 `decreases` 类标注，未支持的永远是语法位，不是函数）。

> **修订（RFC-027a，2026-09-14）**：自动策略无法证明的终止义务，程序员可提供测度函数与终止证明函数（`Terminates`）兜底——皆为普通 YaoXiang 函数，零新语法，类型检查器验证。全自动优先原则不变，兜底只发生在 Unproven 之后。见 [RFC-027a: 终止检查的证明函数兜底](../draft/027a-termination-proof-fallback.md)。
