...
**信任模型**：
- 类型检查器是唯一的信任根。SMT 返回 `unsat` 时，编译器将其推理痕迹重构为类型检查器内核可验证的证明项。如果重构失败（SMT 的推理步骤超出编译器内核的推理规则），回退到 Unproven——程序员可以手动写证明函数。
- 阶段 1/2 中，作为务实起步，SMT `unsat` 结果由编译器信任（这是 F\*/Dafny 的路线——Z3 出错的概率低于编译器自身的 bug 率）。阶段 3/4 中加入完整的证明重构作为更强的保证。
...
| **阶段 4** | 证明重构（SMT unsat → 内核可验证的证明项）+ 增量验证 + 缓存 + 反例格式化 + IDE 支持 |
