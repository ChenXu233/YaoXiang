...
**SMT 是精度层，不是 soundness 依赖。** 借用系统谓词的 sound 判定完全由快速通道承担
（区间 + 反向 BFS + break 切断）；SMT 逻辑切断只决定「循环边界的合法程序能否放行」。
SMT 不可用 / 超时 / 未实现（RFC-027 impl: in_progress）时，fallback = 回边穿越 = 保守拒绝，
该拒的仍然必拒。**无 SMT 时的保守度 = 循环内借用+写一律拒，与 Rust NLL 同级**
（Rust 生产级借用检查同样无 SMT）。SMT 落地是纯精度增益，不阻塞 sound 主线的交付。
...
注意判定目标是**写节点自身**的路径条件（`i == n` 属于 `if` 分支内的 `data.push(4)`），
不是回边节点的路径条件。
...
本算法引入「令牌创建时间」维度。令牌活性是**区间** `[created_at, last_use]`，不是反向可达集；
写操作只在令牌的活性区间内才构成冲突——这覆盖「写先、借用后」的合法顺序
（§2.4 语义：参数令牌调用结束释放），避免将其误报。
...
算法：
  ...
  while queue not empty:
    ...
    for each pred in dag.predecessors(cur):
      ...
      if pred 是回边:
        path_cond = 写节点 node 的路径条件   # 判定目标为写节点自身条件
        loop_cond = 循环条件
  ...
  # 判定（写先、借用后）
  # node < created_at(token) → 写发生时令牌尚不存在 → Safe
...
BrandNode:
  ...
  created_at: NodeId         # 令牌创建节点（borrow 区间的左端点）
```
...
令牌活性是**区间** `[created_at, last_use]`，不是反向可达集。`created_at` = 令牌创建节点；
`last_use` = 消费者分析的最大消费节点。写操作 W 与令牌 T 冲突的充分必要条件：
`conflicts(T, W) ∧ created_at(T) ≤ node(W) ∧ node(W) 可前向到达 last_use(T)`（反向 BFS 判定）。
「写先、借用后」的合法顺序（§2.4：参数令牌调用结束释放）由 `created_at(T) ≤ node(W)`
直接排除，无需任何特殊规则。此模型使 §权衡优点 5「算法不保守」的声明在全部顺序下成立。
...
> 上表复杂度为**设计估算，未经实测**；「~1ms」「极罕见」应视为量级预期而非测量值，
> 实现落地后以可观测性数据校准。
