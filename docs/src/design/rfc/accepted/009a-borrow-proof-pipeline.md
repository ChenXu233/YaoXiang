...
>
> **前置条件**: RFC-027 已被接受。本 RFC 的所有机制（证明管道、SMT fallback、路径条件收集）依赖 RFC-027 的实现。
>
...
**三种失败模式，同一验证引擎。** 类型命题证不出来 → 编译错误（不可绕过）。借用命题证不出来 → 编译错误，重构代码（不可绕过）。用户谓词证不出来 → 编译错误，可写证明函数（可绕过）。失败策略不同，但验证引擎是同一个——SMT 求解器 + 编译器内核推理规则。区别仅在于"证不出来时谁负责补证明"——编译器拒绝替程序员写借用证明（借用命题的证明策略是结构分析 + SMT，不需要程序员介入），但接受程序员写的用户谓词证明函数。这不是管道不一致——是不同命题类别的责任边界不同。
...
这些约束消除了 Rust CFG 不动点迭代的主要复杂度来源。不是 DAG 比 CFG "更高级"——是更简单的语言设计允许更简单的分析。
...
这是消费者分析的自然推论：消费者的位置定义了令牌的最后使用。`use(v)` 是 `v` 的消费者 → `v` 在 `use(v)` 之后即刻死亡。不需要额外的 `{}` 或 `drop()` 来提前结束令牌生命。
...
此设计不需要额外的"循环保守存活"规则。反向 BFS 从消费者出发，消费者在循环体内 → 活性被限制在当前迭代内 → 回边不穿越。与 RFC-009a §用例分析的循环示例完全一致。
...
- [x] **品牌树在循环展开时的 `ref_count` 跨迭代语义**——走 NLL：令牌在最后使用后死亡。循环内绑定的副本在迭代边界死亡，反向 BFS 不携带跨迭代活性。详见 §NLL 与迭代边界。
- [x] **`?` 错误传播路径上的令牌释放顺序**——Release 由作用域分析驱动（保留在 ir_gen.rs）。每个作用域出口点（`}`、`?`、显式 return）按 LIFO 释放活跃令牌。品牌树父子关系自动处理级联释放。详见 §`?` 错误传播与作用域驱动 Release。
