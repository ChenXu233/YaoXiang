...
>
> **前置条件**: RFC-027 已被接受。本 RFC 的所有机制（证明管道、SMT
> fallback、路径条件收集）依赖 RFC-027 的实现。
>
...
`{conflicting_tokens 全部死亡} op {WriteToken 安全获取}` —— 同一个
`{P} op {Q}`，和类型检查、谓词验证共享 RFC-027 的证明管道。没有新分析框架。一条管道，多种命题。
...
2. **令牌生命期交互**（子令牌存活 → 父令牌暂停 → 子令牌死亡 → 父令牌复活）——
   `{conflicting_tokens 全部死亡} write(data) {安全}`。需要**活性分析**，不是线性追踪。
...
上一版（009a
v1）用"DAG 替代 NLL"叙事，引入了不必要的新概念（保守分支规则、循环特殊处理）。核心矛盾没有说清楚：**借用检查不是独立系统——它是霍尔命题的一种。**
...
**系统谓词的证明不给程序员开证明函数——编译器全自动。** 证不出来时用户重构代码。

**三种失败模式，同一验证引擎。**
类型命题证不出来 → 编译错误（不可绕过）。借用命题证不出来 → 编译错误，重构代码（不可绕过）。用户谓词证不出来 → 编译错误，可写证明函数（可绕过）。失败策略不同，但验证引擎是同一个——SMT 求解器 + 编译器内核推理规则。区别仅在于"证不出来时谁负责补证明"——编译器拒绝替程序员写借用证明（借用命题的证明策略是结构分析 +
SMT，不需要程序员介入），但接受程序员写的用户谓词证明函数。这不是管道不一致——是不同命题类别的责任边界不同。

这和 Rust `'a` 不同：`'a` 是必修课，证明函数是选修课——绝大多数用户一辈子碰不到选修课的门。
...
> **勘误（2026-08-17，SMT 定位修正）**：SMT 是**精度层，不是 soundness 依赖**。
> 借用系统谓词的 sound 判定完全由快速通道承担（区间 + 反向 BFS + break 切断）；
> SMT 逻辑切断只决定「循环边界的合法程序能否放行」。SMT 不可用 / 超时 /
> 未实现（RFC-027 impl: in_progress）时，fallback = 回边穿越 = 保守拒绝，
> 该拒的仍然必拒。**无 SMT 时的保守度 = 循环内借用+写一律拒，与 Rust NLL
> 同级**（Rust 生产级借用检查同样无 SMT）。SMT 落地是纯精度增益，不阻塞
> sound 主线的交付。
...
反向 BFS 从 `view.total_count`（节点 3）出发 → 节点 3 是 #1 的最后一个消费者 → 节点 4 > 节点 3
→ 节点 4 不在 unsafe → ✅
...
> **澄清（#264 后）**："不合并分支状态"仅指**借用活性**（品牌消费者反向 BFS）。
> **move 状态**（变量所有权）是另一条分析：per-CFG-node 前向数据流
> （#264，NLL/Polonius 风格），分支汇合时**保守 meet**（任一分支 Moved → 汇合 Moved），
> 字面量不可达分支（`if false`）不参与。两者分层：借用活性看"有没有后续消费者"，
> move 分析看"变量是否可能已被转移"。
...
view 通过返回值逃逸 → `use(result)` 是 view 的消费者 → 从 `push` 反向走能到达 `use(result)` →
unsafe。
...
反向 BFS 从 `use(view)` → 回边 → 向前走到 `data.push(4)` → 碰到 `break` → **切断** → `data.push(4)`
不在 unsafe → ✅
...
反向 BFS 从 `use(view)` → 回边 → 走到 `data.push(4)` → 检查路径条件 `i == n` →
SMT 查询：`i == n ⇒ !(i < n)`？→ Proved → **逻辑切断** → `data.push(4)` 不在 unsafe → ✅

> 勘误（2026-08-17）：`i == n` 是写节点（if 分支内的 `data.push(4)`）自身的路径条件，
> 判定目标为写节点（见 §路径条件收集勘误规则 1）。
...
**我们没有发明新分析。我们只是把 `'a` 从类型签名层降到了证明层。** 品牌 ID 做的事和 `'a`
完全一样——标记借用的身份、追踪派生关系、判定冲突。区别只有一个：`'a` 在用户写的类型签名里；`#42`
在编译器内部。

这不丢人。Curry-Howard 说类型是命题，程序是证明。`'a`
不是命题的一部分——它是证明策略的一部分。Rust 把证明策略写进了命题签名。我们把它放回该在的地方。
...
YaoXiang 的三项语言设计约束使 DAG 分析可行：

- **无变量遮蔽**——一个名字永远指向同一个东西，无需跨重绑定追踪
- **for 每次迭代新绑定**——迭代间天然隔离，不存在跨迭代借用
- **结构化并发**——任务边界清晰，无需跨任务活性传播

这些约束消除了 Rust CFG 不动点迭代的主要复杂度来源。不是 DAG 比 CFG
"更高级"——是更简单的语言设计允许更简单的分析。
...
现有的 `BorrowChecker`、`MoveChecker`、`DropChecker`、`MutChecker`
**变成命题生成器**——不是消失，换身份。它们生成命题，管道验证命题。
...
> **勘误（2026-08-17，#251 P0-6 审计）**：以下算法补充「令牌创建时间」维度。
> 原文依赖 DAG 节点全序隐含「借用在写之前」的假设，未覆盖「写先、借用后」
> 的合法顺序（§2.4 语义：参数令牌调用结束释放）——审计实测该场景被误报
> （#290）。令牌活性是**区间** `[created_at, last_use]`，不是反向可达集；
> 写操作只在令牌的活性区间内才构成冲突。
...
O(N)，其中 SMT 调用次数 = 回边数 × 有路径条件的回边比例。实际代码中 SMT 调用极为罕见——仅 `while`
循环体内、有精化类型变量的路径条件时触发。
...
> **勘误（2026-08-17，传播规则补完）**：原文未定义路径条件如何传播到回边起点，
> 用例「while + SMT 逻辑切断」的 `i == n` 按原文算法不可复现。补规则：
>
> 1. **路径条件附着在写节点自身**：分支内写操作 W 携带其分支条件（`if i == n { W }`
>    → path_cond(W) = `i == n`）。反向 BFS 穿越回边时，SMT 判定的是
>    `path_cond(W) ⇒ !loop_cond`（能走到 W 的路径必然退出循环 → 下一轮
>    consumer 不可达 → 切断），不是回边节点的路径条件。
> 2. **join 保守清空**：if/else 汇合点不携带分支内路径条件（两分支条件析取后
>    通常不可判定，直接清空）。汇合点之后的写操作 path_cond 为空 → 回边穿越。
> 3. **路径条件语义化**：path_cond 是 ConstExpr（RFC-027 §3.2 语义），不是
>    源码文本；smt_cut 将其翻译为 SMT 约束后求解。
> 4. **无路径条件 → 回边直接穿越**（unsafe），不调用 SMT。
...
SMT 求解器在借用检查中的角色：**不是主力，是安全网。**
只在 while 回边需要逻辑切断时调用。绝大多数借用检查在快速通道完成——O(N) 反向 BFS，零 SMT 开销。
...
> **勘误（2026-08-17，区间模型补完）**：令牌活性是**区间** `[created_at, last_use]`，
> 不是反向可达集。`created_at` = 令牌创建节点；`last_use` = 消费者分析的最大消费节点。
> 写操作 W 与令牌 T 冲突的充分必要条件：`conflicts(T, W) ∧ created_at(T) ≤ node(W)`
> `∧ node(W) 可前向到达 last_use(T)`（反向 BFS 判定）。「写先、借用后」的合法顺序
> （§2.4：参数令牌调用结束释放）由 `created_at(T) ≤ node(W)` 直接排除，无需任何特殊规则。
> 此模型使 §权衡优点 5「算法不保守」的声明在全部顺序下成立。

**令牌死亡时刻 = 最后使用点（NLL），非词法作用域末尾。**

这是消费者分析的自然推论：消费者的位置定义了令牌的最后使用。`use(v)` 是 `v` 的消费者 → `v` 在
`use(v)` 之后即刻死亡。不需要额外的 `{}` 或 `drop()` 来提前结束令牌生命。

**循环迭代边界是令牌副本的死亡线。** 三条规则：
...
此设计不需要额外的"循环保守存活"规则。反向 BFS 从消费者出发，消费者在循环体内 → 活性被限制在当前迭代内 → 回边不穿越。与 RFC-009a
§用例分析的循环示例完全一致。

### `?` 错误传播与作用域驱动 Release

`?`
是提前返回——作用域的正常出口之外多了一条退出路径。令牌必须在这条路径上被释放，释放顺序错误即 UB。

**Release 指令由作用域分析生成，不是硬编码在 Call 之后。**
...
在每个出口点，按声明逆序（LIFO）插入该作用域内所有活跃令牌的 Release 指令。品牌树的父子关系自动处理派生令牌的级联释放：
...
实现位置：保留在 `ir_gen.rs`，改为作用域驱动——不引入新 compiler pass。
...
> 勘误（2026-08-17）：上表复杂度为设计估算，未经实测；"~1ms""极罕见"
> 应视为量级预期而非测量值，实现落地后以可观测性数据校准。

**SMT fallback 的触发条件极为苛刻**：同时满足 (1)
while 循环 (2) 循环体内有写操作 (3) 写操作后有路径条件可判循环终止 (4) 编译器需要依赖该条件切断回边。实际代码中占比远低于 1%。其余的借用检查全部在快速通道完成。

和 RFC-027 用户谓词的关系：用户谓词以 SMT 为主力，借用系统谓词以结构分析为主力。两者共享同一 SMT 求解器和预算上限（RFC-027
§8），但借用系统谓词几乎不消耗 SMT 预算。

线性代码 → 无回边 → 分层 1 O(N) 秒出。循环 + 路径条件 →
SMT 调用，线性算术毫秒级（RFC-027 预算 100ms）。一次 BFS 结果可缓存供同一令牌的多次查询复用。
...
**变量级错误**：E0597（活得不够长）、E0502（可变+不可变同时借用）、E0499（多次可变借用）。Rust 已经是标杆——变量名+行号，不出现
`'a`。YaoXiang 精确度持平。信息全在品牌树里：令牌创建点、消费者位置、请求点。

**签名级错误**：E0623（lifetime mismatch）、E0106（missing lifetime
specifier）、E0477（不满足 required lifetime）。围绕 `'a` 展开。YaoXiang
**不存在这类错误**——签名里没有 `'a`。不是"报不出来"，是用户没写过的东西不用报。
...
3. 明确：品牌 ID（`#42`）就是
   `'a`——信息完全一样，编码不同。不是发明了新分析——是把生命周期从类型层降到了证明层
...
1. **类型签名不含生命周期**：`#42` 就是
   `'42`——同一个信息，编码在品牌树里，不暴露在类型签名中。此点不可证伪：数一下 Rust 里有 3 个引用参数的泛型类型需要几个
   `'a` 参数，YaoXiang 里需要几个。答案是 3 vs 0。

2. **概念统一**：借用检查和用户谓词共享同一条证明管道——`{P} op {Q}`，管道验证 P。Curry-Howard 一致。

3. **零新分析框架**：不引入新分析框架。用户不感知"借用检查器"的存在——就像用户不感知"类型检查器"的实现细节。

4. **错误信息只含用户写过的符号**：少了一整个维度的错误类别（E0623、E0106、E0477——全部围绕
   `'a`）。变量级错误与 Rust 精确度持平。

5. **算法不保守**：反向 BFS + break 切断 +
   SMT 逻辑切断。不需要"循环内保守存活"。不需要"分支保守合并"。

### 缺点

1. **不是新发明**：品牌 ID 做的事和 `'a`
   完全一样——编译器内部的约束求解复杂度没有消失，只是编码方式从"变量名+约束集"变成了"品牌路径+前缀匹配"。对终端用户的差异仅在于签名里不写
   `'a`。
...
4. **极少数模式需重构**：编译器自动证明覆盖不了的跨分支借用，用户需重构代码。和 Rust `'a`
   的兜底不同：Rust 有 `'a` 当笔（标注即能过）；YaoXiang 的兜底（证明函数）不是 MVP。
...
- [x] **品牌树在循环展开时的 `ref_count`
      跨迭代语义**——走 NLL：令牌在最后使用后死亡。循环内绑定的副本在迭代边界死亡，反向 BFS 不携带跨迭代活性。详见 §NLL 与迭代边界。
- [x] **`?`
      错误传播路径上的令牌释放顺序**——Release 由作用域分析驱动（保留在 ir_gen.rs）。每个作用域出口点（`}`、`?`、显式 return）按 LIFO 释放活跃令牌。品牌树父子关系自动处理级联释放。详见 §`?`
      错误传播与作用域驱动 Release。
- [ ] 证明函数语法（远期，非 MVP——不阻塞任何 Phase）
