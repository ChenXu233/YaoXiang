...
> ③ 阶段 1 接线后 `Struct == Struct` 从 `E6007` 运行时错误转为编译期判定，`E6007` 触发面收缩。
> 表内注册文案在实现落地前维持现状。
...
>
> **死代码码义（#321 定案 B）**：`pub` 定义是对外接口，永不报告——对外消费方
> 是否使用超出单文件分析边界，宁可静默不误报。W1001/W1002/W1004/W1005 仅针对
> **私有（非 pub）定义**：从 `main` 与 `pub` 定义出发的引用可达性分析中从未被
> 引用即报告。方法（W1005）按调用点短名匹配。bin/lib target 语义（bin 内未用
> pub 报警）为未来扩展（#289 方案 A），需项目模型支持。
>
> **未使用导入（W1003）**：由 typecheck 的 use elaboration 检出（pass2 登记
> 导入本地名、表达式解析与类型注解位置命中即视为已使用），覆盖整体导入
> （`use std.io` → 模块别名）与具名导入（`use std.io.{print}`）。
>
