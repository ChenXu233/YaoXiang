...
> **作者**: 晨煦
> **创建日期**: 2025-01-05
> **最后更新**: 2026-03-21（语法示例对齐统一 Binding 规则）
...
| **L2** | @block 内并发 | `spawn` | @block 作用域内可控并发 | ⚠️ 部分 | 需要在同步代码中插入并发 |
| **L3** | 完全并发 | 默认（@auto） | 自动分析 DAG | ✅ 完整 | 专家、自动并行优化 |

#### L1: `@block` 同步模式

```yaoxiang
# L1: @block 同步模式（无 DAG，纯顺序执行）
fetch_sync: (url: String) -> JSON @block = {
    HTTP.get(url).json()
}
...
> **设计说明（2026-03-10）**：L2 spawn 限定在 @block 作用域内使用，用于在需要同步的代码中插入并发。
...
| 维度 | `@auto`（默认） | `@eager` | `@block` | `spawn` |
|------|-----------------|----------|----------|---------|
| **执行方式** | 自动 DAG 分析 | 同步等待依赖 | 纯顺序 | @block 内并发 |
| **并行度** | ✅ 完全并行 | ⚠️ 按依赖顺序 | ❌ 无 | ⚠️ 部分 |
| **调度器参与** | ✅ 完整 | ✅ 完整 | ❌ 不参与 | ✅ 参与 |
| **DAG 构建** | ✅ | ✅ | ❌ | ✅ |

> **注意**：`spawn` 只能在 `@block` 函数内部使用，用于在同步代码中插入并发。
...
```yaoxiang
# @auto（默认）：最大化并行
heavy_calc: (n: Int) -> Int = fibonacci(n)  # 自动并行
...
**关键洞察**：
- 不是"自顶向下"生成 Future，而是"自底向上"从结果反向分析依赖
- 没有消费者的节点（孤岛）不执行，或独立并行执行
- 无限循环是特殊的"后台 DAG"，调度器切片执行

#### 3.2 执行模型：反向遍历 + 全局 DAG
...
全局 DAG（自底向上分析）：
    ...
    ┌──────┴──────┐
    │
fetch(url0)    fetch(url1)  ← 叶子：实际执行

执行流程：
    1. 从 print(a) 开始："我需要 a"
    2. 反向分析：a 依赖 fetch(url0)
    3. fetch(url0) 没有下游 → 叶子 → 执行
    4. print 等待 fetch(url0) 完成
    5. fetch(url1) 没人要 → 孤岛 → 独立并行（可用另一核心）
```
...
```
主流程 DAG：
    fetch(url0) → process → print

孤岛 DAG（无消费者）：
    fetch(url1)  ← 没人要结果，但仍然执行

调度器处理：
    - 主流程：fetch(url0) → process → print（按依赖链）
    - 孤岛：fetch(url1)（独立任务，可用另一核心）
```
...
```
主 DAG（有结束）：
    fetch → process → print → 结束

后台 DAG（无限循环）：
    while True → update_ui → fetch_new → process → 回到开头

调度器：
    - 主 DAG 执行完就结束
    - 后台 DAG 永远运行，但用"切片"方式执行
    - 不会卡死在循环里
```
...
- [x] **问题1：三层运行时如何选择？** ✅ 已解决
  ...
  # 声明了标准式，可在标准/嵌入式之间切换
  # 未声明完整式，无法切换到完整
  main: [R: Runtime](ctx: RuntimeContext[R]) -> Void = {
      ...
  }
  ```
