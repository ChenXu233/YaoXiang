...
author: '晨煦'
created: '2026-06-05'
updated: '2026-07-05（RFC 同步检查：实现进度 ~85%，核心运行时与前端分析已完成）'
...
本文档定义 YaoXiang 编程语言 `spawn` 的**运行时行为语义**：`spawn <expr>`
是唯一的并行原语，可修饰任意表达式，调用方同步阻塞。表达式的形状决定任务拆解粒度，运行时按 GMP 模型调度——无依赖的任务扔进工作队列，worker 抢着跑。
...
> **用户心智模型**：你写的普通代码是顺序执行的。当你希望多件事一起做时，把它们放进 `spawn <expr>`
> 里。没有回调，没有 `await`，没有奇怪的注解。
...
> **不重新引入 `Sync` trait**：与 RFC-024 "无 Send/Sync" 承诺一致。要求用户改用 `ref`
> 或非 spawn 写法。
...
> **迁移说明**（2026-06-11）：现有的 `frontend/core/typecheck/passes/spawn_placement.rs` 将迁移至
> `frontend/core/spawn/placement.rs`。`typecheck/passes/` 目录下的 `spawn_placement`
> 模块声明需同步移除。
