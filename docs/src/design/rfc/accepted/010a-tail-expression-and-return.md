...
| ① 块的值 = 尾表达式       | `spawn {}` 值出口（尾表达式）       | ❌ 未实现（见 #365）          |
...
b = { x = f(); x * 2 }
c = spawn { fetch("a") }   // ⚠️ 尾表达式出口尚未实现，见 #365；当前需写 `spawn { return fetch("a") }`
```
...
- [x] 尾表达式与 RFC-038 语句终止规则的交互：换行终止的末位表达式仍为块值吗？（@晨煦：需与 RFC-038 的「行首
      `(`/`[` 永不合并」等规则一并确认）——已实测：换行终止的末位表达式（含行首 `(` / `[` / 列表字面量）
      均为块值
- [x] `name = { ... }` 何时是函数、何时是块值绑定——见附录D（裁决 C：注解优先，默认函数）
- [ ] `unsafe {}` 改写后的具体形态——`unsafe {}` 尾表达式已实测可用；`spawn {}` 尾表达式仍不可用
      （值丢失或 `E3006`），见 #365
