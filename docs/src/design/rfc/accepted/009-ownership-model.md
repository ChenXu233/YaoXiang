...
**核心原则：`&T` 和 `&mut T`
是零大小的编译期令牌类型。它们不是"引用"，而是"访问权限的类型级证明"。**
...
Lambda 是函数值——可以被返回、存储、传出当前作用域。因此 Lambda
...
> 补充（2026-08-17）：上下文依赖的正解是柯里化固化，不是捕获。闭包逃逸后其
> 定义处作用域可能已死，故不得隐式捕获；但调用点（创建点）作用域必然存活，
> 上下文在该点固化为值进入闭包是安全的。见 SPEC §12.3。
...
编译器不需要 `'a`
标注，因为令牌是**值**，值的生命周期由所有权系统（Move/RAII）统一管理。**将借用问题降维为所有权问题。**
...
和类型检查、用户谓词验证共享 RFC-027 的证明管道。编译器自动生成借用命题（`borrow_conflict`、`use_after_move`、`use_after_drop`、`mut_violation`），送入管道验证。管道返回 Proved
/ Disproved / Unproven。
...
`BorrowPredicateEmitter`（命题生成器），生成的借用命题和其他类型命题共享同一条证明管道。这和类型检查器的概念完全平行：类型检查器生成类型等式命题，借用命题生成器生成借用命题，同一条管道验证。详细设计见
...
| **Lambda 不隐式捕获**              | Lambda 只用显式参数，不隐式捕获外层变量；上下文经柯里化在创建点固化（SPEC §12.3） | 闭包定义处作用域可能已死；固化值在创建点（调用点作用域存活）安全 | 2026-06-16 |
...
**令牌活性是霍尔逻辑命题。**
`{冲突的 ReadToken 全部死亡} write(data) {WriteToken 安全获取}`——和类型检查、用户谓词验证共享 RFC-027 的证明管道。编译器自动生成借用命题（`borrow_conflict`、`use_after_move`、`use_after_drop`、`mut_violation`），管道返回 Proved
/ Disproved / Unproven。

**借用检查没有消失——它降维了。** `BorrowChecker` 变为
`BorrowPredicateEmitter`，生成命题而非执行检查。这和"类型检查器"的概念完全平行：类型检查器生成类型等式命题，借用命题生成器生成借用命题，同一条管道验证。
