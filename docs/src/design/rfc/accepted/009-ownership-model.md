...
**检测方式**：这不是专门的"借用检查器"——这是对令牌值的**流敏感活性分析**。编译器在函数体内追踪每个令牌的状态（活跃/已移动），与追踪任何线性类型值的方式完全相同。
...
> **本节被 [RFC-009a: 令牌生命期分析——基于霍尔证明管道](../draft/009a-borrow-proof-pipeline.md) 修正。** 下文为 RFC-009a 实施前的过渡设计。

令牌冲突检测的完整方案见 RFC-009a。核心要点：

**令牌活性是霍尔逻辑命题**，不是独立的流敏感分析。`{冲突的 ReadToken 全部死亡} write(data) {WriteToken 安全获取}`——和类型检查、谓词验证共享 RFC-027 的证明管道。管道返回 Proved / Disproved / Unproven。

**不需要的**：跨函数生命周期追踪、全局别名分析、借用图约束求解。不是因为"令牌是值，线性追踪就够了"（RFC-009 初稿的错误说法）——是因为品牌树前缀匹配 + 反向 BFS + SMT 逻辑切断替代了 Rust NLL 的 CFG 不动点迭代。

**品牌 ID（`#42`）就是 `'a`。** 信息完全一样，编码不同。区别仅在 `'a` 在类型签名中可见，`#42` 在编译器内部。不是发明了新分析——是把生命周期从类型层降到了证明层。
