...
**消除的复杂性**：
- ❌ 无生命周期 `'a`
- ❌ 无借用检查器（令牌的类型属性——Dup/Linear——替代了专用 borrow checker）
- ❌ 无 GC
...
```
&T      →  零大小，可复制（Dup），授予只读权限
&mut T  →  零大小，线性（非Dup），授予独占读写权限
```

**这不是需要记忆的"规则"——这是类型系统的基本属性。** `Dup` 类型可以自由复制（多个 `&T` 共存），线性类型不能复制（`&mut T` 天然唯一）。没有"借用检查器"——只有类型检查器在做它一直在做的事。
...
filter_by_threshold: (items: List(Point), threshold: &Float) -> List(Point) = {
    # 闭包捕获 threshold 的 &Float 令牌（Dup 类型，自由复制到闭包中）
...
`Dup`（Duplicable，可隐式复制）是编译器自动管理的关键类型属性。实现了 `Dup` 的类型在赋值时不发生所有权转移，而是隐式浅拷贝。`Dup` 蕴含 `Clone`——如果一个类型是 `Dup`，它总能被逐字段克隆；但 `Clone` 不蕴含 `Dup`。

| 类型 | Dup | Clone | 说明 |
|------|-----|-------|------|
| Int, Float, Bool, Char, String, Bytes | ✅（编译器自动实现） | ✅ | 基本类型 |
| `&T` | ✅（零大小，可自由别名） | ✅ | 只读令牌 |
| `&mut T` | ❌（线性，独占） | ❌ | 可变令牌 |
| struct | ✅（所有字段均为 Dup 时自动派生） | ✅ | 结构体 |
| `ref T` | ✅ | ✅ | 共享持有（编译器自动选 Rc/Arc） |
| `*T` | ❌ | ❌ | 裸指针 |

> **注意**：YaoXiang 没有用户可见的 `Send`/`Sync` trait。`ref` 关键字由编译器自动选择 Rc（不跨任务）还是 Arc（跨任务），用户不需要关心类型是否可以跨任务传递。令牌不能跨任务边界是编译器的内置规则，而非 trait 约束。
...
| **借用** | `&T`/`&mut T` 作为零大小令牌类型 | 类型属性（Dup/Linear）自然推导权限，统一类型系统 | 2025-01-15 |
| **借用令牌** | 替代丐版借用，`&T` Dup，`&mut T` Linear | 消除"禁止逃逸"等特殊规则，支持闭包捕获/返回引用/存结构体 | 2026-05-29 |
