...
> **勘误（2026-09-15，RFC-010a）**：本节及下方「返回规则」原以 `return`
> 作为块的值出口，与 RFC-007 的提前返回语义冲突（详见
> [RFC-010a](010a-tail-expression-and-return.md)）。现改为：**块的值 = 尾表达式，`return`
> 退出函数（类型 `Never`）**。原「必须用 `return` 返回值」的错误示例已删除，不在本勘误中保留。
...
> **勘误（2026-09-15，RFC-010a）**：原规则「`= { ... }` 必须用 `return`，否则返回
> `Void`」及理由「需显式 `return`
> 消除『最后一个表达式是否是返回值』的歧义」**已废止**。尾表达式不产生该歧义，`return`
> 亦不干扰它（Rust 已验证同构设计）。
...
> **设计理由（勘误 2026-09-15，RFC-010a）**：`{ ... }`
> 是依赖驱动计算单元（见下文），其求值语义与单表达式不同——花括号引入多语句上下文，
> **其值由尾表达式给出**。原“需显式 `return` 消除歧义”的论证已废止：尾表达式不产生该歧义。
