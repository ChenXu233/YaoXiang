...
### 逻辑运算符：`and` / `or` / `not`（权威定义）

> **定义声明（2026-08-03，issue #251）**：逻辑运算符的权威形式是关键字 `and` / `or` / `not`（与 SPEC
> `syntax.md` §2.2 优先级表一致）。C 风格 `&&` / `||` / `!` 从未进入 SPEC，已从 lexer 移除，书写时直接报错。

**语义**：

| 运算符 | 优先级（SPEC §2.2） | 结合性 | 语义           |
| ------ | ----------------- | ------ | -------------- |
| `not`  | 9（低于比较运算符） | 右到左 | 逻辑非         |
| `and`  | 10                | 左到右 | 短路逻辑与     |
| `or`   | 10                | 左到右 | 短路逻辑或     |
...
not 3 == 4          # true，等价 not (3 == 4)
true or false and false   # true，等价 true or (false and false)
```

以下写法**不再支持**（lexer 报错并提示对应关键字）：
...
1. **消歧**：`&` 在本语言身兼两职——借用令牌（`&p` / `&mut p`，RFC-009）与位与（SPEC §2.2 优先级
   7）。再引入 `&&` 会让一个符号承载三种含义。`and` / `or` / `not` 使借用、位运算、逻辑三个概念在视觉上彻底分离。
2. **自然语法目标**：README 核心设计目标是“Python 般的可读性，接近自然语言”，关键字形式可直接朗读；这也与
   SPEC §2.2 优先级表的原始定义一致（实现中的 `&&` / `||` / `!` 是早期向 C 习惯的漂移，现已纠正）。
3. **先例与反例**：Python / Zig / Lua / Ada / SQL 均用关键字逻辑运算符（Zig 是与本语言同生态位的现代系统语言）；
   Ruby 同时保留 `&&` 与 `and`（两套优先级）是著名 footgun，证明“两个都要”是最差选项——本语言只保留一套。
4. **Curry-Howard 一致性**：类型即命题（见上文同构节），精化类型中的逻辑连接写作 `and` / `or`（如
   `{ 0 <= idx and idx < arr.len }`）是命题的自然表述，而非符号操作。

> **实现**：`and` / `or` 在 IR 层展开为短路跳转序列（`a and b ≡ if a { b } else { false }`），
> `not` 绑定松于比较运算符（操作数按 `BP_LOGICAL_AND + 1` 解析）。回归测试：
