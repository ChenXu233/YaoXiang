...
>
> **补充（裁决 C）**：本文的「空参最简」`name = { ... }`
> 与 RFC-010 的「有值块」语法位置重叠，已裁决为**注解优先、默认函数**——无注解时本文的「空参最简」成立（是函数），写非
> `Fn` 注解时才是块值（`x: Int = { ... }`）。见 RFC-010a 附录D。
...
本 RFC 确定 YaoXiang 语言**函数定义语法**的最终方案。使用统一语法
`name: (params) -> Return = body`，与 RFC-010 的 `name: type = value` 模型完全一致。
