...
2. 标准库各模块单元测试覆盖被阻塞，因为没有可用的测试基础设施
...
- 文件内 per-test `tests` 数组来自 §7 套件收集，随值化模型落地生效
...
`run_project`（orchestrator，目录递归发现）；沿 use 按需发现之后作为纯性能优化叠加。
...
1. 对每个文件按头部指令分流执行（指令文法见 §8.2，经 `src/util/test_markers.rs`
   ...
   - 行为测试：`yaoxiang run <file>` 子进程
     （运行时错误默认携带源码位置与栈帧——debug_map 默认生成，
...
- 明确不采用：进程内 catch 边界（17 关键字铁律）；runner 逐函数入口调用（仅限
  §8.2 编译失败等内部场景）
- API 形态已定案（2026-09-03）：`suite(tests: List((String, () -> Result(Void, String)))) -> Void`；
...
- `assert_not` / `assert_err` / `assert_err_code` 已随值语义族交付（2026-09-03，§3）；
  ...
  同款约束）
- 错误码断言依赖 `Error` 值携带机器可读 `code` 字段——已落地：
...
- 随 Result 化推进，可失败的操作逐个返回 `Result`，语料中的文件级
  负向标记随之迁为文件内断言
...
- **仅服务本仓库语料，不是用户测试框架的一部分**；双 runner 判定约定已收口
  （2026-09-03）：yx_runner（cargo test）与 `yaoxiang test` 共用
...
不设独立机制——会失败的操作按语言方向返回 `Result`，测试统一走
...
| 语料重构（`io.println` → `assert.assert`） | 与 `yaoxiang test` 完全一致的方向 |
| `@` 注解                                      | 不使用，不引入 `@test`            |
...
| 测试回路耗时随语料增长                  | 高   | 主项是每文件全量编译（185 文件实测 11.3s），非子进程启动；`--parallel` 只缓解进程侧，编译成本需测试回路缓存切片 |
...
| 多文件运行   | CLI `run` 委托 `run_project`（orchestrator）作为前置 | 2026-08-02 | 子进程模型继承 CLI 能力；沿 use 按需发现退化为纯性能优化 |
| 报告源码位置 | 运行时错误默认携带                        | 2026-09-07 | debug_map 默认生成，stack trace 输出 `file:line:col`；经嵌入模块（std.test）中转的帧归属不在此保证内，属 RFC-034 |
| 负向测试分层 | 值级反向通用 / 编译失败 runner 结构化标记（仅内部）/ 硬失败归 Result 化 | 2026-09-02 | 值化模型定案；取代隐式 [test:error] 约定 |
...
| 并行执行模型 | `--parallel` 起每核 worker 池（每文件仍独立子进程），skip/invalid 先行按发现序处理、执行结果按完成序流式输出、JSON 按路径排序；`--fail-fast` 停止调度（在途跑完计入）；CWD 仍共享、不扩展隔离边界 | 2026-09-07 | 子进程模型下并行 = OS 线程调度 spawn，无需 yx 层并发；耗时主项是每文件全量编译（风险表），并行只缓解进程侧——测试回路缓存切片才是主缓解 |
