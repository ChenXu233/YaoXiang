...
flowchart TB
    ...
    subgraph LSP["YaoXiang LSP Server"]
        direction TB
    end

    subgraph World["World (编译世界)"]
        SymbolIndex["Symbol Index"]
        TypeEnv["Type Env"]
        Diagnostics["Diagnostics"]
    end

    subgraph DocCache["Document Cache"]
        Version["版本管理"]
        Content["内容缓存"]
        ASTCache["AST 缓存"]
        Incremental["增量变更区域"]
    ...
    subgraph Compiler["Compiler Frontend"]
        Lexer["Lexer (util/span.rs Position)"]
        Parser["Parser (ast.rs 已有 Span)"]
        TypeCheck["Type Check (改为收集模式)"]
        ErrorCollector["ErrorCollector (util/diagnostic/)"]
    end
    ...
    World -.->|"增量更新"| Compiler
    DocCache -->|"提供缓存"| Compiler
    
    World -->|"更新"| SymbolIndex
    World -->|"更新"| TypeEnv
    World -->|"更新"| Diagnostics
    
    DocCache --> Version
    DocCache --> Content
    DocCache --> ASTCache
    DocCache --> Incremental
    
    Compiler --> Lexer
    Compiler --> Parser
    Compiler --> TypeCheck
    Compiler --> ErrorCollector
```
