...
### 1. libuv 是 C 库，无法用于自举后的 YaoXiang

YaoXiang 最终需要自举（用 YaoXiang 自己实现解释器），此时无法依赖 C 库。
libuv 作为 C 库需要 FFI 调用，这将阻碍自举过程。

### 2. tokio 是更合适的选择

Rust 生态中 tokio 是主导的异步运行时（占有率 >90%），它是纯 Rust 实现，
自举后可以通过绑定继续使用，比 libuv 更符合长期架构。

### 3. 实用主义考量

当前阶段应优先让 YaoXiang 语言能够跑通，I/O 实现可以用 Rust std 快速实现，
真正的异步运行时可以在自举后使用 tokio 绑定或自研。
