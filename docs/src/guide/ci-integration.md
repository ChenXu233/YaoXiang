---
title: CI 集成指南
description: 将 yaoxiang check 和 yaoxiang fmt 集成到 CI/CD 流水线
---
...
1. **分离检查和格式化**：分别运行 `check` 和 `fmt --check`，便于定位问题
2. **使用 `--no-progress`**：CI 环境不需要进度条
3. **使用 `--color never`**：避免 ANSI 颜色码污染日志
4. **缓存依赖**：利用 CI 缓存机制加速构建
